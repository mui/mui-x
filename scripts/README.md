...
4. Update the versions of the other `package.json` files and of the dependencies with `pnpm release:version` (`pnpm release:version prerelease` for alpha / beta releases).
   Make sure to not skip the version bump if Lerna is suggesting it. It is important to release the packages if there were **any** changes.
   If Lerna doesn't suggest a version bump for the package, we don't need to release it.
