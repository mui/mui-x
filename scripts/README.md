...
4. Update the versions of the other `package.json` files and of the dependencies with `pnpm release:version` (`pnpm release:version prerelease` for alpha / beta releases).
   Make sure to not skip the version bump if Lerna detected a change in the package. It is important to release the package if there were **any** changes to it.
   If Lerna doesn't suggest a version bump for the package, we don't release it.
