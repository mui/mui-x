...
export function linkify(text: string | undefined, apisToGenerate: string[], format: 'markdown' | 'html') {
    if (text == null) {
        return '';
    }
...
export const isDeclarationReflection = (reflection: TypeDoc.Reflection): reflection is TypeDoc.DeclarationReflection => (reflection instanceof TypeDoc.DeclarationReflection)
...
export function generateSignatureStr(signature: TypeDoc.SignatureReflection, needsParenthesis = false) {
    let text = needsParenthesis ? '(' : '';
