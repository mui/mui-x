...
type Renderer = 'svg-single' | 'webgl';

const POINTS = 5000;

function generateRanges(seed: number): [number, number][] {
  const data = new Array<[number, number]>(POINTS);
  let center = 50;
...
export default function WebGLRangeBars() {
  const [renderer, setRenderer] = React.useState<Renderer>('webgl');
    ...
    <Stack sx={{ width: '100%' }}>
      <FormControl fullWidth>
        ...
        <RadioGroup
          ...
          value={renderer}
          onChange={(event) => setRenderer(event.target.value as Renderer)}
        >
          <FormControlLabel
            value="svg-single"
            control={<Radio />}
