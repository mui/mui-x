...
The `useLegend` hook provides access to the legend data for creating custom legend components.
...
The hook must be called within a chart data provider context (either inside a chart component or within `ChartDataProvider`).
...
|:-----------|:---------|:------------------------------------------------------|
| `id`       | `string \| number` | Unique identifier for the legend item          |
| `label`    | `string` | The text label to display                            |
| `color`    | `string` | The color associated with the legend item            |
| `markType` | `string` | The type of mark to display (`'line'`, `'rect'`, etc.) |
| `seriesId` | `string` | _(optional)_ The identifier of the series            |
| `itemId`   | `string \| number` | _(optional)_ For pie charts, the identifier of the pie slice |

### Requirements

- The hook must be used within a chart context (inside a chart component or `ChartDataProvider`)
- The component using this hook should be rendered as an HTML element, not within the SVG chart surface
        ...
        <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <div
            style={{
              ...
              borderRadius: item.markType === 'line' ? 0 : 2,
            }}
          />
...
### Alternative: useSeries hook

For more advanced use cases, you can use the `useSeries` hook to access raw series data and create your own legend formatting:

```jsx
import { useSeries } from '@mui/x-charts/hooks';

function AdvancedLegend() {
  const series = useSeries();
  
  // Custom logic to format series data into legend items
  const legendItems = Object.keys(series).flatMap(seriesType => {
    const seriesData = series[seriesType];
    if (!seriesData) return [];
    
    return seriesData.seriesOrder.map(seriesId => {
      const seriesInfo = seriesData.series[seriesId];
      return {
        id: seriesId,
        label: seriesInfo.label || seriesId,
