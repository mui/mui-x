...
type Quadrangle = {
  name: string;
  elevation: number;
  // one or more [west, south, east, north] boxes
  boxes: [number, number, number, number][];
};

const QUADRANGLES: Quadrangle[] = [
  // North polar
  {
    name: 'Mare Boreum',
    elevation: -4,
...
const box = ([west, south, east, north]: [number, number, number, number]) => {
  const ring: number[][] = [];
...
export const marsRegions: ExtendedFeatureCollection = {
  type: 'FeatureCollection',
  features: QUADRANGLES.flatMap((quad) =>
    quad.boxes.map((bbox) => ({
      type: 'Feature' as const,
      properties: { name: quad.name, elevation: quad.elevation },
      geometry: { type: 'Polygon' as const, coordinates: box(bbox) },
    })),
  ),
};
 ...
 */
export type MarsFeature = {
  name: string;
  lon: number;
  lat: number;
  kind: 'landmark' | 'mission';
};

export const marsFeatures: MarsFeature[] = [
