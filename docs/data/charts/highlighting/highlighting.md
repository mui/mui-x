...
It can be controlled programmatically or synchronized across multiple charts.

## Highlighting axis

Data can be highlighted based on mouse position.
By default, highlights are lines, but they can also be a vertical band if your x-axis uses `scaleType: 'band'`.
...
See [Line demos—Larger interaction area](/x/react-charts/line-demo/#larger-interaction-area) for more details.
:::
...
The highlighted item can be controlled using the `highlightedItem` and `onHighlightChange` props.
...
The highlighted axis item can be controlled using the `highlightedAxis` prop.
...
:::warning
The `onHighlightedAxisChange` handler can be triggered at a high frequency when the user moves their pointer over the chart.
