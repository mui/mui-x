...
export default function TreemapStyling() {
  ...
  const controls: { label: string; value: number; setValue: (value: number) => void; max: number }[] =
    [
      { label: 'Border radius', value: borderRadius, setValue: setBorderRadius, max: 20 },
      { label: 'Padding inner', value: paddingInner, setValue: setPaddingInner, max: 20 },
      { label: 'Padding outer', value: paddingOuter, setValue: setPaddingOuter, max: 20 },
    ...
    <Stack direction="column" spacing={2} sx={{ width: '100%' }}>
      <Stack direction="row" spacing={4} sx={{ flexWrap: 'wrap' }}>
        {controls.map((control) => (
          <Stack key={control.label} direction="column" sx={{ flex: 1, minWidth: 140 }}>
            <Typography gutterBottom>{control.label}</Typography>
            <Slider
              value={control.value}
              onChange={(event, value) => control.setValue(value as number)}
