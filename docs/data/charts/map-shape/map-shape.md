---
title: React Map chart Shape
productId: x-charts
components: ChartsGeoDataProviderPremium, GeoDataPlot, MapImagePlot, MapShapePlot, MapShape, Graticule, FocusedMapShape
---

# Charts - Map Shape [<span class="plan-premium"></span>](/x/introduction/licensing/#premium-plan 'Premium plan') 🧪

<p class="description">Display and interact with geographic shapes using the mapShape series.</p>
...
But the data from Our World in Data merges them.
Returning `'Somalia'` for both gives them the same key, so the single `{ name: 'Somalia' }`
series item highlights both shapes at once.
...
World maps often have no data for Antarctica.
