...
Instead of loading all events upfront, you can use the `dataSource` prop to fetch events on demand as the user navigates between dates and presets.
The `dataSource.getEvents(start, end)` method is called whenever the visible date range changes, so only the events needed for the current preset are fetched.

## Basic usage

To enable lazy loading, pass a `dataSource` object with a `getEvents` method to the Event Timeline.
