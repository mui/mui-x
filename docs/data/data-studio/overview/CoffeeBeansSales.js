...
function createDemoCellValue(column, rowIdField, rowIndex) {
...
function createDemoRow(columns, rowIdField) {
...
function CoffeeBeansDataGrid(inProps) {
  const {
    apiRef,
    columns: columnsProp,
    dataSource,
  ...
  const handleCreateRow = React.useCallback(async () => {
    ...
    try {
      ...
      const createdRow = await createRow(demoRow);
  ...
  const handleDeleteClick = React.useCallback(
    (id) => async () => {
      const deleteRow = dataSource?.deleteRow;
    ...
    <GridChartsIntegrationContextProvider>
      ...
      >
        <DataGridPremium
          ...
          getRowId={
            dataset.getRowId ??
            (dataset.rowIdField ? (row) => row[dataset.rowIdField] : undefined)
          }
...
export default function CoffeeBeansSales() {
  ...
  const [addedRowId, setAddedRowId] = React.useState(null);
  ...
  const handleRowAdded = React.useCallback((id) => {
    setAddedRowId(id);
  }, []);
  ...
  const datasetsWithAddRow = React.useMemo(
    () =>
      datasets.map((dataset) => ({
        ...dataset,
        onAddRow: dataset.dataSource?.createRow
      ...
      {schemaError === null ? (
        <React.Fragment>
          ...
          <DataStudio
            ...
            routing={routing}
            slots={{
              dataGrid: CoffeeBeansDataGrid,
              chartView: CoffeeBeansChartView,
            }}
            sx={{
              height: '100dvh',
              bgcolor: 'background.paper',
