...
`ChatBox` renders a `ChatProvider` internally, so hooks work naturally inside any component rendered as a child or descendent of `ChatBox`:
...
The component will not re-render when a new message is sent — only when the status fields themselves change.
...
Use it sparingly — the dedicated hooks above are simpler, better typed, and will remain stable across minor versions.
