...
Customize the headless primitives to produce a full-page ChatGPT-style chat layout—dark theme, sidebar with conversation history, no message bubbles, and a centered composer.

- Two-pane layout with a fixed-width dark sidebar and a main thread area.
- Conversation list with title only, unread blue-dot indicator, and a three-dot menu on hover.
...
The demo below shows the full ChatGPT-style layout assembled from the headless primitives:
...
- Hover-based three-dot menu using React state and a fixed-position dropdown.
- Set `borderRadius: 24` on the composer to produce a pill-shaped input area.
