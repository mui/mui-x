...
function getRowText(message: ChatMessage | null) {
  if (!message) {
    return null;
  }
...
function RenderCountChip({ count }: { count: number }) {
  return (
    <Chip
      size="small"
...
const GranularRow = React.memo(function GranularRow({ id }: { id: string }) {
...
function CoarsePane() {
  ...
  const renderCounts = React.useRef<Map<string, number>>(new Map());
