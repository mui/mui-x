...
<p class="description">Read chat state and trigger runtime actions from custom components built on top of <code>@mui/x-chat</code>.</p>

{{"component": "@mui/internal-core-docs/ComponentLinkHeader"}}

`ChatBox` covers most cases, but sometimes you need to reach into chat state from components that live outside `ChatBox`.
Examples include a page header that shows streaming status, a sidebar that renders conversation metadata, or a custom toolbar that controls the composer.
...
`useChatStore()` returns the underlying `ChatStore<Cursor>` instance directly.
...
- See [Adapters](/x/react-chat/backend/adapters/) for details on the interface that the actions call into.
