...
function createStream(text: string, thinkingDelay: number) {
...
type IndicatorMode = 'auto' | 'always' | 'never';

const MODE_OPTIONS: Array<{ label: string; value: IndicatorMode }> = [
  { label: "'auto' (default)", value: 'auto' },
  { label: 'true (always)', value: 'always' },
  { label: 'false (never)', value: 'never' },
];

const FEATURE_VALUE: Record<IndicatorMode, boolean | 'auto'> = {
  auto: 'auto',
  always: true,
  never: false,
};
...
export default function ChatStreamingIndicatorDemo() {
  const [mode, setMode] = React.useState<IndicatorMode>('auto');
