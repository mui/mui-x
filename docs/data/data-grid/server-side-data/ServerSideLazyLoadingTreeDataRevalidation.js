...
const experimentalStockTree = EXPERIMENTAL_SECTOR_SPECS.map(
  (sector, sectorIndex) => ({
    id: sector.id,
    name: sector.name,
...
const STOCK_TREE = [...BASE_STOCK_TREE, ...experimentalStockTree];
...
function fakeTreeStockServer(params) {
  const tick = getTick();
...
function ServerSideLazyLoadingTreeDataRevalidation() {
...
export default ServerSideLazyLoadingTreeDataRevalidation;
