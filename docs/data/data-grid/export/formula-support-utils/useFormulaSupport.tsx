...
export type FormulaColumnDef = Omit<GridColDef, 'type'> & {
  /**
   * Column type. Set to 'formula' to enable formula support for this column.
   * This will automatically add renderEditCell, renderHeader, and editable: true.
   */
...
export interface UseFormulaSupportOptions {
  /**
   * Column definitions. Columns with `type: 'formula'` will be enhanced with formula support.
   */
  columns: FormulaColumnDef[];
  /**
   * Row data in Data Grid format (array of objects with named properties).
   * Values can be strings, numbers, or formula strings (starting with '=').
   * The hook will convert this to HyperFormula's internal 2D array format.
   */
  initialData: Record<string, RawCellContent>[];
  /**
   * Grid API ref from useGridApiRef().
   */
  apiRef: ReturnType<typeof useGridApiRef>;
  // Optional function to generate row IDs from row data.
  getRowId?: (row: Record<string, RawCellContent>, index: number) => GridRowId;
  /**
   * Optional HyperFormula sheet name.
   * @default 'Sheet1'
   */
  sheetName?: string;
}
...
export interface UseFormulaSupportResult {
  /**
   * Enhanced columns with formula support injected for columns with `type: 'formula'`.
   */
  columns: GridColDef[];
  /**
   * Minimal rows containing just id and _hfRowIndex reference.
   * Cell values are retrieved via valueGetter from HyperFormula.
   */
  rows: GridRowModel[];
  /**
   * Props for an optional formula bar component.
   */
  formulaBarProps: FormulaBarProps;
  /**
   * Current formula value being edited.
   */
  ...
  currentField: string;
  /**
   * Whether the current value is a formula (starts with '=').
   */
  isFormula: boolean;
  /**
   * The HyperFormula context value for use with HyperFormulaContext.Provider.
   */
  ...
  // Checks if a field name already exists.
