...
If we take for example the movie dataset, you can detect autogenerated rows like this:

```tsx
{
  field: 'title',
  valueGetter: (value, row) => {
    if (String(apiRef.current.getRowId(row)).startsWith('auto-generated-row-')) {
