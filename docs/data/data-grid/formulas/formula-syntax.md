...
Pinned rows have positions (top-pinned rows come before the data rows, bottom-pinned rows after) and can be referenced individually, but ranges and `COLUMN_VALUES` never include them—this is what lets a pinned summary row aggregate the data without covering itself.
