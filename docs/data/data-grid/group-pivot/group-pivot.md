...
### Get the row in a group

You can use the `apiRef.current.getGroupingCriteriaRows` method to get all rows contained inside a group.
It will not contain the autogenerated rows (ie the subgroup rows or the aggregation footers).

```ts
const rows = apiRef.current.getGroupingCriteriaRows({
  groupId: params.id,

  // If true, the rows will be in the order displayed on screen
  // If false, the rows will be in the order provided to the grid
  applySorting: true,
...
If you want to get the row id of a group given its grouping criteria, use `getGroupRowIdFromPath`
...
{{"demo": "RowGroupingGetGroupingCriteriaRows.js", "bg": "inline", "defaultCodeOpen": false}}
