...
When the `columns` prop reference is updated, the column width is reset to the `colDef.width` value and any resizes will be lost. This is because the Data Grid considers update of the columns prop as a new set of columns, and the previous state is discarded.

To persist the column width when the `columns` prop is updated, set the `persistColumnsWidth` prop to `true`.
