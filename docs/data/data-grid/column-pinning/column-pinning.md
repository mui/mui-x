...
It may be desirable to allow one or more columns to be pinned (or unpinned) programmatically which don't support pinning (i.e. columns for which prop `disableColumnPinning = true` or `colDef.pinnable = false`).
