...
function CustomDateField(props: DatePickerFieldProps) {
  // sx, slots and slotProps are MUI-specific concepts before `@mui/material` v6.0
  const { slots, slotProps, sx, ...other } = props;
  const {
    internalProps,
    forwardedProps: {
      // The container ref is never used on non-range pickers and will be removed before the next major.
      ref: unusedRef,
      InputProps,
      inputProps,
      inputRef,
  ...
  return (
    ...
    >
      ...
      <input
        ...
        style={{ border: `1px solid ${hasValidationError ? 'red' : 'grey'}` }}
      />
...
export default function BrowserDatePicker() {
  return (
