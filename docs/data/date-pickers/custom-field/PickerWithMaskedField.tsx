...
import {splitFieldInternalAndForwardedProps, useValidation, validateDate} from "@mui/x-date-pickers/internals";
...
function getFormatHelpTextFromFormat(format: string) {
    ...
    ).replace(/a/gi, '(a|p)m').toLocaleLowerCase();
}
...
function getMaskFromCurrentFormat(format: string) {
    ...
    const inferredFormatPatternWith2Digits = staticDateWith2DigitTokens.format(format).replace(ACCEPT_REGEX, '_')
...
function MaskedField(props: MaskedFieldProps) {
    ...
    const { forwardedProps, internalProps } = splitFieldInternalAndForwardedProps<
        typeof props,
        keyof UseDateFieldProps<Dayjs, false>
    >(props, 'date');
    ...
    return (
        <TextField placeholder={formatHelperText} error={!!validationError} {...rifmProps} {...forwardedProps}  />
    );
}
...
function MaskedDatePicker(
    ...
    return (
        <DatePicker
            slots={{ ...props.slots, field: MaskedField }}
            {...props}
        />
    );
}
