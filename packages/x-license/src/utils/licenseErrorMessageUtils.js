...
function showLicenseKeyPlanMismatchError(_a) {
    var packageName = _a.packageName;
    var rootPackageName = packageName.replace(/-(premium|pro)$/, '');
    showError([
        ...
        "Or if you didn't intend to use Premium features, you can replace the import of `".concat(rootPackageName, "-premium` with `").concat(rootPackageName, "-pro`."),
    ]);
}
