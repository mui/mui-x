...
/**
 * Parses MONTHLY BYDAY expecting exactly one ordinal entry (e.g. 2TU or -1FR).
 * Returns normalized tokens with positive/negative ordinals.
 * @throws if missing, multiple, or missing ordinal.
 */
...
export function countMonthlyOccurrencesUpToExact(
  ...
  // Path A: BYDAY with ordinal (single). Not mixed with BYMONTHDAY.
  if (rule.byDay?.length) {
    if (rule.byMonthDay?.length) {
      throw new Error('MONTHLY: use either BYDAY (ordinal) or BYMONTHDAY, not both.');
    }
