...
function GridAiAssistantPanelConversationsMenu() {
    ...
    return (_jsxs(React.Fragment, { children: [_jsx(rootProps.slots.baseTooltip, { title: apiRef.current.getLocaleText('aiAssistantPanelConversationHistory'), enterDelay: 500, children: _jsx("span", { children: _jsx(rootProps.slots.baseIconButton, { ...rootProps.slotProps?.baseIconButton, disabled: conversations.length === 0, id: triggerId, "aria-haspopup": "true", "aria-controls": open ? menuId : undefined, "aria-expanded": open ? 'true' : undefined, "aria-label": apiRef.current.getLocaleText('aiAssistantPanelConversationHistory'), onClick: handleOpen, ref: triggerRef, children: _jsx(rootProps.slots.aiAssistantPanelHistoryIcon, { fontSize: "small" }) }) }) }), _jsx(GridMenu, { target: triggerRef.current, open: open, onClose: handleClose, position: "bottom-end", children: _jsx(rootProps.slots.baseMenuList, { id: menuId, "aria-labelledby": triggerId, autoFocusItem: true, ...rootProps.slotProps?.baseMenuList, children: sortedConversations.map((conversation, sortedIndex) => {
                        const conversationIndex = conversations.findIndex((c) => c === conversation);
                        return (_jsx(rootProps.slots.baseMenuItem, { selected: conversationIndex === activeConversationIndex, onClick: () => {
