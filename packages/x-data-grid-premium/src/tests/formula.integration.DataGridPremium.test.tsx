...
describe('<DataGridPremium /> - Formulas feature integration', () => {
  ...
  describe('export', () => {
    it('should export evaluated values to CSV, escaping `=` results and bypassing the valueFormatter for errors', async () => {
      await render(
        <Test
          rows={[
            {
          ...
          columns={[
            ...
            { field: 'escaped', allowFormulas: true },
            {
              field: 'errored',
              type: 'number',
              allowFormulas: true,
      ...
      // Evaluated value, never the source.
      expect(csv).to.include('6');
      expect(csv).not.to.include('price * 3');
      // A string result starting with `=` is escaped.
      expect(csv).to.include("'=2");
      // Error codes bypass the column valueFormatter.
  ...
  describe('row grouping', () => {
    ...
    it('should exclude autogenerated group rows and the grouping column from the position context', async () => {
      await render(
        <Test
          rows={[
            {
              ...
              category: 'x',
              price: 2,
              sumAll: '=SUM(COLUMN_VALUES("price"))',
              firstRow: '=REF(COLUMN("price"), ROW_POSITION(1))',
              firstColumn: '=REF(COLUMN_POSITION(1), ROW(0))',
            },
          ...
          columns={[
            { field: 'category' },
            { field: 'price', type: 'number' },
            { field: 'sumAll', type: 'number', allowFormulas: true },
            { field: 'firstRow', type: 'number', allowFormulas: true },
            { field: 'firstColumn', allowFormulas: true },
          ]}
          initialState={{ rowGrouping: { model: ['category'] } }}
          defaultGroupingExpansionDepth={-1}
        />,
      );
      // Autogenerated group rows contribute nothing to COLUMN_VALUES.
      expect(formulaApi().getCellFormulaResult(0, 'sumAll')).to.deep.equal({
        type: 'value',
        value: 10,
      });
      // Position 1 is the first leaf, not the autogenerated group header
      // (whose `price` would resolve to null).
      expect(formulaApi().getCellFormulaResult(0, 'firstRow')).to.deep.equal({
        type: 'value',
        value: 2,
      });
      ...
      expect(formulaApi().getCellFormulaResult(0, 'firstColumn')).to.deep.equal({
        type: 'value',
        value: 'x',
      });
    });
