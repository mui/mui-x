...
describe('<DataGridPremium /> - Formula autocomplete', () => {
  ...
  // Signature help renders the call signature verbatim. `value1` never appears in
  // a formula the user typed, so the document text is an unambiguous probe.
  const AVERAGE_SIGNATURE = 'AVERAGE(value1, value2, …)';
  ...
  it('hides the signature help once the call is closed over a range argument', async () => {
    ...
    // Caret inside AVERAGE's argument list: its signature is the help to show.
    typeFormula(editable, '=ROUND(AVERAGE(B5:D5', 20);
    await waitFor(() => {
      expect(document.body.textContent).to.contain(AVERAGE_SIGNATURE);
    });

    // Both calls closed, caret at the end: nothing left to help with. The `:` used
    // to abort the tokenization, leaving AVERAGE reported as the enclosing call.
    typeFormula(editable, '=ROUND(AVERAGE(B5:D5))');
    await waitFor(() => {
      expect(document.body.textContent).not.to.contain(AVERAGE_SIGNATURE);
    });
    expect(getListbox()).to.equal(null);
  });
  ...
  it('suggests column letters for the end of a range being typed', async () => {
    ...
    typeFormula(editable, '=SUM(A1:D');
    await waitFor(() => {
      expect(getOptionLabels()).to.deep.equal(['D']);
    });
  });
