...
function ColumnPropTest() {
    return (<div>
      ...
      <x_data_grid_premium_1.DataGridPremium rows={[]} columns={[
            {
                field: 'firstName',
                // @ts-expect-error
                groupingValueGetter: function (value, row) { return row.lastName; },
            },
        ]}/>
      ...
      <x_data_grid_premium_1.DataGridPremium rows={[]} columns={[
            {
                field: 'firstName',
                groupingValueGetter: function (value, row) { return row.firstName; },
            },
        ]}/>
      {/* Wrong column without explicit generic on DataGrid */}
      <x_data_grid_premium_1.DataGridPremium rows={[{ firstName: 'John' }]} columns={[
            {
                field: 'firstName',
                // @ts-expect-error
                groupingValueGetter: function (value, row) { return row.lastName; },
...
function ApiRefPrivateMethods() {
    var apiRef = (0, x_data_grid_premium_1.useGridApiRef)();
