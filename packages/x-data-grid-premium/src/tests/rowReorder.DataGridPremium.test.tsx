...
describe.skipIf(isJSDOM)('<DataGridPremium /> - Row reorder with row grouping', () => {
  ...
  describe('Edge cases', () => {
    ...
    describe.skip('priority handling', () => {
      ...
      it('should prioritize dataSource.editRow over processRowUpdate', async () => {
        const processRowUpdate = spy();
        const editRowSpy = spy(async (params) => params.updatedRow);
        const dataSource: GridDataSource = {
          getRows: async () => ({ rows: [...singleLevelData] }),
          updateRow: editRowSpy,
          getGroupKey: (row) => row.category,
          getChildrenCount: () => 0,
        };
        ...
        // processRowUpdate should not be called when dataSource is present
        expect(processRowUpdate.callCount).to.equal(0);
      });
    });
  });
