...
const getGroupAggregatedValue = (groupId, apiRef, aggregationRowsScope, aggregatedFields, aggregationRules, position, applySorting, valueGetters, publicApi, groupAggregatedValuesLookup, columnsLookup) => {
    ...
    for (let i = 0; i < aggregatedValues.length; i += 1) {
        ...
        const formattedValue = aggregationFunction.valueFormatter
            ? aggregationFunction.valueFormatter(value, rowForFormatter, columnsLookup[aggregatedField], apiRef)
            : undefined;
...
export const createAggregationLookup = ({ apiRef, aggregationRules, aggregatedFields, aggregationRowsScope, getAggregationPosition, isDataSource, applySorting = false, }) => {
