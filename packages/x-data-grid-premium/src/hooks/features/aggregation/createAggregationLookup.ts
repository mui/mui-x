...
const getGroupAggregatedValue = (
  ...
  // For autogenerated rows, we need to create a row object with aggregated values for the valueFormatter
  const autogeneratedAggregatedRow: Record<string, any> = { id: groupId };
  ...
  for (let i = 0; i < aggregatedValues.length; i += 1) {
    ...
    // Add the aggregated value to the autogenerated row
    autogeneratedAggregatedRow[aggregatedField] = value;
  }

  for (let i = 0; i < aggregatedValues.length; i += 1) {
    const { aggregatedField } = aggregatedValues[i];
    const aggregationFunction = aggregationRules[aggregatedField]
      .aggregationFunction as GridAggregationFunction;
    const value = autogeneratedAggregatedRow[aggregatedField];

    // Use the actual row from rowLookup if it exists, otherwise use the compiled autogenerated row
    const rowForFormatter = rowLookup[groupId] || autogeneratedAggregatedRow;
