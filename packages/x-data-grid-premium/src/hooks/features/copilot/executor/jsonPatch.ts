...
function cloneAlong(root: unknown, tokens: string[]): unknown {
  ...
  // Path drives into a primitive; let the caller handle it.
  return root;
}
 ...
 *
 * Throws if `op` is not one of the allowed operations or the path is malformed.
 * Throws if the parent location does not exist for `add`/`remove`/`replace`.
 */
export function applyJsonPatch<T>(doc: T, op: JsonPatchOp): T {
  if (op.op !== 'replace' && op.op !== 'add' && op.op !== 'remove') {
    throw new Error(`MUI X Data Grid: unsupported JSON Patch op '${op.op}'.`);
  }
