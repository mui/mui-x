...
import { gridHistoryCurrentPositionSelector, gridHistoryStackSelector, gridHistoryCanUndoSelector, gridHistoryCanRedoSelector, } from './gridHistorySelectors';
...
export const useGridHistory = (apiRef, props) => {
    ...
    const isValidationNeeded = React.useMemo(() => isEnabled &&
        historyValidationEvents.length > 0 &&
        Object.values(historyEventHandlers).some((handler) => handler.validate), [isEnabled, historyEventHandlers, historyValidationEvents]);
    ...
    useGridApiMethod(apiRef, { history: historyApi }, 'public');
