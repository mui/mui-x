...
import { gridChartsPanelOpenSelector, gridChartsDimensionsSelector, gridChartsValuesSelector, gridChartsIntegrationActiveChartIdSelector, gridChartableColumnsSelector, gridChartsIntegrationChartsLookupSelector, } from './gridChartsIntegrationSelectors';
...
import { getAggregationFunctionLabel, getAvailableAggregationFunctions, } from '../aggregation/gridAggregationUtils';
...
export const chartsIntegrationStateInitializer = (state, props) => {
    ...
    const charts = Object.fromEntries(Object.entries(props.initialState?.chartsIntegration?.charts || {}).map(([chartId, chart]) => [
        chartId,
        {
            dimensions: (chart.dimensions || [])
                .map((dimension) => typeof dimension === 'string' ? { field: dimension, hidden: false } : dimension)
                .filter((dimension) => columnsLookup[dimension.field]?.chartable === true &&
                !isBlockedForSection(columnsLookup[dimension.field], 'dimensions', rowGroupingModel, pivotModel)),
            values: (chart.values || [])
                .map((value) => (typeof value === 'string' ? { field: value, hidden: false } : value))
                .filter((value) => columnsLookup[value.field]?.chartable === true &&
                !isBlockedForSection(columnsLookup[value.field], 'values', rowGroupingModel, pivotModel)),
        },
    ]));
    return {
        ...state,
...
export const useGridChartsIntegration = (apiRef, props) => {
    ...
    const handleRowDataUpdate = React.useCallback((chartIds) => {
        if (chartIds.length === 0 ||
            chartIds.some((chartId) => !visibleDimensions.current[chartId] || !visibleValues.current[chartId])) {
            return;
        }
        ...
        const dataColumns = [
            ...
            if (isColumnGrouped) {
                const groupedFieldName = isColumnGrouped
                    ? getRowGroupingFieldFromGroupingCriteria(orderedFields.includes(GRID_ROW_GROUPING_SINGLE_GROUPING_FIELD)
                        ? null
                        : column.field)
                    : column.field;
        ...
        chartIds.forEach((chartId) => {
            ...
            for (let i = 0; i < valuesSize; i += 1) {
                if (chartableColumns[selectedFields[chartId].values[i].field] &&
                    !isBlockedForSection(chartableColumns[selectedFields[chartId].values[i].field], 'values', rowGroupingModel, pivotActive ? pivotModel : undefined)) {
                    if (!values[chartId]) {
                        values[chartId] = [];
                    }
            ...
            for (let i = 0; i < dimensionsSize; i += 1) {
                ...
                if (!selectedFields[chartId].values.some((valueItem) => valueItem.field === item.field) &&
                    chartableColumns[item.field] &&
                    !isBlockedForSection(chartableColumns[item.field], 'dimensions', rowGroupingModel, pivotActive ? pivotModel : undefined)) {
    ...
    const updateChartDimensionsData = React.useCallback((chartId, dimensions) => {
        ...
        apiRef.current.setState((state) => {
            const newDimensions = typeof dimensions === 'function'
    ...
    const updateDataReference = React.useCallback((field, originSection, targetSection, targetField, placementRelativeToTargetField) => {
        ...
        if (targetSection) {
            ...
            const currentMaxItems = targetSection === 'dimensions'
        ...
        if (targetSection) {
            ...
            const remainingItems = targetSection === originSection
    ...
    const addChartsPanel = React.useCallback((initialValue, value) => {
        if (props.slots.chartsPanel &&
            ...
            return _jsx(props.slots.chartsPanel, { ...props.slotProps?.chartsPanel });
        }
        return initialValue;
    }, [props, isChartsIntegrationAvailable]);
    useGridRegisterPipeProcessor(apiRef, 'sidebar', addChartsPanel);
    useGridApiMethod(apiRef, { chartsIntegration: { updateDataReference, getColumnName } }, 'private');
    ...
    const stateExportPreProcessing = React.useCallback((prevState, exportContext) => {
        ...
        const integrationContextToExport = Object.fromEntries(Object.entries(chartStateLookup).map(([chartId, chartState]) => [
            chartId,
    ...
    const stateRestorePreProcessing = React.useCallback((params, restoreContext) => {
        ...
        const { activeChartId: activeChartIdToRestore, charts: chartsToRestore, integrationContext, } = chartsRestoreState;
