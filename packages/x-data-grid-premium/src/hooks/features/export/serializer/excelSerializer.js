...
import { addColumnGroupingHeaders, addSerializedRowToWorksheet, createValueOptionsSheetIfNeeded, getExcelJs, } from './utils';
...
const defaultColumnsStyles = {
    [GRID_DATE_COL_DEF.type]: { numFmt: 'dd.mm.yyyy' },
    [GRID_DATETIME_COL_DEF.type]: { numFmt: 'dd.mm.yyyy hh:mm' },
};
...
export async function buildExcel(options, apiRef) {
    const { columns, rowIds, includeHeaders, includeColumnGroupsHeaders, valueOptionsSheetName = 'Options', exceljsPreProcess, exceljsPostProcess, columnsStyles = {}, } = options;
    const excelJS = await getExcelJs();
    const workbook = new excelJS.Workbook();
    const worksheet = workbook.addWorksheet('Sheet1');
