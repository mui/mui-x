...
export const useGridCellSelection = (apiRef, props) => {
    ...
    const handleCellMouseOver = React.useCallback((params, event) => {
        ...
        apiRef.current.selectCellRange(lastMouseDownCell.current, { id, field }, event.ctrlKey || event.metaKey);
    ...
    const checkIfCellIsSelected = React.useCallback((isSelected, { id, field }) => {
        return apiRef.current.isCellSelected(id, field);
    }, [apiRef]);
    const addClassesToCells = React.useCallback((classes, { id, field }) => {
        const visibleRows = getVisibleRows(apiRef);
    ...
    const canUpdateFocus = React.useCallback((initialValue, { event, cell }) => {
