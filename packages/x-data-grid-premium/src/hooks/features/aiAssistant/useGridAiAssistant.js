...
var gridChartsIntegrationSelectors_1 = require("../chartsIntegration/gridChartsIntegrationSelectors");
var DEFAULT_SAMPLE_COUNT = 5;
var MAX_CHART_DATA_POINTS = 1000;
...
var useGridAiAssistant = function (apiRef, props) {
    var onPrompt = props.onPrompt, allowAiAssistantDataSampling = props.allowAiAssistantDataSampling, slots = props.slots, rowSelection = props.rowSelection, disableColumnFilter = props.disableColumnFilter, disableRowGrouping = props.disableRowGrouping, disableAggregation = props.disableAggregation, disableColumnSorting = props.disableColumnSorting, disablePivoting = props.disablePivoting, chartsIntegration = props.chartsIntegration, experimentalFeatures = props.experimentalFeatures, getPivotDerivedColumns = props.getPivotDerivedColumns;
    var previousUnwrappedGroupingModel = React.useRef([]);
    ...
    var preferencePanelPreProcessing = React.useCallback(function (initialValue, value) {
        if (isAiAssistantAvailable &&
            ...
            return (0, jsx_runtime_1.jsx)(slots.aiAssistantPanel, {});
        }
        return initialValue;
    ...
    var getPromptContext = React.useCallback(function (allowDataSampling) {
        ...
        var columnsContext = columns.reduce(function (acc, column) {
            var _a, _b, _c, _d, _e, _f;
            var columnContextWithoutExamples = {
                field: column.field,
                description: (_a = column.description) !== null && _a !== void 0 ? _a : null,
                examples: [],
                type: (_b = column.type) !== null && _b !== void 0 ? _b : 'string',
                allowedOperators: (_d = (_c = column.filterOperators) === null || _c === void 0 ? void 0 : _c.map(function (operator) { return operator.value; })) !== null && _d !== void 0 ? _d : [],
            };
            acc.push(__assign(__assign({}, columnContextWithoutExamples), { examples: (_f = (_e = examples[column.field]) !== null && _e !== void 0 ? _e : column.examples) !== null && _f !== void 0 ? _f : [] }));
            if (disablePivoting) {
                return acc;
            }
            ((getPivotDerivedColumns === null || getPivotDerivedColumns === void 0 ? void 0 : getPivotDerivedColumns(column, apiRef.current.getLocaleText)) || []).forEach(function (col) {
                return acc.push(__assign(__assign(__assign({}, columnContextWithoutExamples), col), { derivedFrom: column.field }));
            });
            return acc;
        }, []);
    ...
    var updateChart = React.useCallback(function (result) {
        if (!result.chart) {
            return;
        }
        apiRef.current.updateChartDimensionsData(activeChartId, result.chart.dimensions.map(function (item) { return ({ field: item }); }));
        apiRef.current.updateChartValuesData(activeChartId, result.chart.values.map(function (item) { return ({ field: item }); }));
    ...
    var applyPromptResult = React.useCallback(function (result) {
        ...
        if ((experimentalFeatures === null || experimentalFeatures === void 0 ? void 0 : experimentalFeatures.charts) && chartsIntegration && activeChartId && result.chart) {
            if (appliedPivoting) {
                var unsubscribe_1 = apiRef.current.subscribeEvent('rowsSet', function () {
                    var unwrappedGroupingModel = Object.keys((0, x_data_grid_pro_1.gridColumnGroupsUnwrappedModelSelector)(apiRef));
                    // wait until unwrapped grouping model changes
                    if (!result.chart ||
                        unwrappedGroupingModel.length === 0 ||
                        (0, isDeepEqual_1.isDeepEqual)(previousUnwrappedGroupingModel.current, unwrappedGroupingModel)) {
                        return;
                    }
                    previousUnwrappedGroupingModel.current = unwrappedGroupingModel;
                    var visibleRowsCount = (0, x_data_grid_pro_1.gridVisibleRowsSelector)(apiRef).rows.length;
                    var maxColumns = Math.floor(MAX_CHART_DATA_POINTS / visibleRowsCount);
        ...
        var selection = rowSelection ? result.select : -1;
        if (selection !== -1) {
        ...
        activeChartId,
        chartsIntegration,
        experimentalFeatures === null || experimentalFeatures === void 0 ? void 0 : experimentalFeatures.charts,
    ]);
