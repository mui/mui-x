...
import { gridPivotModelSelector, gridPivotActiveSelector, gridPivotPanelOpenSelector, } from './gridPivotingSelectors';
...
const emptyPivotModel = { rows: [], columns: [], values: [] };
...
export const useGridPivoting = (apiRef, props, originalColumnsProp, originalRowsProp) => {
    ...
    const computePivotingState = React.useCallback(({ active, model: pivotModel }) => {
    ...
    const updatePivotModel = React.useCallback(({ field, targetSection, originSection, targetField, targetFieldPosition }) => {
        ...
        apiRef.current.setPivotModel((prev) => {
            ...
            const hidden = originSection === null
                ? false
    ...
    useGridApiMethod(apiRef, { updatePivotModel, updateNonPivotColumns, updateNonPivotRows }, 'private');
    useEnhancedEffect(() => {
