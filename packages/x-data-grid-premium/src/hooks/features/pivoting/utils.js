...
exports.createPivotPropsFromRows = exports.getPivotForcedProps = exports.getInitialColumns = exports.defaultGetPivotDerivedColumns = void 0;
...
var sortColumnGroups = function (columnGroups, pivotModelColumns, depth) {
    var _a;
    ...
    if (columnGroups.length < 2) {
        if ((_a = columnGroups[0]) === null || _a === void 0 ? void 0 : _a.children) {
...
var getPivotForcedProps = function (pivotModel, columns, groupingColDef) {
    ...
    for (var _i = 0, _a = columns.values(); _i < _a.length; _i++) {
        var column = _a[_i];
...
var createPivotPropsFromRows = function (_a) {
    var _b;
    var rows = _a.rows, columns = _a.columns, pivotModel = _a.pivotModel, pivotingColDef = _a.pivotingColDef, apiRef = _a.apiRef;
    var visibleColumns = pivotModel.columns.filter(function (column) { return !column.hidden; });
    var visibleRows = pivotModel.rows.filter(function (row) { return !row.hidden; });
    var visibleValues = pivotModel.values.filter(function (value) { return !value.hidden; });
    var pivotColumns = [];
    ...
    if (visibleColumns.length === 0) {
        ...
        var _loop_2 = function (i) {
            ...
            for (var j = 0; j < visibleColumns.length; j += 1) {
                ...
                var groupId = columnGroupPath.join(columnGroups_1.COLUMN_GROUP_ID_SEPARATOR);
                if (!columnGroupingModelLookup.has(groupId)) {
                    ...
                    if (depth === 0) {
                        ...
                        var parentGroupId = columnGroupPath.slice(0, -1).join(columnGroups_1.COLUMN_GROUP_ID_SEPARATOR);
                ...
                if (isLastColumnGroupLevel) {
                    visibleValues.forEach(function (pivotValue) {
                        ...
                        var valueKey = "".concat(columnGroupPath.join(columnGroups_1.COLUMN_GROUP_ID_SEPARATOR)).concat(columnGroups_1.COLUMN_GROUP_ID_SEPARATOR).concat(valueField);
        ...
        var _loop_3 = function (i) {
            ...
            if (isLastColumnGroupLevel) {
                if (visibleValues.length === 0) {
                    ...
                    var emptyColumnField = "".concat(columnGroup.groupId).concat(columnGroups_1.COLUMN_GROUP_ID_SEPARATOR, "empty");
                    var emptyColumn = {
                    ...
                    visibleValues.forEach(function (pivotValue) {
                        var valueField = pivotValue.field;
                        var mapValueKey = "".concat(columnGroup.groupId).concat(columnGroups_1.COLUMN_GROUP_ID_SEPARATOR).concat(valueField);
                        var overrides = typeof pivotingColDef === 'function'
                            ? pivotingColDef(valueField, columnGroup.groupId.split(columnGroups_1.COLUMN_GROUP_ID_SEPARATOR))
                            : pivotingColDef;
                        var column = __assign(__assign(__assign({ headerName: String(valueField) }, getAttributesFromInitialColumn(pivotValue.field)), overrides), { 
                            // pivoting values are always numbers
                            type: 'number', field: mapValueKey, aggregable: false, groupable: false, filterable: false, hideable: false, editable: false, disableReorder: true, availableAggregationFunctions: [pivotValue.aggFunc] });
