...
var reorderValidator_1 = require("../rowReorder/reorderValidator");
...
var useGridRowGrouping = function (apiRef, props) {
    ...
    var getRowReorderTargetIndex = React.useCallback(function (initialValue, _a) {
        ...
        if ((0, x_data_grid_pro_1.gridRowMaximumTreeDepthSelector)(apiRef) === 1 || props.treeData) {
            return initialValue;
        }
        var expandedSortedRowIndexLookup = (0, internals_1.gridExpandedSortedRowIndexLookupSelector)(apiRef);
        var expandedSortedRowIds = (0, x_data_grid_pro_1.gridExpandedSortedRowIdsSelector)(apiRef);
        var rowTree = (0, x_data_grid_pro_1.gridRowTreeSelector)(apiRef);
        var sourceRowIndex = expandedSortedRowIndexLookup[sourceRowId];
        var targetRowIndex = expandedSortedRowIndexLookup[targetRowId];
        var sourceNode = rowTree[sourceRowId];
        var targetNode = rowTree[targetRowId];
        var prevNode = targetRowIndex > 0 ? rowTree[expandedSortedRowIds[targetRowIndex - 1]] : null;
        var nextNode = targetRowIndex < expandedSortedRowIds.length - 1
        ...
        var context = {
            ...
            expandedSortedRowIndexLookup: expandedSortedRowIndexLookup,
        };
        // Check if the reorder is valid
        if (reorderValidator_1.rowGroupingReorderValidator.validate(context)) {
