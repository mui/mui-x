...
var STEPS = [
    ...
    { views: internals_1.TIME_VIEWS, rangePosition: 'start' },
    { views: ['day'], rangePosition: 'end' },
    { views: internals_1.TIME_VIEWS, rangePosition: 'end' },
];
var rendererInterceptor = function RendererInterceptor(props) {
    ...
    var view = rendererProps.view, openTo = rendererProps.openTo, otherRendererProps = __rest(rendererProps, ["view", "openTo"]);
    var finalProps = __assign(__assign({}, otherRendererProps), { sx: [
            (_a = {
    ...
    var isTimeView = (0, internals_1.isInternalTimeView)(popperView);
    var viewRenderer = viewRenderers[popperView];
    if (!viewRenderer) {
        return null;
    }
    if (isTimeView) {
        return (<DateTimeRangePickerTimeWrapper_1.DateTimeRangePickerTimeWrapper {...finalProps} viewRenderer={viewRenderer} view={view && (0, internals_1.isInternalTimeView)(view) ? view : 'hours'} views={finalProps.views} openTo={(0, internals_1.isInternalTimeView)(openTo) ? openTo : 'hours'}/>);
    }
    // avoiding problem of `props: never`
    var typedViewRenderer = viewRenderer;
    return typedViewRenderer(__assign(__assign({}, finalProps), { availableRangePositions: [rangePosition], views: finalProps.views.filter(internals_1.isDatePickerView), view: view && (0, internals_1.isDatePickerView)(view) ? view : 'day', openTo: (0, internals_1.isDatePickerView)(openTo) ? openTo : 'day' }));
};
/**
 * Demos:
 *
 ...
 */
var MobileDateTimeRangePicker = React.forwardRef(function MobileDateTimeRangePicker(inProps, ref) {
    var _a, _b;
    var adapter = (0, hooks_1.usePickerAdapter)();
    // Props with the default values common to all date time range pickers
    var defaultizedProps = (0, shared_1.useDateTimeRangePickerDefaultizedProps)(inProps, 'MuiMobileDateTimeRangePicker');
    ...
    var props = __assign(__assign({}, defaultizedProps), { viewRenderers: viewRenderers, format: (0, internals_1.resolveDateTimeFormat)(adapter, defaultizedProps, true), 
        // Force one calendar on mobile to avoid layout issues
        calendars: 1, 
        // force true to correctly handle `renderTimeViewClock` as a renderer
        ampmInClock: true, 
        // force current calendar position, since we only have one calendar
        currentMonthCalendarPosition: 1, slots: __assign({ field: SingleInputDateTimeRangeField_1.SingleInputDateTimeRangeField }, defaultizedProps.slots), slotProps: __assign(__assign({}, defaultizedProps.slotProps), { field: function (ownerState) {
                var _a;
...
exports.MobileDateTimeRangePicker = MobileDateTimeRangePicker;
