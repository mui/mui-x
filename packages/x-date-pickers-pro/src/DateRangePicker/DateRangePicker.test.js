...
describe('<DateRangePicker />', function () {
    ...
    it('should not use the mobile picker by default', function () {
        (0, pickers_1.stubMatchMedia)(true);
        // Test with accessible DOM structure
        var unmount = renderWithProps({ enableAccessibleFieldDOMStructure: true }).unmount;
        (0, pickers_1.openPicker)({ type: 'date-range', initialFocus: 'start', fieldType: 'single-input' });
        expect(createRenderer_1.screen.queryByRole('dialog')).to.have.class(internals_1.pickerPopperClasses.root);
        unmount();
    ...
    it('should use the mobile picker when `useMediaQuery` returns `false`', function () {
        (0, pickers_1.stubMatchMedia)(false);
        // Test with accessible DOM structure
        var unmount = renderWithProps({ enableAccessibleFieldDOMStructure: true }).unmount;
        (0, pickers_1.openPicker)({ type: 'date-range', initialFocus: 'start', fieldType: 'single-input' });
        expect(createRenderer_1.screen.queryByRole('dialog')).not.to.have.class(internals_1.pickerPopperClasses.root);
        unmount();
        // Test with non-accessible DOM structure
        renderWithProps({ enableAccessibleFieldDOMStructure: false });
        (0, pickers_1.openPicker)({ type: 'date-range', initialFocus: 'start', fieldType: 'single-input' });
    ...
    describe('form behavior', function () {
        function TestComponent(_a) {
          ...
          <DateRangePicker_1.DateRangePicker name="testDate" defaultValue={[new Date('2022-04-17'), new Date('2022-04-21')]} {...other}/>
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 2:
                        ...
                        expect(__spreadArray([], handleSubmit.lastCall.args[0], true)[0]).to.deep.equal([
                            'testDate',
        ...
        it('should not submit the form when "Enter" is pressed on the multi input field', function () { return __awaiter(void 0, void 0, void 0, function () {
            var handleSubmit, user;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        handleSubmit = (0, sinon_1.spy)();
                        user = render(<TestComponent onSubmit={handleSubmit} slots={{ field: MultiInputDateRangeField_1.MultiInputDateRangeField }} name={undefined}/>).user;
                        // focus the input
                        return [4 /*yield*/, user.keyboard('{Tab}')];
                    case 1:
                        // focus the input
