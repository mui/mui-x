...
describe('<DataGridPro /> - Row selection', () => {
  ...
  describe('prop: checkboxSelectionVisibleOnly = false', () => {
    ...
    it('should unselect all rows of all the pages if 1 row of another page is selected', async () => {
      const { user } = render(
        <TestDataGridSelection
          checkboxSelection
          initialState={{ pagination: { paginationModel: { pageSize: 2 } } }}
  ...
  describe('prop: checkboxSelectionVisibleOnly = true', () => {
    ...
    it('should unselect all the rows of the current page if 1 row of the current page is selected', async () => {
      const { user } = render(
        <TestDataGridSelection
          checkboxSelection
          initialState={{ pagination: { paginationModel: { pageSize: 2 } } }}
  ...
  describe('prop: rowSelectionPropagation = { descendants: false, parents: false }', () => {
    ...
    it('should not auto select the parent when all the children are selected using controlled row selection model', async () => {
      ...
      expect(onRowSelectionModelChange.callCount).to.equal(0);
      act(() => {
        setProps({ rowSelectionModel: [2, 3, 4, 5, 6, 7] });
      });
      expect(onRowSelectionModelChange.callCount).to.equal(0);
    });
  ...
  describe('prop: rowSelectionPropagation = { descendants: true, parents: false }', () => {
    ...
    it('should auto select descendants when a parent is selected using controlled row selection model', async () => {
      ...
      expect(onRowSelectionModelChange.lastCall.args[0]).to.deep.equal([1, 2, 3, 4, 5, 6, 7]);
    });
  ...
  describe('prop: rowSelectionPropagation = { descendants: true, parents: true }', () => {
    ...
    describe("prop: indeterminateCheckboxAction = 'deselect'", () => {
      it('should deselect all the children when selecting an indeterminate parent', async () => {
        const { user } = render(
          <SelectionPropagationGrid
            defaultGroupingExpansionDepth={-1}
            density="compact"
