...
describe('<DataGridPro /> - Edit components', () => {
  ...
  describe('column type: string', () => {
    ...
    it('should call setEditCellValue with debounce', async () => {
      ...
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0]).to.deep.equal({
        id: 0,
        field: 'brand',
        value: 'Puma',
        debounceMs: 200,
    ...
    it('should call onValueChange if defined', async () => {
      ...
      expect(onValueChange.mock.calls[onValueChange.mock.calls.length - 1][1]).to.equal('Puma');
    });
  });
  ...
  describe('column type: number', () => {
    ...
    it('should keep values as numbers', async () => {
      ...
      await user.type(input, '[Backspace>2]10');
      await waitFor(() =>
        expect(preProcessEditCellPropsSpy.mock.calls[preProcessEditCellPropsSpy.mock.calls.length - 1][0].props.value).to.equal(110),
      );
    });
  ...
  describe('column type: date', () => {
    ...
    it('should call setEditCellValue with the value converted to Date', async () => {
      ...
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].id).to.equal(0);
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].field).to.equal('createdAt');
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].debounceMs).to.equal(undefined);
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].value?.toISOString()).to.equal(
    ...
    it('should call setEditCellValue with null when entered an empty value', async () => {
      ...
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].value).to.equal(null);
    });
    ...
    it('should handle correctly dates with partial years', async () => {
      ...
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].value.getTime()).to.equal(
    ...
    it('should call onValueChange if defined', async () => {
      ...
      expect((onValueChange.mock.calls[onValueChange.mock.calls.length - 1][1]! as Date).toISOString()).to.equal(
        new Date(2022, 1, 10).toISOString(),
      );
    });
  });
  ...
  describe('column type: dateTime', () => {
    ...
    it('should call setEditCellValue with the value converted to Date', async () => {
      ...
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].field).to.equal('createdAt');
      expect(spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].debounceMs).to.equal(undefined);
      expect((spiedSetEditCellValue.mock.calls[spiedSetEditCellValue.mock.calls.length - 1][0].value! as Date).toISOString()).to.equal(
  ...
  describe('column type: singleSelect', () => {
    ...
    it('should call onValueChange if defined', async () => {
      ...
      expect(onValueChange.callCount).to.equal(1);
      expect(onValueChange.mock.calls[onValueChange.mock.calls.length - 1][1]).to.equal('Adidas');
    });
  ...
  describe('column type: boolean', () => {
    ...
    it('should call onValueChange if defined', async () => {
      ...
      expect(onValueChange.callCount).to.equal(1);
      expect(onValueChange.mock.calls[onValueChange.mock.calls.length - 1][1]).to.equal(true);
    });
  });
});
