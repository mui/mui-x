...
describe('<DataGridPro /> - Detail panel', () => {
  ...
  it('should only call getDetailPanelHeight on the rows that have detail content', () => {
    ...
    expect(getDetailPanelHeight.callCount).to.equal(expectedCallCount);
    expect(getDetailPanelHeight.mock.calls[getDetailPanelHeight.mock.calls.length - 1][0].id).to.equal(0);
  });
  ...
  describe('prop: onDetailPanelsExpandedRowIds', () => {
    it('should call when a row is expanded or closed', async () => {
      ...
      expect(handleDetailPanelsExpandedRowIdsChange.mock.calls[handleDetailPanelsExpandedRowIdsChange.mock.calls.length - 1][0]).to.deep.equal(new Set([0]));
      await user.click(screen.getAllByRole('button', { name: 'Expand' })[0]); // Expand the 2nd row
      expect(handleDetailPanelsExpandedRowIdsChange.mock.calls[handleDetailPanelsExpandedRowIdsChange.mock.calls.length - 1][0]).to.deep.equal(
        new Set([0, 1]),
      );
      await user.click(screen.getAllByRole('button', { name: 'Collapse' })[0]); // Close the 1st row
      expect(handleDetailPanelsExpandedRowIdsChange.mock.calls[handleDetailPanelsExpandedRowIdsChange.mock.calls.length - 1][0]).to.deep.equal(new Set([1]));
      await user.click(screen.getAllByRole('button', { name: 'Collapse' })[0]); // Close the 2nd row
      expect(handleDetailPanelsExpandedRowIdsChange.mock.calls[handleDetailPanelsExpandedRowIdsChange.mock.calls.length - 1][0]).to.deep.equal(new Set([]));
    });
