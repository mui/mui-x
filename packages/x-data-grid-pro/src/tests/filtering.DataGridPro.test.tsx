...
describe('<DataGridPro /> - Filter', () => {
  ...
  it("should call onFilterModelChange with reason=changeLogicOperator when the logic operator changes but doesn't change the state", () => {
    ...
    expect(onFilterModelChange.mock.calls[onFilterModelChange.mock.calls.length - 1][1].reason).to.equal('changeLogicOperator');
    expect(getColumnValues(0)).to.deep.equal([]);
  });
  ...
  it('should call onFilterModelChange with reason=upsertFilterItem when the value is emptied', async () => {
    ...
    expect(onFilterModelChange.mock.calls[onFilterModelChange.mock.calls.length - 1][1].reason).to.equal('upsertFilterItem');
  });
  ...
  it('should call onFilterModelChange with reason=deleteFilterItem when a filter is removed', () => {
    ...
    expect(onFilterModelChange.callCount).to.equal(1);
    expect(onFilterModelChange.mock.calls[onFilterModelChange.mock.calls.length - 1][1].reason).to.equal('deleteFilterItem');
  });
  ...
  it('should call onFilterModelChange with reason=upsertFilterItems when a filter is added', () => {
    ...
    expect(onFilterModelChange.callCount).to.equal(1);
    expect(onFilterModelChange.mock.calls[onFilterModelChange.mock.calls.length - 1][1].reason).to.equal('upsertFilterItems');
  });
  ...
  it('should publish filterModelChange with the reason whenever the model changes', () => {
    ...
    expect(listener.mock.calls[listener.mock.calls.length - 1][1].reason).to.equal('upsertFilterItems');
  });
  ...
  describe('Header filters', () => {
    ...
    it('should allow temporary invalid values while updating the number filter', async () => {
      ...
      expect(changeSpy.mock.calls[changeSpy.mock.calls.length - 1][0].items[0].value).to.equal(0);

      await user.keyboard('.');
      await waitFor(() => expect(getColumnValues(0)).to.deep.equal(['10', '100', '1,000']));
      expect(changeSpy.mock.calls[changeSpy.mock.calls.length - 1][0].items[0].value).to.equal(0); // 0.

      await user.keyboard('1');
      await waitFor(() => expect(getColumnValues(0)).to.deep.equal(['10', '100', '1,000']));
      await waitFor(() => expect(changeSpy.mock.calls[changeSpy.mock.calls.length - 1][0].items[0].value).to.equal(0.1)); // 0.1

      await user.keyboard('e');
      await waitFor(() => expect(getColumnValues(0)).to.deep.equal(['-10', '10', '100', '1,000']));
      expect(changeSpy.mock.calls[changeSpy.mock.calls.length - 1][0].items[0].value).to.equal(undefined); // 0.1e

      await user.keyboard('2');
      await waitFor(() => expect(getColumnValues(0)).to.deep.equal(['100', '1,000']));
      expect(changeSpy.mock.calls[changeSpy.mock.calls.length - 1][0].items[0].value).to.equal(10); // 0.1e2
    });
