...
describe('<DataGridPro /> - Filter', () => {
  ...
  describe('api method: `upsertFilterItems`', () => {
    it('should be able to add multiple filters', async () => {
      render(<TestCase getRowId={(row) => row.brand} />);

      act(() =>
        apiRef.current?.upsertFilterItems([
          { field: 'brand', value: 'i', operator: 'contains', id: 1 },
          { field: 'brand', value: 'as', operator: 'contains', id: 2 },
        ]),
      );
      await waitFor(() => {
    ...
    it('should not remove filters which are not passed to `upsertFilterItems`', async () => {
      render(<TestCase getRowId={(row) => row.brand} />);
      ...
      act(() =>
        apiRef.current?.upsertFilterItems([
          { field: 'brand', value: '', operator: 'contains', id: 2 },
        ]),
      );
      await waitFor(() => {
  ...
  it('slotProps `filterColumns` and `getColumnForNewFilter` should allow custom filtering', async () => {
  ...
  it('should call `getColumnForNewFilter` when filters are added', async () => {
  ...
  it('should apply the filterModel prop correctly', async () => {
  ...
  it('should not apply items that are incomplete with AND operator', async () => {
    render(
      <TestCase
        filterModel={{
          items: [
            { id: 1, field: 'brand', value: 'a', operator: 'contains' },
            { id: 2, field: 'brand', operator: 'contains' },
          ],
  ...
  it('should not apply items that are incomplete with OR operator', async () => {
    render(
      <TestCase
        filterModel={{
  ...
  it('should apply the filterModel prop correctly on GridApiRef setRows', async () => {
    render(<TestCase filterModel={filterModel} />);
  ...
  it('should apply the filterModel prop correctly on GridApiRef update row data', async () => {
    render(<TestCase filterModel={filterModel} />);
  ...
  it('should allow apiRef to setFilterModel', async () => {
    ...
    act(() =>
      apiRef.current?.setFilterModel({
        items: [{ field: 'brand', value: 'a', operator: 'startsWith' }],
      }),
    );
    await waitFor(() => {
      expect(getColumnValues(0)).to.deep.equal(['Adidas']);
  ...
  it('should allow multiple filter via apiRef', async () => {
    ...
    const newModel = {
      items: [
        { id: 1, field: 'brand', value: 'a', operator: 'startsWith' },
        { id: 2, field: 'brand', value: 's', operator: 'endsWith' },
      ],
    };
  ...
  it('should hide `Add filter` in filter panel when `disableAddFilterButton` is `true`', async () => {
    render(
      <TestCase
        initialState={{
          preferencePanel: {
  ...
  it('should hide `Remove all` in filter panel when `disableRemoveAllButton` is `true`', async () => {
    render(
      <TestCase
        initialState={{
          preferencePanel: {
  ...
  it('should allow multiple filter and changing the logicOperator', async () => {
    const newModel: GridFilterModel = {
      items: [
        { id: 1, field: 'brand', value: 'a', operator: 'startsWith' },
        { id: 2, field: 'brand', value: 'a', operator: 'endsWith' },
      ],
      logicOperator: GridLogicOperator.Or,
    };
    render(<TestCase filterModel={newModel} />);
  ...
  it('should call onFilterModelChange with reason=deleteFilterItem when a filter is removed', async () => {
    const onFilterModelChange = spy();
    render(
      <TestCase
        onFilterModelChange={onFilterModelChange}
  ...
  it('should call onFilterModelChange with reason=upsertFilterItems when a filter is added', async () => {
    const onFilterModelChange = spy();
    render(
      <TestCase
        onFilterModelChange={onFilterModelChange}
  ...
  it('should publish filterModelChange with the reason whenever the model changes', async () => {
    const listener = spy();
    render(
      <TestCase
        initialState={{
  ...
  it('should only select visible rows', async () => {
    const newModel: GridFilterModel = {
      items: [{ field: 'brand', value: 'a', operator: 'startsWith' }],
      logicOperator: GridLogicOperator.Or,
    };
  ...
  it('should allow to clear filters by passing an empty filter model', async () => {
    const newModel: GridFilterModel = {
      items: [{ field: 'brand', value: 'a', operator: 'startsWith' }],
    };
  ...
  it('should display the number of results in the footer', async () => {
  ...
  describe('control Filter', () => {
    it('should update the filter state when neither the model nor the onChange are set', async () => {
      render(
        <TestCase
          initialState={{
            preferencePanel: {
    ...
    it('should not update the filter state when the filterModelProp is set', async () => {
    ...
    it('should update the filter state when the model is not set, but the onChange is set', async () => {
    ...
    it('should control filter state when the model and the onChange are set', async () => {
  ...
  it.skipIf(isJSDOM)('should give a stable ID to the filter item used as placeholder', async () => {
  ...
  describe('Header filters', () => {
    it('should reflect the `filterModel` prop in header filters correctly', async () => {
    ...
    it('should allow to change the operator from operator menu', async () => {
      const onFilterModelChange = spy();
      render(
        <TestCase
          initialState={{
    ...
    it('should allow to clear the filter from operator menu', async () => {
      render(
        <TestCase
          initialState={{
            filter: {
    ...
    it('should allow to clear the filter with clear button', async () => {
      render(
        <TestCase
          initialState={{
            filter: {
    ...
    it('should allow to customize header filter cell using `renderHeaderFilter`', async () => {
      render(
        <TestCase
          columns={[
    ...
    it('should allow to customize header filter cell using `filterOperators`', async () => {
      render(
        <TestCase
          columns={[
            {
        ...
        render(
          <TestCase
            ...
            initialState={{
              filter: {
                filterModel: {
                  items: [{ field: 'brand', operator: 'looksLike', value: 'a' }],
                },
              },
            }}
            headerFilters
    ...
    it('should work correctly with boolean column type', async () => {
      ...
      const { user } = render(
        <TestCase
          ...
          initialState={{
            filter: {
              filterModel: {
                items: [{ field: 'brand', operator: 'contains', value: 'abc' }],
              },
            },
          }}
        />,
  ...
  describe('Read-only filters', () => {
    const columns: GridColDef[] = [
      { field: 'id', type: 'number', filterable: false },
      { field: 'brand' },
    ];

    it('should allow multiple filters for `filterable: false` columns', async () => {
      const newModel = {
        items: [
          { id: 1, field: 'id', value: 0, operator: '>' },
          { id: 2, field: 'id', operator: 'isNotEmpty' },
          { id: 3, field: 'brand', value: 'm', operator: 'contains' },
        ],
      };
    ...
    it('should disable `Remove all` button for only read-only filters', async () => {
      const newModel = {
        items: [{ id: 1, field: 'id', value: 0, operator: '>' }],
      };
