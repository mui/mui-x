...
function GridDetailPanelToggleCell(props) {
    ...
    const isExpanded = useGridSelector({ current: api }, isExpandedSelector, rowId);
    const rootProps = useGridRootProps();
    const apiRef = useGridApiContext();
    const ownerState = { classes: rootProps.classes, isExpanded };
    ...
    return (_jsx(rootProps.slots.baseIconButton, { size: "small", tabIndex: -1, disabled: !hasContent, className: classes.root, "aria-expanded": isExpanded, "aria-label": isExpanded
            ? apiRef.current.getLocaleText('collapseDetailPanel')
            : apiRef.current.getLocaleText('expandDetailPanel'), ...rootProps.slotProps?.baseIconButton, children: _jsx(Icon, { fontSize: "inherit" }) }));
}
