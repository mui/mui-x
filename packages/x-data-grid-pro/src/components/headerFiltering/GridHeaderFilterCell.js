...
const GridHeaderFilterCell = forwardRef((props, ref) => {
    const { colIndex, height, hasFocus, width, headerClassName, colDef, item, headerFilterMenuRef, InputComponentProps, showClearIcon = false, pinnedPosition, pinnedOffset, style: styleProp, showLeftBorder, showRightBorder, ...other } = props;
    ...
    const InputComponent = colDef.filterable || isFilterReadOnly
        ? (currentOperator.InputComponent ?? DEFAULT_INPUT_COMPONENTS[colDef.type])
        : null;
    ...
    const label = currentOperator.headerLabel ??
        apiRef.current.getLocaleText(`headerFilterOperator${capitalize(item.operator)}`);
    ...
    const headerFilterMenu = (_jsx(GridHeaderFilterMenuContainer, { operators: filterOperators, item: item, field: colDef.field, disabled: isFilterReadOnly, applyFilterChanges: apiRef.current.upsertFilterItem, headerFilterMenuRef: headerFilterMenuRef, buttonRef: buttonRef, showClearItem: !showClearIcon && isApplied, clearFilterItem: clearFilterItem }));
    const clearButton = showClearIcon && isApplied ? (_jsx(GridHeaderFilterClearButton, { onClick: clearFilterItem, disabled: isFilterReadOnly })) : null;
    return (_jsxs("div", { className: clsx(classes.root, headerClassName), style: attachPinnedStyle({
            height,
            width,
            ...styleProp,
        }, isRtl, pinnedPosition, pinnedOffset), role: "columnheader", "aria-colindex": colIndex + 1, "aria-label": headerFilterComponent == null ? (colDef.headerName ?? colDef.field) : undefined, ...other, ...mouseEventsHandlers, ref: handleRef, children: [headerFilterComponent, headerFilterComponent === undefined ? (_jsxs(React.Fragment, { children: [isNoInputOperator ? (_jsxs(React.Fragment, { children: [_jsx(OperatorLabel, { className: classes.operatorLabel, children: label }), clearButton, headerFilterMenu] })) : null, InputComponent && !isNoInputOperator ? (_jsx(StyledInputComponent, { as: InputComponent, className: classes.input, apiRef: apiRef, item: item, inputRef: inputRef, applyValue: apiRef.current.upsertFilterItem, onFocus: () => apiRef.current.startHeaderFilterEditMode(colDef.field), onBlur: (event) => {
                            apiRef.current.stopHeaderFilterEditMode();
                        ...
                        }, ...(isNoInputOperator ? { value: '' } : {}), ...currentOperator?.InputComponentProps, ...InputComponentProps })) : null] })) : null] }));
});
