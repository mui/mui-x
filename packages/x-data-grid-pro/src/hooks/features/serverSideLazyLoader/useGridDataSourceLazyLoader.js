...
export const useGridDataSourceLazyLoader = (privateApiRef, props) => {
    ...
    const handleDataUpdate = React.useCallback((params) => {
        ...
        if (rowsStale.current && params.fetchParams.start === 0) {
            ...
            const startingIndex = typeof fetchParams.start === 'string'
            ...
            if (isRevalidation) {
                ...
                const sameRowIds = existingRowIds.length === newRowIds.length &&
    ...
    const handleRenderedRowsIntervalChange = React.useCallback((renderContext) => {
        ...
        if (renderedRowsIntervalCache.current.firstRowToRender === renderContext.firstRowIndex &&
    ...
    const handleStrategyActivityChange = React.useCallback(() => {
        setLazyLoadingRowsUpdateStrategyActive(privateApiRef.current.getActiveStrategy(GridStrategyGroup.DataSource) ===
