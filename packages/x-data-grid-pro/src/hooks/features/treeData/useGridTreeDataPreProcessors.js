...
export const useGridTreeDataPreProcessors = (privateApiRef, props) => {
    ...
    const getGroupingColDef = React.useCallback(() => {
        ...
        const commonProperties = {
            ...
            renderCell: (params) => (_jsx(GridTreeDataGroupingCell, { ...params, hideDescendantCount: hideDescendantCount })),
    ...
    const createRowTreeForTreeData = React.useCallback((params) => {
        if (!props.getTreeDataPath) {
            throw new Error('MUI X Data Grid: No getTreeDataPath function provided. ' +
        ...
        const getRowTreeBuilderNode = (rowId) => ({
            id: rowId,
            path: props.getTreeDataPath(params.dataRowIdToModelLookup[rowId]).map((key) => ({ key, field: null })),
        });
        const onDuplicatePath = (firstId, secondId, path) => {
            throw new Error(`MUI X Data Grid: The path returned by getTreeDataPath must be unique for each row. ` +
