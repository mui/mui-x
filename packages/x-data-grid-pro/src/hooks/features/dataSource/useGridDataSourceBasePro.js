...
var useGridDataSourceBasePro = function (apiRef, props, options) {
    ...
    var _b = (0, internals_1.useGridDataSourceBase)(apiRef, props, __assign({ fetchRowChildren: nestedDataManager.queue, clearDataSourceState: clearDataSourceState, handleEditRow: handleEditRow }, options)), api = _b.api, debouncedFetchRows = _b.debouncedFetchRows, flatTreeStrategyProcessor = _b.strategyProcessor, events = _b.events, cacheChunkManager = _b.cacheChunkManager, cache = _b.cache;
    var setStrategyAvailability = React.useCallback(function () {
        var targetStrategy = props.treeData
    ...
    var handleGroupedDataUpdate = React.useCallback(function (params) {
        if ('error' in params) {
            apiRef.current.setRows([]);
            return;
        }
        var response = params.response, keepChildrenExpanded = params.options.keepChildrenExpanded;
        ...
        if (keepChildrenExpanded === false) {
            ...
            var tree_1 = (0, x_data_grid_1.gridRowTreeSelector)(apiRef);
            // Remove existing outdated rows before setting the new ones
            // Create a set of the current root rows
            var parentRowsToDelete_1 = new Set((0, internals_1.getTreeNodeDescendants)(tree_1, x_data_grid_1.GRID_ROOT_GROUP_ID, false, true));
            // Remove from the list the rows that are again in the response
            response.rows.forEach(function (row) {
                parentRowsToDelete_1.delete((0, x_data_grid_1.gridRowIdSelector)(apiRef, row));
            });
            var rowsToDelete_2 = [];
            if (parentRowsToDelete_1.size > 0) {
                parentRowsToDelete_1.forEach(function (parentRowId) {
                    var descendants = (0, internals_1.getTreeNodeDescendants)(tree_1, parentRowId, false, false);
                    for (var i = descendants.length - 1; i >= 0; i -= 1) {
                        // delete deepest descendants first
                        rowsToDelete_2.push({ id: descendants[i], _action: 'delete' });
                    }
                    rowsToDelete_2.push({ id: parentRowId, _action: 'delete' });
                });
            }
            apiRef.current.updateRows(response.rows.concat(rowsToDelete_2));
        }
    ...
    return {
        ...
        flatTreeStrategyProcessor: flatTreeStrategyProcessor,
        groupedDataStrategyProcessor: {
            strategyName: internals_1.DataSourceRowsUpdateStrategy.GroupedData,
