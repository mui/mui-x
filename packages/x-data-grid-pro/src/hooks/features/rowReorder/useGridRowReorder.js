...
export const rowReorderStateInitializer = (state) => ({
    ...state,
    rowReorder: {
        isActive: false,
    },
...
export const useGridRowReorder = (apiRef, props) => {
        ...
        requestAnimationFrame(() => {
            ...
            newRows.forEach((row) => {
                ...
                if (Math.abs(deltaY) > 1) {
                    const animation = row.animate([{ transform: `translateY(${deltaY}px)` }, { transform: 'translateY(0)' }], {
                        duration: 200,
                        easing: 'ease-in-out',
                        fill: 'forwards',
                    });
    ...
    const handleDragOver = React.useCallback((params, event) => {
        ...
        if (previousReorderState.current.previousTargetId !== params.id ||
            ...
            const isAdjacentPosition = (dropPosition === 'above' && targetRowIndex === sourceRowIndex + 1) ||
