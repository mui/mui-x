...
var removeDataRowFromTree = function (_a) {
    var id = _a.id, tree = _a.tree, treeDepths = _a.treeDepths, groupingName = _a.groupingName;
    var node = tree[id];
    if (node.type === 'group' && node.children.length > 0) {
        replaceDataGroupWithAutoGeneratedGroup({ node: node, tree: tree, treeDepths: treeDepths });
    }
    else if (groupingName === gridTreeDataUtils_1.TreeDataStrategy.Default ||
        groupingName === internals_1.RowGroupingStrategy.Default) {
        removeNodeAndCleanParent({ node: node, tree: tree, treeDepths: treeDepths });
    }
    else {
        removeNode({ node: node, tree: tree, treeDepths: treeDepths });
    }
