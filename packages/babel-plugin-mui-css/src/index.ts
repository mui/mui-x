    ...
    visitor: {
      CallExpression(path: Babel.NodePath, { file }) {
        ...
        if (calleeName === 'css') {
          ...
          //   class1: { ... },
          // });
          const [prefixNode, classesNode] = argumentNodes;
          ...
          stylesNode = classesNode;
        } else if (calleeName === 'slot') {
          // const slotN = slot({ name: 'Grid', slot: 'label' }, {
          //   class1: { ... },
          // });
          const [metaNode, classesNode] = argumentNodes;
        ...
        try {
          const classes = evaluateStyles(state, stylesSource);
          ...
          switch (calleeName) {
            ...
            case 'slot': {
              argumentNodes[1] = buildClassesNode(file.metadata.muiCSS, prefix, classes);
              break;
            }
            default:
          }
      ...
      Program: {
        exit(path, params) {
          ...
          targetImpl[BuildTarget.CSS].finalize(path, params);
        },
      },
    },
  };
