...
export const getHourSectionOptions = <TDate extends PickerValidDate>({
  ...
  // `utils.setHours(now, N)` on a transition day can roll forward and emit
  ...
  const labelReferenceDate = utils.setDate(utils.setMonth(utils.startOfDay(now), 0), 15);

  const endHour = ampm ? 11 : 23;
  for (let hour = 0; hour <= endHour; hour += timeStep) {
    let label = utils.format(
      utils.setHours(labelReferenceDate, hour),
