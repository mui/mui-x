...
describe('findClosestEnabledDate', () => {
  const day18thText = adapterToUse.format(new Date(2018, 7, 18), 'dayOfMonth');
  ...
  it('should return null if all dates are disabled', () => {
    const result = findClosestEnabledDate({
      ...
      minDate: new Date(1999, 0, 1), // Use close-by min/max dates to reduce the test runtime.
  ...
  it('should return given date if it is enabled', () => {
    ...
    expect(adapterToUse.isSameDay(result, new Date(2000, 0, 1))).to.equal(true);
  });
  ...
  it('should return next 18th going from 10th', () => {
    ...
    expect(adapterToUse.isSameDay(result, new Date(2018, 7, 18))).to.equal(true);
  });
  ...
  it('should return previous 18th going from 1st', () => {
    ...
    expect(adapterToUse.isSameDay(result, new Date(2018, 6, 18))).to.equal(true);
  });
