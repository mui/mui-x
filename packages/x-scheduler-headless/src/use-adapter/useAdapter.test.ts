  ...
  describe('diffIn', () => {
    describe('minutes', () => {
      it('returns whole-minute difference for same-hour timestamps (floored)', () => {
        const later = adapter.date('2025-04-10T10:45:30Z', 'default');
        const earlier = adapter.date('2025-04-10T10:00:10Z', 'default');
        // 45m 20s -> 45
      ...
      it('returns 0 when both instants are identical', () => {
        const t = adapter.date('2025-04-10T10:45:30Z', 'default');
    ...
    describe('days', () => {
      it('ignores time-of-day and compares calendar dates only (end of day vs start of day)', () => {
        const endOf = adapter.date('2025-02-15T23:59:59Z', 'default');
        const startOf = adapter.date('2025-02-10T00:00:01Z', 'default');
      ...
      it('handles month boundary correctly (Feb to Mar)', () => {
        const a = adapter.date('2025-03-02T05:00:00Z', 'default');
        const b = adapter.date('2025-02-27T18:00:00Z', 'default');
        // 27 -> 28 -> 1 -> 2 = 3
        expect(diffIn(adapter, a, b, 'days')).to.equal(3);
      });

      it('returns negative days when order is reversed', () => {
        const earlier = adapter.date('2025-03-02T12:00:00Z', 'default');
        const later = adapter.date('2025-03-05T01:00:00Z', 'default');
    ...
    describe('weeks', () => {
      it('computes whole-week difference based on startOfWeek alignment', () => {
        const week3 = adapter.date('2025-01-18T09:00:00Z', 'default');
        const week1 = adapter.date('2025-01-03T09:00:00Z', 'default');
        expect(diffIn(adapter, week3, week1, 'weeks')).to.equal(2);
      });

      it('returns 0 for dates inside the same calendar week', () => {
        const monday = adapter.date('2025-01-06T08:00:00Z', 'default');
        const friday = adapter.date('2025-01-10T18:00:00Z', 'default');
        expect(diffIn(adapter, friday, monday, 'weeks')).to.equal(0);
      });

      it('returns negative when first date is in an earlier week', () => {
        const earlierWeek = adapter.date('2025-01-06T12:00:00Z', 'default');
        const laterWeek = adapter.date('2025-01-27T12:00:00Z', 'default');
    ...
    describe('months', () => {
      it('computes difference across year boundary', () => {
        const a = adapter.date('2026-03-10T00:00:00Z', 'default');
        const b = adapter.date('2025-12-10T00:00:00Z', 'default');
        // Dec(2025) -> Mar(2026) = 3
        expect(diffIn(adapter, a, b, 'months')).to.equal(3);
      });

      it('returns negative for earlier month', () => {
        const earlierMonth = adapter.date('2025-05-01T00:00:00Z', 'default');
        const laterMonth = adapter.date('2025-07-01T00:00:00Z', 'default');
        expect(diffIn(adapter, earlierMonth, laterMonth, 'months')).to.equal(-2);
      });

      it('returns 0 for same month regardless of day/time', () => {
        const a = adapter.date('2025-07-31T23:59:59Z', 'default');
        const b = adapter.date('2025-07-01T00:00:00Z', 'default');
    ...
    describe('years', () => {
      it('returns positive difference across multiple years', () => {
        const a = adapter.date('2030-05-01T00:00:00Z', 'default');
        const b = adapter.date('2025-05-01T00:00:00Z', 'default');
        expect(diffIn(adapter, a, b, 'years')).to.equal(5);
      });

      it('returns negative difference when first date is earlier', () => {
        const a = adapter.date('2022-12-31T23:59:59Z', 'default');
        const b = adapter.date('2025-01-01T00:00:00Z', 'default');
        expect(diffIn(adapter, a, b, 'years')).to.equal(-3);
      });

      it('returns 0 for dates within the same calendar year', () => {
        const a = adapter.date('2025-12-31T23:59:59Z', 'default');
        const b = adapter.date('2025-01-01T00:00:00Z', 'default');
