...
describe('<StandaloneCompactDayViewPremium /> - License', () => {
  ...
  it('should render without watermark when a valid premium license is set', () => {
    ...
    render(
      <StandaloneCompactDayViewPremium events={[]} visibleDate={DEFAULT_TESTING_VISIBLE_DATE} />,
    );

    expect(screen.queryByText('MUI X Missing license key')).to.equal(null);
  });

  it('should throw out of scope error when using StandaloneCompactDayViewPremium with a pro license', () => {
