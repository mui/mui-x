...
import { getGroupRowIdFromPath, insertNodeInTree, removeNodeFromTree } from './utils';
    ...
    if (depth === path.length - 1) {
      ...
      if (existingNodeIdWithPartialPath == null) {
        ...
        if (
          ...
          // 1. Set the new parent for all children from the auto generated group
          existingNodeWithPartialPath.children.forEach((childId) => {
            tree[childId] = {
              ...tree[childId],
              parent: id,
            };
          ...
          // 2. Remove the auto generated group from its parent's children
          ...
          tree[parentNodeId] = {
            ...
            children: parentNode.children.filter(
              (childId) => childId !== existingNodeIdWithPartialPath,
            ),
          };

          // 3. Remove the replaced group from the tree
          removeNodeFromTree({
            id: existingNodeIdWithPartialPath,
            tree,
            treeDepths,
          });

          // 4. Add the group creates from the data row in the tree
