...
import { getNonAutoGeneratedDescendants } from '../../../utils/tree/getNonAutoGeneratedDescendants';
  ...
  const getGroupingCriteriaRows = React.useCallback<GridRowGroupingApi['getGroupingCriteriaRows']>(
    (params) => {
      ...
      if (params.applySorting) {
        const groupNode = tree[params.groupId];
        if (!groupNode) {
          return [];
        }
        ...
        let index = sortedRowIds.findIndex((id) => id === params.groupId) + 1;
        while (index < sortedRowIds.length && tree[sortedRowIds[index]].depth > groupNode.depth) {
        ...
        children = getNonAutoGeneratedDescendants(tree, params.groupId);
      }

      if (params.applyFiltering) {
