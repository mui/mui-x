...
type AggregationValueGetter = GridColDef['valueGetter'] & {
  originalMethod?: GridColDef['valueGetter'];
...
export const wrapColumnWithAggregation = ({
  ...
  const originalValueGetter = colDef.valueGetter;
  ...
  const wrappedValueGetter: AggregationValueGetter = (params) => {
...
export const unwrapColumnFromAggregation = ({ colDef }: { colDef: GridColDef }) => {
  const valueGetter = colDef.valueGetter as AggregationValueGetter;
  if (valueGetter?.isWrappedWithAggregation) {
    return {
      ...colDef,
      valueGetter: valueGetter.originalMethod,
    };
  }
