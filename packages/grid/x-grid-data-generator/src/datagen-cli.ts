...
export function datagenCli(args) {
  program
    ...
    .option('-o, --output <output>', 'The output file, if not passed, it will generate "./<dataset>-<size>.json"')
    .option('-p, --pretty', 'print a prettier output', false)
    .action(function() {
      console.log(`Generating new ${program.dataset} dataset with ${Number(program.size).toLocaleString()} rows`);
