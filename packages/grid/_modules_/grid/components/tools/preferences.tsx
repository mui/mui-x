...
export const PreferencesPanel = ()=> {
	...
	const changeTab = React.useCallback((event: React.ChangeEvent<{}>, newValue: PreferencePanelsValue) => {
		setGridState(state => ({
			...state,
	...
	const updateColumnTarget = React.useCallback(
		({open, targetField}: PreferencePanelState) => {
			if (targetField && open) {
				const headerCellEl = findHeaderElementFromField(apiRef!.current!.rootElementRef!.current!, targetField!);
	...
	const isColumnsTabOpen = gridState.preferencePanel.openedPanelValue === PreferencePanelsValue.columns;
	const isFiltersTabOpen = !isColumnsTabOpen;
	return (
		<Popper placement="bottom" open={gridState.preferencePanel.open}
			...
			<ClickAwayListener onClickAway={hidePreferencesDelayed}>
				<Paper square className={classes.root}>
					<Tabs
						...
						onChange={changeTab}
					>
						<Tab value={PreferencePanelsValue.filters} icon={filterIconElement} fullWidth className="tab"
						     label={'Filters'}/>
						<Tab value={PreferencePanelsValue.columns} icon={<ViewWeekIcon/>} fullWidth className="tab"
						     label={'Columns'}/>
					</Tabs>
					{isColumnsTabOpen && (

						<TabPanel value={PreferencePanelsValue.columns}>
							Column Picker here
						</TabPanel>
					)}
					{isFiltersTabOpen && (
