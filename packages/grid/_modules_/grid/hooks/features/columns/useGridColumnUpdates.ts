...
import { GridColumnUpdateApi } from '../../../models/api/gridColumnApi';
...
import { allGridColumnsSelector } from './gridColumnsSelector';
...
export const useGridColumnUpdates = (
  apiRef: GridApiRef,
  ...
  const updateColumns = React.useCallback<GridColumnUpdateApi['updateColumns']>(
    (cols) => {
      // Avoid dependency on gridState to avoid infinite loop
  ...
  const updateColumn = React.useCallback<GridColumnUpdateApi['updateColumn']>(
    (col) => updateColumns([col]),
    [updateColumns],
  );

  const setColumnIndex = React.useCallback<GridColumnUpdateApi['setColumnIndex']>(
  ...
  const setColumnWidth = React.useCallback<GridColumnUpdateApi['setColumnWidth']>(
    (field, width) => {
  ...
  const colApi: GridColumnUpdateApi = {
    updateColumn,
    updateColumns,
    setColumnIndex,
    setColumnWidth,
