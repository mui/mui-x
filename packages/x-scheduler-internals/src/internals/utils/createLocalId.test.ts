...
describe('createLocalId', () => {
  ...
  it('should return unique ids across 100 calls', () => {
    const ids = new Set(Array.from({ length: 100 }, () => createLocalId()));

    expect(ids.size).toBe(100);
  });

  it('should return local-{timestamp}-{counter} fallback when crypto.randomUUID is unavailable', () => {
  ...
  it('should increment the counter in fallback mode', () => {
    vi.stubGlobal('crypto', { randomUUID: undefined });
