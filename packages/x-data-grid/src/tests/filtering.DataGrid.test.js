...
describe('<DataGrid /> - Filter', () => {
    ...
    function TestCase(props) {
        return (_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, ...props, disableEval: disableEval }) }));
    }
    describe('prop: filterModel', () => {
        it('should throw for more than one filter item', () => {
            expect(() => {
                render(_jsx(TestCase, { rows: [], columns: [], filterModel: {
                        items: [
        ...
        it('should apply the model for `filterable: false` columns but the applied filter should be readonly', () => {
            render(_jsx(TestCase, { columns: [{ field: 'brand', filterable: false }], filterModel: { items: [{ field: 'brand', operator: 'contains', value: 'Adidas' }] }, initialState: {
                    preferencePanel: {
                        open: true,
        ...
        it('should apply the model', () => {
            render(_jsx(TestCase, { filterModel: { items: [{ field: 'brand', operator: 'contains', value: 'a' }] } }));
            expect(getColumnValues(0)).to.deep.equal(['Adidas', 'Puma']);
        });
        it('should apply the model when row prop changes', () => {
            render(_jsx(TestCase, { filterModel: { items: [{ field: 'brand', operator: 'contains', value: 'a' }] }, rows: [
                    {
                        id: 3,
                        brand: 'Asics',
                    },
        ...
        it('should support new dataset', () => {
            const { setProps } = render(_jsx(TestCase, { filterModel: { items: [{ field: 'brand', operator: 'contains', value: 'a' }] } }));
            expect(getColumnValues(0)).to.deep.equal(['Adidas', 'Puma']);
            setProps({
                rows: [
                    {
    ...
    describe('prop: initialState.filter', () => {
        ...
        it('should allow to initialize the filterModel for non-filterable columns', () => {
            render(_jsx(TestCase, { columns: [{ field: 'brand', filterable: false }], initialState: {
                    filter: {
                        filterModel: {
                            items: [{ field: 'brand', operator: 'equals', value: 'Adidas' }],
                        },
        ...
        it('should use the control state upon the initialize state when both are defined', () => {
            render(_jsx(TestCase, { filterModel: {
                    items: [{ field: 'brand', operator: 'equals', value: 'Adidas' }],
                }, initialState: {
                    filter: {
                        filterModel: {
        ...
        it('should not update the filters when updating the initial state', () => {
            const { setProps } = render(_jsx(TestCase, { initialState: {
                    filter: {
                        filterModel: {
                            items: [{ field: 'brand', operator: 'equals', value: 'Adidas' }],
                        },
    ...
    describe('prop: getRowId', () => {
        it('works with filter', () => {
            render(_jsx(TestCase, { getRowId: (row) => row.brand, filterModel: {
    ...
    describe('column type: string', () => {
        ...
        describe('ignoreDiacritics', () => {
            function DiacriticsTestCase({ filterValue, ...props }) {
                return (_jsx(TestCase, { filterModel: {
            ...
            it('should not ignore diacritics by default', () => {
                testEval(() => {
                    const { unmount } = render(_jsx(DiacriticsTestCase, { filterValue: "apa" }));
                    expect(getColumnValues(0)).to.deep.equal([]);
                    unmount();
                });
                testEval(() => {
                    const { unmount } = render(_jsx(DiacriticsTestCase, { filterValue: "ap\u0103" }));
            ...
            it('should ignore diacritics when `ignoreDiacritics` is enabled', () => {
                testEval(() => {
                    const { unmount } = render(_jsx(DiacriticsTestCase, { filterValue: "apa", ignoreDiacritics: true }));
                    expect(getColumnValues(0)).to.deep.equal(['Apă']);
                    unmount();
                });
                testEval(() => {
                    const { unmount } = render(_jsx(DiacriticsTestCase, { filterValue: "ap\u0103", ignoreDiacritics: true }));
                    expect(getColumnValues(0)).to.deep.equal(['Apă']);
                    unmount();
                });
            });
    ...
    describe('column type: dateTime', () => {
        ...
        describe('should consider seconds when filtering', () => {
            const getRowsWithSeconds = (operator) => {
                const { unmount } = render(_jsx(TestCase, { filterModel: {
                    ...
                    ], columns: [
                        {
                            field: 'date',
                            type: 'dateTime',
                            valueFormatter: (value) => (value ? value.toLocaleString('en-US') : ''),
                        },
                    ] }));
                const values = getColumnValues(0);
                unmount();
    ...
    describe('column type: singleSelect', () => {
        ...
        it('should support `valueParser`', () => {
            ...
            const { setProps } = render(_jsx(TestCase, { filterModel: {
        ...
        it('should work if valueOptions is not provided', () => {
            const { setProps } = render(_jsx(TestCase, { rows: [
                    ...
                    { id: 2, name: 'Dishwasher', voltage: 110 },
                    { id: 3, name: 'Microwave', voltage: 220 },
                ], columns: [{ field: 'name' }, { field: 'voltage', type: 'singleSelect' }], filterModel: { items: [{ field: 'voltage', operator: 'is' }] } }));
            expect(getColumnValues(0)).to.deep.equal(['Hair Dryer', 'Dishwasher', 'Microwave']);
            setProps({
                filterModel: { items: [{ field: 'voltage', operator: 'is', value: 220 }] },
            });
    ...
    describe('toolbar active filter count', () => {
        it('should not include operators with value when the value is empty', () => {
            const getFilterCount = (item) => {
                const { unmount } = render(_jsx(TestCase, { rows: [], columns: [
        ...
        it('should include value-less operators', () => {
            render(_jsx(TestCase, { rows: [], columns: [{ field: 'brand', type: 'string' }], filterModel: {
                    items: [
                        {
                            field: 'brand',
                            operator: 'isNotEmpty',
    ...
    describe('filter button tooltip', () => {
        it('should display `falsy` value', async () => {
            const { setProps, user } = render(_jsx(DataGrid, { filterModel: {
                ...
                ], slots: { toolbar: GridToolbarFilterButton }, showToolbar: true }));
    ...
    describe('custom `filterOperators`', () => {
        it('should allow to customize filter tooltip using `filterOperator.getValueAsString`', async () => {
            const { user } = render(_jsx("div", { style: { width: '100%', height: '400px' }, children: _jsx(DataGrid, { filterModel: {
    ...
    it('should translate operators dynamically in toolbar without crashing', () => {
        expect(() => {
            return (_jsx("div", { style: { height: 400, width: '100%' }, children: _jsx(DataGrid, { rows: [
                        {
                            id: 1,
                            quantity: 1,
                        },
    ...
    it('should update the filter model on columns change', () => {
        ...
        function Demo(props) {
            return (_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { autoHeight: isJSDOM, columns: columns, filterModel: {
                        items: [{ field: 'brand', operator: 'equals', value: 'Puma' }],
