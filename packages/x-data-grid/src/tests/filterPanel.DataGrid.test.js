...
import { DataGrid, GridFilterInputValue, GridPreferencePanelsValue, getGridStringOperators, getGridSingleSelectOperators, } from '@mui/x-data-grid';
...
function CustomInputValue(props) {
    ...
    return (_jsx("input", { name: "custom-filter-operator", placeholder: "Filter value", value: item.value, onChange: handleFilterChange, "data-testid": "customInput" }));
}
...
describe('<DataGrid /> - Filter panel', () => {
    ...
    it('should show an empty string as the default filter input value', () => {
        render(_jsx(TestCase, { initialState: {
                preferencePanel: {
                    open: true,
    ...
    ['string', 'singleSelect'].forEach((type) => {
        it(`should forward InputComponentProps for ${type} isAnyOf operator`, () => {
            ...
            render(_jsx(TestCase, { columns: [
                    {
                        field: 'brand',
                        type,
                        ...(type === 'singleSelect' && { valueOptions: ['Nike', 'Adidas', 'Puma'] }),
                        filterOperators: operators.map((op) => op.value === 'isAnyOf'
