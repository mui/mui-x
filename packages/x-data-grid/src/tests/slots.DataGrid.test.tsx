...
describe('<DataGrid /> - Slots', () => {
  ...
  describe('slotProps', () => {
    ...
    it('should pass the props from slotProps.columnHeaderFilterIconButton to the column header filter icon', async () => {
      ...
      expect(onClick.mock.calls[onClick.mock.calls.length - 1][0]).to.have.property('field', 'brand');
      expect(onClick.mock.calls[onClick.mock.calls.length - 1][1]).to.have.property('target', button);
    });
  });
