...
import { createRenderer, fireEvent, reactMajor, screen, waitFor, act, } from '@mui/internal-test-utils';
...
describe('<DataGrid /> - Pagination', () => {
    ...
    function BaselineTestCase(props) {
        ...
        return (_jsx("div", { style: { width: 300, height }, children: _jsx(DataGrid, { ...basicData, apiRef: apiRef, autoHeight: isJSDOM, ...other }) }));
    }
    // Need layouting
    describe.skipIf(isJSDOM)('prop: paginationModel and onPaginationModelChange', () => {
        it('should display the rows of page given in props', () => {
            render(_jsx(BaselineTestCase, { paginationModel: { page: 1, pageSize: 1 }, pageSizeOptions: [1] }));
            expect(getColumnValues(0)).to.deep.equal(['1']);
        });
        it('should not call onPaginationModelChange on initialisation', () => {
            const onPaginationModelChange = spy();
            render(_jsx(BaselineTestCase, { paginationModel: { page: 0, pageSize: 1 }, onPaginationModelChange: onPaginationModelChange, pageSizeOptions: [1] }));
            expect(onPaginationModelChange.callCount).to.equal(0);
        });
        it('should allow to update the paginationModel from the outside', () => {
            const { setProps } = render(_jsx(BaselineTestCase, { paginationModel: { page: 0, pageSize: 1 }, pageSizeOptions: [1] }));
        ...
        it('should not apply new page when clicking on next / previous button and onPaginationModelChange is not defined and paginationModel is controlled', () => {
            render(_jsx(BaselineTestCase, { paginationModel: { pageSize: 1, page: 0 }, pageSizeOptions: [1] }));
        ...
        it('should call onPaginationModelChange and apply new page when clicking on next / previous button', () => {
            const onPaginationModelChange = spy();
            render(_jsx(BaselineTestCase, { onPaginationModelChange: onPaginationModelChange, initialState: { pagination: { paginationModel: { pageSize: 1 } } }, pageSizeOptions: [1] }));
            fireEvent.click(screen.getByRole('button', { name: /next page/i }));
        ...
        it('should apply the new pageSize when clicking on a page size option and onPaginationModelChange is not defined and paginationModel is not controlled', () => {
            render(_jsx(BaselineTestCase, { pageSizeOptions: [1, 2, 3, 100] }));
        ...
        it('should call onPaginationModelChange and apply the new pageSize when clicking on a page size option and paginationModel is not controlled', () => {
            const onPaginationModelChange = spy();
            render(_jsx(BaselineTestCase, { onPaginationModelChange: onPaginationModelChange, pageSizeOptions: [1, 2, 3, 100] }));
            fireEvent.mouseDown(screen.getByLabelText('Rows per page:'));
            expect(screen.queryAllByRole('option').length).to.equal(4);
            fireEvent.click(screen.queryAllByRole('option')[1]);
            expect(onPaginationModelChange.callCount).to.equal(1);
        ...
        it('should call onPaginationModelChange with the correct paginationModel when clicking on next / previous button when paginationModel is controlled', () => {
            const onPaginationModelChange = spy();
            function TestCase({ handlePaginationModelChange, }) {
                const [paginationModel, setPaginationModel] = React.useState({ pageSize: 1, page: 0 });
                return (_jsx(BaselineTestCase, { onPaginationModelChange: (newModel) => {
                        handlePaginationModelChange(newModel);
                        setPaginationModel(newModel);
                    }, paginationModel: paginationModel, pageSizeOptions: [1] }));
            }
            render(_jsx(TestCase, { handlePaginationModelChange: onPaginationModelChange }));
            fireEvent.click(screen.getByRole('button', { name: /next page/i }));
        ...
        it('should call onPaginationModelChange when clicking on next / previous button in "server" mode', () => {
            const onPaginationModelChange = spy();
            render(_jsx(BaselineTestCase, { onPaginationModelChange: onPaginationModelChange, initialState: { pagination: { paginationModel: { pageSize: 1 } } }, pageSizeOptions: [1], paginationMode: "server", rowCount: 4 }));
            fireEvent.click(screen.getByRole('button', { name: /next page/i }));
            expect(onPaginationModelChange.callCount).to.equal(1);
            expect(onPaginationModelChange.lastCall.args[0]).to.deep.equal({ page: 1, pageSize: 1 });
            fireEvent.click(screen.getByRole('button', { name: /previous page/i }));
        ...
        it('should control `paginationModel` state when the prop and the onChange are set', () => {
            function ControlCase() {
                ...
                return (_jsx(BaselineTestCase, { paginationModel: paginationModel, onPaginationModelChange: (newPaginationModel) => setPaginationModel(newPaginationModel), pageSizeOptions: [1] }));
            }
        ...
        it('should go to last page when paginationModel is controlled and the current page is greater than the last page', async () => {
            ...
            function TestCasePaginationFilteredData(props) {
                ...
                return (_jsx(BaselineTestCase, { paginationModel: paginationModel, onPaginationModelChange: handlePaginationModelChange, pageSizeOptions: [5], ...props }));
            }
            const { setProps } = render(_jsx(TestCasePaginationFilteredData, {}));
            expect(onPaginationModelChange.callCount).to.equal(0);
            setProps({
                filterModel: {
        ...
        it('should scroll to the top of the page when changing page', () => {
            const { setProps } = render(_jsx(BaselineTestCase, { paginationModel: { page: 0, pageSize: 5 }, pageSizeOptions: [5] }));
        ...
        it('should display the amount of rows given in props', () => {
            render(_jsx(BaselineTestCase, { paginationModel: { page: 0, pageSize: 2 }, pageSizeOptions: [2] }));
            expect(getColumnValues(0)).to.deep.equal(['0', '1']);
        });
        it('should throw if pageSize exceeds 100', () => {
            render(_jsx(BaselineTestCase, { paginationModel: { pageSize: 1, page: 0 }, pageSizeOptions: [1, 2, 101] }));
            expect(() => apiRef.current?.setPageSize(101)).to.throw(/`pageSize` cannot exceed 100 in the MIT version of the DataGrid./);
        });
        it('should call onPaginationModelChange with the correct page when clicking on a page size option when paginationModel is controlled', () => {
            const onPaginationModelChange = spy();
            render(_jsx(BaselineTestCase, { onPaginationModelChange: onPaginationModelChange, paginationModel: { pageSize: 1, page: 0 }, pageSizeOptions: [1, 2, 3] }));
            fireEvent.mouseDown(screen.getByLabelText('Rows per page:'));
        ...
        it('should not change the pageSize state when clicking on a page size option when paginationModel prop is provided', () => {
            render(_jsx(BaselineTestCase, { paginationModel: { pageSize: 1, page: 0 }, pageSizeOptions: [1, 2, 3] }));
            fireEvent.mouseDown(screen.getByLabelText('Rows per page:'));
            expect(screen.queryAllByRole('option').length).to.equal(3);
            fireEvent.click(screen.queryAllByRole('option')[1]);
            expect(getColumnValues(0)).to.deep.equal(['0']);
        ...
        it('should display a warning if the prop pageSize is not in the prop pageSizeOptions', () => {
            const pageSize = 12;
            expect(() => {
                render(_jsx(BaselineTestCase, { paginationModel: { pageSize, page: 0 }, pageSizeOptions: [25, 50, 100] }));
            }).toWarnDev([
        ...
        it('should not display a warning if the prop pageSize is in the prop pageSizeOptions when it is an array of objects.', () => {
            const pageSize = 10;
            expect(() => {
                render(_jsx(BaselineTestCase, { paginationModel: { pageSize, page: 0 }, pageSizeOptions: [
                        { label: '10', value: 10 },
                        { label: '20', value: 20 },
                        { label: '30', value: 30 },
                    ] }));
        ...
        it('should display a warning if the prop pageSize is not in the default pageSizeOptions', () => {
            const pageSize = 12;
            expect(() => {
                render(_jsx(BaselineTestCase, { paginationModel: { pageSize, page: 0 } }));
            }).toWarnDev([
                `MUI X: The page size \`${pageSize}\` is not present in the \`pageSizeOptions\``,
                reactMajor < 19 &&
                    `MUI X: The page size \`${pageSize}\` is not present in the \`pageSizeOptions\``,
        ...
        it('should display a warning if the default pageSize given as props is not in the prop pageSizeOptions', () => {
            expect(() => {
                render(_jsx(BaselineTestCase, { pageSizeOptions: [25, 50] }));
            }).toWarnDev([
        ...
        it('should update the pageCount state when updating the paginationModel prop with a lower pageSize value', () => {
            function TestCase(props) {
                ...
                return (_jsx(BaselineTestCase, { pageSizeOptions: [10, 20], paginationModel: paginationModel, onPaginationModelChange: setPaginationModel, disableVirtualization: true, ...props }));
            }
    ...
    describe.skipIf(isJSDOM)('prop: autoPageSize', () => {
        function TestCaseAutoPageSize(props) {
            ...
            return (_jsx("div", { style: { width: 300, height: props.height }, children: _jsx(DataGrid, { columns: data.columns, rows: data.rows, autoPageSize: true, ...other }) }));
        }
        it('should give priority to the controlled paginationModel', () => {
            render(_jsx(BaselineTestCase, { autoPageSize: true, paginationModel: { pageSize: 3, page: 2 }, pageSizeOptions: [3] }));
        ...
        it('should always render the same amount of rows and fit the viewport', () => {
            ...
            const rowHeight = 52;
            render(_jsx(TestCaseAutoPageSize, { nbRows: nbRows, height: height, columnHeaderHeight: columnHeaderHeight, rowHeight: rowHeight }));
            const footerHeight = document.querySelector('.MuiDataGrid-footerContainer').clientHeight;
            const expectedFullPageRowsLength = Math.floor((height - columnHeaderHeight - footerHeight) / rowHeight);
            let rows = getRows();
        ...
        it('should update the amount of rows rendered and call onPageSizeChange when changing the table height', async () => {
            ...
            const columnHeaderHeight = 56;
            const rowHeight = 52;
            const { setProps } = render(_jsx(TestCaseAutoPageSize, { nbRows: nbRows, height: heightBefore, columnHeaderHeight: columnHeaderHeight, rowHeight: rowHeight, onPaginationModelChange: onPaginationModelChange }));
            const footerHeight = document.querySelector('.MuiDataGrid-footerContainer').clientHeight;
            const expectedViewportRowsLengthBefore = Math.floor((heightBefore - columnHeaderHeight - footerHeight) / rowHeight);
            const expectedViewportRowsLengthAfter = Math.floor((heightAfter - columnHeaderHeight - footerHeight) / rowHeight);
            ...
            await waitFor(() => {
                expect(document.querySelector('.MuiTablePagination-displayedRows').innerHTML).to.equal(`1–${expectedViewportRowsLengthAfter} of ${nbRows}`);
            });
    ...
    it('should react to an update of rowCount when `paginationMode = server`', () => {
        const { setProps } = render(_jsx(BaselineTestCase, { rowCount: 5, paginationModel: { page: 0, pageSize: 1 }, pageSizeOptions: [1], paginationMode: "server" }));
    ...
    describe('server-side pagination', () => {
        function ServerPaginationGrid(props) {
            ...
            return (_jsx("div", { style: { height: 300, width: 300 }, children: _jsx(DataGrid, { columns: [{ field: 'id' }], rows: rows, paginationMeta: { hasNextPage: props.rowCount === -1 }, paginationModel: paginationModel, pageSizeOptions: [1], paginationMode: "server", onPaginationModelChange: handlePaginationModelChange, ...props }) }));
        }
        it('should support server side pagination with known row count', () => {
            render(_jsx(ServerPaginationGrid, { rowCount: 3 }));
            expect(getColumnValues(0)).to.deep.equal(['0']);
            fireEvent.click(screen.getByRole('button', { name: /next page/i }));
            expect(getColumnValues(0)).to.deep.equal(['1']);
        });
        it('should support server side pagination with unknown row count', () => {
            const { setProps } = render(_jsx(ServerPaginationGrid, { rowCount: -1 }));
            expect(getColumnValues(0)).to.deep.equal(['0']);
        ...
        it('should support server side pagination with estimated row count', () => {
            const { setProps } = render(_jsx(ServerPaginationGrid, { rowCount: -1, estimatedRowCount: 2 }));
            expect(getColumnValues(0)).to.deep.equal(['0']);
    ...
    it.skipIf(isJSDOM)('should automatically calculate row count when hasNextPage becomes false', async () => {
        function TestComponent(props) {
            ...
            return (_jsx("div", { style: { height: 300, width: 300 }, children: _jsx(DataGrid, { apiRef: apiRef, columns: [{ field: 'id' }], rows: rows, paginationMeta: { hasNextPage: props.hasNextPage }, paginationModel: paginationModel, pageSizeOptions: [1], paginationMode: "server", onPaginationModelChange: setPaginationModel }) }));
        }
        const { setProps } = render(_jsx(TestComponent, { hasNextPage: true }));
        // start at page 0
        expect(getColumnValues(0)).to.deep.equal(['0']);
    ...
    it('should reset page to 0 and scroll to top if sort or filter is applied', async () => {
        render(_jsx(BaselineTestCase, { initialState: { pagination: { paginationModel: { page: 0, pageSize: 50 }, rowCount: 0 } }, pageSizeOptions: [50] }));
    ...
    it('should make the first cell focusable after changing the page', () => {
        render(_jsx(BaselineTestCase, { initialState: { pagination: { paginationModel: { pageSize: 1 } } }, pageSizeOptions: [1] }));
    ...
    describe.skipIf(isJSDOM)('prop: initialState.pagination', () => {
        it('should allow to initialize the paginationModel', () => {
            render(_jsx(BaselineTestCase, { initialState: {
                    pagination: {
        ...
        it('should use the paginationModel control state upon the initialize state when both are defined', () => {
            render(_jsx(BaselineTestCase, { paginationModel: { pageSize: 5, page: 0 }, initialState: {
                    pagination: {
                        paginationModel: { pageSize: 2, page: 0 },
                    },
                }, pageSizeOptions: [2, 5] }));
        ...
        it('should not update the paginationModel when updating the initial state', () => {
            const { setProps } = render(_jsx(BaselineTestCase, { initialState: {
                    pagination: {
                        paginationModel: { pageSize: 2, page: 0 },
                    },
                }, pageSizeOptions: [2, 5] }));
    ...
    it('should not throw on deleting the last row of a page > 0', () => {
        ...
        expect(() => {
            const { setProps } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { columns: columns, rows: rows, initialState: { pagination: { paginationModel: { pageSize: 2, page: 1 } } }, pageSizeOptions: [2] }) }));
            setProps({ rows: rows.slice(0, 2) });
        }).not.to.throw();
    });
    it('should log an error if rowCount is used with client-side pagination', () => {
        expect(() => {
            render(_jsx(BaselineTestCase, { paginationMode: "client", rowCount: 100 }));
        }).toErrorDev([
