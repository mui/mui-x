...
describe.skipIf(isJSDOM)('<DataGrid /> - Export', () => {
    ...
    function TestCase(props) {
        ...
        return (_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...basicData, ...props }) }));
    }
    let spyCreateObjectURL;
    beforeEach(() => {
    ...
    describe('component: GridToolbar', () => {
        it('should export with the default csvOptions', async () => {
            render(_jsx(TestCase, { showToolbar: true }));
        ...
        it('should apply custom csvOptions', async () => {
            render(_jsx(TestCase, { showToolbar: true, slotProps: { toolbar: { csvOptions: { delimiter: ';' } } } }));
            fireEvent.click(screen.getByRole('button', { name: 'Export' }));
            expect(screen.queryByRole('menu')).not.to.equal(null);
            fireEvent.click(screen.getByRole('menuitem', { name: 'Download as CSV' }));
            expect(spyCreateObjectURL.callCount).to.equal(1);
        ...
        it('should disable csv export when passing `csvOptions.disableToolbarButton`', () => {
            render(_jsx(TestCase, { showToolbar: true, slotProps: { toolbar: { csvOptions: { disableToolbarButton: true } } } }));
            fireEvent.click(screen.getByRole('button', { name: 'Export' }));
            expect(screen.queryByRole('menu')).not.to.equal(null);
            expect(screen.queryByRole('menuitem', { name: 'Download as CSV' })).to.equal(null);
        });
        it('should escape formulas in the cells', async () => {
            render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { columns: [{ field: 'name' }], rows: [
                        { id: 0, name: '=1+1' },
                        { id: 1, name: '+1+1' },
                        { id: 2, name: '-1+1' },
                        { id: 3, name: '@1+1' },
    ...
    describe('component: GridToolbarExport', () => {
        it('should export with the default csvOptions', async () => {
            render(_jsx(TestCase, { slots: { toolbar: () => _jsx(GridToolbarExport, {}) }, showToolbar: true }));
            fireEvent.click(screen.getByRole('button', { name: 'Export' }));
            expect(screen.queryByRole('menu')).not.to.equal(null);
            fireEvent.click(screen.getByRole('menuitem', { name: 'Download as CSV' }));
            expect(spyCreateObjectURL.callCount).to.equal(1);
        ...
        it('should apply custom csvOptions', async () => {
            render(_jsx(TestCase, { slots: { toolbar: () => _jsx(GridToolbarExport, { csvOptions: { delimiter: ';' } }) }, showToolbar: true }));
            fireEvent.click(screen.getByRole('button', { name: 'Export' }));
            expect(screen.queryByRole('menu')).not.to.equal(null);
            fireEvent.click(screen.getByRole('menuitem', { name: 'Download as CSV' }));
            expect(spyCreateObjectURL.callCount).to.equal(1);
        ...
        it('should disable csv export when passing `csvOptions.disableToolbarButton`', async () => {
            render(_jsx(TestCase, { slots: {
                    toolbar: () => _jsx(GridToolbarExport, { csvOptions: { disableToolbarButton: true } }),
                }, showToolbar: true }));
            fireEvent.click(screen.getByRole('button', { name: 'Export' }));
            expect(screen.queryByRole('menu')).not.to.equal(null);
            expect(screen.queryByRole('menuitem', { name: 'Download as CSV' })).to.equal(null);
