...
describe('<DataGrid /> - Accessibility', () => {
    ...
    it('should use the `label` prop as the `aria-label` attribute of role="grid"', () => {
        render(_jsx(DataGrid, { ...baselineProps, label: "Grid label" }));
    ...
    it('should only use the `label` prop as the `aria-label` attribute of role="grid" if `aria-label` is not provided', () => {
        render(_jsx(DataGrid, { ...baselineProps, label: "Grid label", "aria-label": "Grid aria-label" }));
    ...
    it('should only use the `label` prop as the `aria-label` attribute of role="grid" if `aria-labelledby` is not provided', () => {
        render(_jsx(DataGrid, { ...baselineProps, label: "Grid label", "aria-labelledby": "Grid aria-labelledby" }));
    ...
    describe('column type: longText', () => {
        const longTextProps = {
            ...
            columns: [{ field: 'bio', type: 'longText', headerName: 'Biography' }],
        };
        ...
        it('edit popup should have role="dialog" with aria-label', async () => {
            const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { rows: [{ id: 0, bio: 'Long text content' }], columns: [{ field: 'bio', type: 'longText', headerName: 'Biography', editable: true }] }) }));
            const cell = getCell(0, 0);
