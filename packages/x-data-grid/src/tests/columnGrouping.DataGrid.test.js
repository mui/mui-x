...
const getDefaultProps = (nbColumns) => {
    const columns = [];
    const row = {};
...
describe('<DataGrid /> - Column grouping', () => {
    ...
    function TestDataGrid(props) {
        ...
        return (_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...getDefaultProps(nbColumns), ...other }) }));
    }
    describe('Header grouping columns', () => {
        it('should add one header row when columns have a group', () => {
            render(_jsx(TestDataGrid, { nbColumns: 2, columnGroupingModel: [{ groupId: 'A', children: [{ field: 'col1' }, { field: 'col2' }] }] }));
            expect(screen.queryAllByRole('row')).to.have.length(3);
        });
        it('should add header rows to match max depth of column groups', () => {
            render(_jsx(TestDataGrid, { nbColumns: 3, columnGroupingModel: [
                    {
                        groupId: 'col123',
                        children: [
        ...
        it('should add correct aria-colspan, aria-colindex on headers', () => {
            ...
            const row1Headers = document.querySelectorAll('[aria-rowindex="1"] [role="columnheader"]');
            const row2Headers = document.querySelectorAll('[aria-rowindex="2"] [role="columnheader"]');
            expect(Array.from(row1Headers).map((header) => header.getAttribute('aria-colspan'))).to.deep.equal(['3']);
            expect(Array.from(row1Headers).map((header) => header.getAttribute('aria-colindex'))).to.deep.equal(['1']);
            expect(Array.from(row2Headers).map((header) => header.getAttribute('aria-colspan'))).to.deep.equal(['2', '1']);
            expect(Array.from(row2Headers).map((header) => header.getAttribute('aria-colindex'))).to.deep.equal(['1', '3']);
        });
        it('should support non connexe groups', () => {
            render(_jsx(TestDataGrid, { nbColumns: 4, columnGroupingModel: [
                    {
                        groupId: 'col134',
                        children: [
            ...
            const row1Headers = document.querySelectorAll('[aria-rowindex="1"] [role="columnheader"]');
            const row2Headers = document.querySelectorAll('[aria-rowindex="2"] [role="columnheader"]');
            expect(Array.from(row1Headers).map((header) => header.getAttribute('aria-colspan'))).to.deep.equal(['1', '1', '2']);
            expect(Array.from(row1Headers).map((header) => header.getAttribute('aria-colindex'))).to.deep.equal(['1', '2', '3']);
            expect(Array.from(row2Headers).map((header) => header.getAttribute('aria-colspan'))).to.deep.equal(['1', '1', '1', '1']);
            expect(Array.from(row2Headers).map((header) => header.getAttribute('aria-colindex'))).to.deep.equal(['1', '2', '3', '4']);
        });
        it('should only consider visible columns non connexe groups', () => {
            const { setProps } = render(_jsx(TestDataGrid, { nbColumns: 3, columnGroupingModel: [
                    {
                        groupId: 'col12',
        ...
        it('should split empty group cell if they are children of different group', () => {
            ...
            const row2Headers = document.querySelectorAll('[aria-rowindex="2"] [role="columnheader"]');
            expect(Array.from(row2Headers).map((header) => header.getAttribute('aria-colspan'))).to.deep.equal(['1', '1', '1']);
            expect(Array.from(row2Headers).map((header) => header.getAttribute('aria-colindex'))).to.deep.equal(['1', '2', '3']);
        });
        ...
        it('should not throw when both `columns` and `columnGroupingModel` are updated', () => {
            ...
            const { setProps } = render(_jsx(TestDataGrid, { nbColumns: 0, ...defaultProps, columnGroupingModel: [
                    {
                        groupId: 'testGroup',
                        children: [{ field: 'col1' }, { field: 'col2' }],
                    },
        ...
        it('should not throw when both `columns` and `columnGroupingModel` are updated twice', () => {
            function Demo() {
                ...
                return (_jsxs("div", { style: { width: 300, height: 300 }, children: [_jsx("button", { onClick: handleClick, children: "Update columns" }), _jsx(DataGrid, { rows: [{ id: 1, field_0: 'Value 0', field_1: 'Value 1' }], ...props })] }));
            }
            render(_jsx(Demo, {}));
            fireEvent.click(screen.getByRole('button', { name: /Update columns/ }));
            const row1Headers = document.querySelectorAll('[aria-rowindex="1"] [role="columnheader"] .MuiDataGrid-columnHeaderTitle');
            const row2Headers = document.querySelectorAll('[aria-rowindex="2"] [role="columnheader"] .MuiDataGrid-columnHeaderTitle');
    ...
    describe.todo('error messages', () => {
        function TestWithError(props) {
            return (_jsx(ErrorBoundary, { children: _jsx(TestDataGrid, { ...props }) }));
        }
        it('should log an error if two groups have the same id', () => {
            expect(() => {
                render(_jsx(TestWithError, { nbColumns: 2, columnGroupingModel: [
                        {
                            groupId: 'col12',
        ...
        it('should log an error if a columns is referenced in two groups', () => {
            expect(() => {
                render(_jsx(TestWithError, { nbColumns: 3, columnGroupingModel: [
                        {
                            groupId: 'col12',
                            children: [{ field: 'col1' }, { field: 'col2' }],
                        },
