...
describe('<DataGrid /> - Column headers', () => {
    ...
    describe('headerClassName', () => {
        it('should append the CSS class defined in headerClassName', () => {
            render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, columns: [{ field: 'brand', headerClassName: 'foobar' }] }) }));
    ...
    describe('Column menu', () => {
        it('should allow to hide column', async () => {
            const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: [{ field: 'id' }, { field: 'brand', headerClassName: 'foobar' }] }) }));
        ...
        it('should not render the column menu icon when the column menu has no items', () => {
            render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, disableColumnSelector: true, columns: [
            ...
            expect(within(getColumnHeaderCell(1)).queryByLabelText('brand column menu')).not.to.equal(null);
        });
        it('should render the column menu icon when there is a custom menu item', async () => {
            function CustomMenuItem() {
                return _jsx("li", { children: "Custom Item" });
            }
            const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, disableColumnSelector: true, columns: [
                        { field: 'id', filterable: false, sortable: false, hideable: false },
                        { field: 'brand' },
                    ], slotProps: {
                        columnMenu: {
        ...
        it('should not allow to hide the only visible column', async () => {
            const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: [{ field: 'id' }, { field: 'brand', headerClassName: 'foobar' }], initialState: {
                        columns: {
        ...
        it('should not allow to hide the only visible column that has menu', async () => {
            const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: [
        ...
        it('menu icon button should close column menu when already open', async () => {
            const { user } = render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, columns: [{ field: 'brand' }] }) }));
        ...
        it('should close the column menu when the grid is scrolled', async () => {
            const { user } = render(_jsx("div", { style: { height: 300, width: 300 }, children: _jsx(DataGrid, { rows: Array.from({ length: 50 }, (_, id) => ({ id, name: id })), columns: [{ field: 'brand' }] }) }));
            await user.click(within(getColumnHeaderCell(0)).getByLabelText('brand column menu'));
    ...
    it("should use 'headerName' as the aria-label for the menu icon button", async () => {
        render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, disableColumnSorting: true, columns: [{ headerName: 'brand header name', field: 'brand' }] }) }));
    ...
    it('should display sort column menu items as per sortingOrder prop', async () => {
        const { user } = render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, sortingOrder: ['desc', 'asc'], columns: [{ field: 'brand', headerClassName: 'foobar' }] }) }));
    ...
    it('should use baseTooltip slot for sort icon', () => {
        function CustomTooltip({ title, children, enterDelay, ...other }) {
            return (_jsx("div", { "data-testid": "custom-tooltip", "data-title": title, ...other, children: children }));
        }
        render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { columns: [{ field: 'id', sortable: true }], rows: [{ id: 1 }], initialState: {
                    sorting: {
