...
import { createRenderer, screen, act, waitFor, fireEvent, flushMicrotasks, } from '@mui/internal-test-utils';
import { DataGrid, GridEditModes, useGridApiRef, gridRowSelectionIdsSelector, } from '@mui/x-data-grid';
import { getCell, getRow, getRows, getColumnHeaderCell, getColumnHeadersTextContent, getActiveCell, getActiveColumnHeader, grid, includeRowSelection, } from 'test/utils/helperFn';
...
function getSelectedRowIds() {
    ...
    return Array.from(getRows())
        ...
        .map((row) => Number(row.querySelector(`[role="gridcell"][data-colindex="${hasCheckbox ? 1 : 0}"]`).textContent));
}
describe('<DataGrid /> - Row selection', () => {
    const { render } = createRenderer();
    const defaultData = getBasicGridData(4, 2);
    function TestDataGridSelection(props) {
        return (_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...defaultData, ...props, autoHeight: isJSDOM, experimentalFeatures: {
    ...
    it('should not call `onRowSelectionModelChange` twice when using filterMode="server"', async () => {
        ...
        function TestDataGrid() {
            const [, setRowSelectionModel] = React.useState(includeRowSelection([]));
            const handleRowSelectionModelChange = React.useCallback((model) => {
                setRowSelectionModel(model);
                onRowSelectionModelChange(model);
            }, []);
            return (_jsx(TestDataGridSelection, { getRowId: (row) => row.id, checkboxSelection: true, onRowSelectionModelChange: handleRowSelectionModelChange, filterMode: "server" }));
        }
        const { user } = render(_jsx(TestDataGrid, {}));
    ...
    describe('prop: checkboxSelection = false (single selection)', () => {
        it('should select one row at a time on click WITHOUT ctrl or meta pressed', async () => {
            const { user } = render(_jsx(TestDataGridSelection, {}));
    ...
    describe('prop: checkboxSelection = false (single selection), with keyboard events', () => {
        it('should select one row at a time on Shift + Space', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { disableRowSelectionOnClick: true }));
        ...
        [GridEditModes.Cell, GridEditModes.Row].forEach((editMode) => {
            it(`should select row on Shift + Space without starting editing the ${editMode}`, async () => {
                ...
                const { user } = render(_jsx(TestDataGridSelection, { columns: [
                        { field: 'id', type: 'number' },
                        { field: 'name', editable: true },
                    ], rows: [
                        { id: 0, name: 'React' },
                        { id: 1, name: 'Vue' },
                    ], onCellEditStart: onCellEditStart, editMode: editMode, disableRowSelectionOnClick: true }));
    ...
    describe('prop: checkboxSelection = true (multi selection)', () => {
        it('should allow to toggle prop.checkboxSelection', () => {
            const { setProps } = render(_jsx(TestDataGridSelection, {}));
        ...
        it('should check then uncheck when clicking twice the row', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true }));
            expect(getSelectedRowIds()).to.deep.equal([]);
        ...
        it('should return all row IDs when selection type is exclude with empty ids', async () => {
            ...
            function TestWithApiRef() {
                return (_jsx(TestDataGridSelection, { checkboxSelection: true, apiRef: apiRef, onRowSelectionModelChange: onRowSelectionModelChange }));
            }
            const { user } = render(_jsx(TestWithApiRef, {}));
        ...
        it('should handle exclude type selection when deselecting a single row after select all', async () => {
            // Context: https://github.com/mui/mui-x/issues/17878#issuecomment-3084263294
            const onRowSelectionModelChange = spy();
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, onRowSelectionModelChange: onRowSelectionModelChange }));
        ...
        it('should select all visible rows regardless of pagination', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, initialState: { pagination: { paginationModel: { pageSize: 1 } } }, pageSizeOptions: [1] }));
            const selectAllCheckbox = screen.getByRole('checkbox', { name: 'Select all rows' });
            await user.click(selectAllCheckbox);
            expect(getSelectedRowIds()).to.deep.equal([0]);
        ...
        it('should check the checkbox when there is no rows', () => {
            render(_jsx(TestDataGridSelection, { rows: [], checkboxSelection: true }));
        ...
        it('should disable the checkbox if isRowSelectable returns false', () => {
            render(_jsx(TestDataGridSelection, { isRowSelectable: (params) => params.id === 0, checkboxSelection: true }));
        ...
        it('disabled checkboxes cannot be focused', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { isRowSelectable: (params) => params.id !== 1, checkboxSelection: true }));
            expect(getSelectedRowIds()).to.deep.equal([]);
            const firstCheckbox = getCell(0, 0).querySelector('input');
        ...
        it('should create selection range only between selected rows', async () => {
            ...
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, disableVirtualization: true, ...extendedData }));
            await user.click(getCell(0, 1));
            await user.keyboard('{Shift>}');
            await user.click(getCell(2, 1));
            await user.keyboard('{/Shift}');
        ...
        it('should unselect from last clicked cell to cell after clicked cell if clicking inside a selected range', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, disableVirtualization: true }));
            await user.click(getCell(0, 0).querySelector('input'));
            expect(getSelectedRowIds()).to.deep.equal([0]);
            await user.keyboard('{Shift>}');
        ...
        it('should reset selected rows when turning off checkboxSelection', async () => {
            const { setProps, user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true }));
            await user.click(getCell(0, 0).querySelector('input'));
            await user.click(getCell(1, 0).querySelector('input'));
            expect(getSelectedRowIds()).to.deep.equal([0, 1]);
        ...
        it('should reset row selection in the current page as selected when turning off checkboxSelection', async () => {
            const { setProps, user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, pagination: true, initialState: { pagination: { paginationModel: { pageSize: 2 } } }, pageSizeOptions: [2] }));
            await user.click(getCell(0, 0).querySelector('input'));
            expect(getSelectedRowIds()).to.deep.equal([0]);
            await user.click(screen.getByRole('button', { name: /next page/i }));
            await user.click(getCell(2, 0).querySelector('input'));
        ...
        it('should set the correct aria-label on the cell checkbox', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, rows: [{ id: 0, name: 'React' }] }));
        ...
        it('should not select more than one row when disableMultipleRowSelection = true', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, disableMultipleRowSelection: true }));
        ...
        it('keyboard navigation when disableMultipleRowSelection is true', async () => {
            const { user } = render(_jsxs("div", { children: [_jsx("button", { children: "Before" }), _jsx(TestDataGridSelection, { checkboxSelection: true, disableMultipleRowSelection: true, hideFooter: true }), _jsx("button", { children: "After" })] }));
    ...
    describe('prop: checkboxSelection = true (multi selection), with keyboard events', () => {
        ...
        it('should add new row to the selection when pressing Shift+Space', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, disableRowSelectionOnClick: true }));
            expect(getSelectedRowIds()).to.deep.equal([]);
        ...
        it.skipIf(isJSDOM)('should not jump during scroll while the focus is on the checkbox', async () => {
            const data = getBasicGridData(20, 1);
            const { user } = render(_jsx(TestDataGridSelection, { ...data, rowHeight: 50, checkboxSelection: true, hideFooter: true }));
        ...
        it('should select/unselect all rows when pressing space', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, disableVirtualization: true }));
            const selectAllCell = document.querySelector('[role="columnheader"][data-field="__check__"] input');
    ...
    describe('prop: isRowSelectable', () => {
        it('should update the selected rows when the isRowSelectable prop changes', async () => {
            const { setProps, user } = render(_jsx(TestDataGridSelection, { isRowSelectable: () => true, checkboxSelection: true }));
            await user.click(getCell(0, 0).querySelector('input'));
            await user.click(getCell(1, 0).querySelector('input'));
            expect(getSelectedRowIds()).to.deep.equal([0, 1]);
        ...
        it('should not select unselectable rows given in rowSelectionModel', () => {
            const { setProps } = render(_jsx(TestDataGridSelection, { rowSelectionModel: includeRowSelection([0, 1]), isRowSelectable: (params) => Number(params.id) % 2 === 0, checkboxSelection: true }));
            expect(getSelectedRowIds()).to.deep.equal([0]);
        ...
        it('should filter out unselectable rows when the rowSelectionModel prop changes', () => {
            const { setProps } = render(_jsx(TestDataGridSelection, { rowSelectionModel: includeRowSelection([1]), isRowSelectable: (params) => Number(params.id) > 0, checkboxSelection: true }));
            expect(getSelectedRowIds()).to.deep.equal([1]);
            expect(getColumnHeaderCell(0).querySelector('input')).to.have.attr('data-indeterminate', 'true');
            setProps({ rowSelectionModel: includeRowSelection([0]) });
            expect(getColumnHeaderCell(0).querySelector('input')).to.have.attr('data-indeterminate', 'false');
            expect(getSelectedRowIds()).to.deep.equal([]);
        });
        it('should not crash when paginationMode="server" and some selected rows are not provided to the grid', () => {
            expect(() => {
                render(_jsx(TestDataGridSelection, { paginationMode: "server", rowCount: 4, rowSelectionModel: includeRowSelection([1, 4]), isRowSelectable: (params) => Number(params.id) > 0, checkboxSelection: true }));
            }).not.toErrorDev();
        });
        it('should set the "Select all" checkbox to selected state on clicking even when some rows are not selectable', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, isRowSelectable: ({ id }) => Number(id) % 2 === 0 }));
    ...
    describe('prop: rows', () => {
        it('should remove the outdated selected rows when rows prop changes', () => {
            ...
            const { setProps } = render(_jsx(TestDataGridSelection, { rowSelectionModel: includeRowSelection([0, 1, 2]), checkboxSelection: true, ...data }));
            expect(getSelectedRowIds()).to.deep.equal([0, 1, 2]);
            setProps({
        ...
        it('should call `onRowSelectionModelChange` when outdated selected rows are removed', () => {
            ...
            const { setProps } = render(_jsx(TestDataGridSelection, { rowSelectionModel: includeRowSelection([0, 1, 2]), onRowSelectionModelChange: onRowSelectionModelChangeSpy, checkboxSelection: true, ...data }));
            setProps({
                rows: data.rows.slice(0, 1),
            });
        ...
        it('should retain the outdated selected rows when the rows prop changes when keepNonExistentRowsSelected is true', () => {
            ...
            const onRowSelectionModelChange = spy();
            const { setProps } = render(_jsx(TestDataGridSelection, { rowSelectionModel: includeRowSelection([0, 1, 2]), checkboxSelection: true, keepNonExistentRowsSelected: true, onRowSelectionModelChange: onRowSelectionModelChange, ...data }));
            expect(getSelectedRowIds()).to.deep.equal([0, 1, 2]);
    ...
    describe('prop: rowSelectionModel and onRowSelectionModelChange', () => {
        it('should select rows when initialised', () => {
            render(_jsx(TestDataGridSelection, { rowSelectionModel: includeRowSelection([1]) }));
            expect(getSelectedRowIds()).to.deep.equal([1]);
        });
        it('should not call onRowSelectionModelChange on initialization or on rowSelectionModel prop change', () => {
            const onRowSelectionModelChange = spy();
            const { setProps } = render(_jsx(TestDataGridSelection, { onRowSelectionModelChange: onRowSelectionModelChange, rowSelectionModel: includeRowSelection([0]) }));
        ...
        it('should call onRowSelectionModelChange with an empty array if no row is selectable in the current page when turning off checkboxSelection', async () => {
            const onRowSelectionModelChange = spy();
            const { setProps, user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, pagination: true, initialState: { pagination: { paginationModel: { pageSize: 2 } } }, pageSizeOptions: [2], onRowSelectionModelChange: onRowSelectionModelChange }));
            await user.click(getCell(0, 0).querySelector('input'));
        ...
        it('should call onRowSelectionModelChange with the correct reason when clicking on a row', async () => {
            const onRowSelectionModelChange = spy();
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, pagination: true, onRowSelectionModelChange: onRowSelectionModelChange }));
            await user.click(getCell(0, 0).querySelector('input'));
        ...
        it('should call onRowSelectionModelChange with an empty array if there is no selected row in the current page when turning off checkboxSelection', async () => {
            const onRowSelectionModelChange = spy();
            const { setProps, user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, initialState: { pagination: { paginationModel: { pageSize: 2 } } }, pageSizeOptions: [2], onRowSelectionModelChange: onRowSelectionModelChange }));
            await user.click(getCell(0, 0).querySelector('input'));
            await user.click(getCell(1, 0).querySelector('input'));
        ...
        it('should deselect the old selected rows when updating rowSelectionModel', () => {
            const { setProps } = render(_jsx(TestDataGridSelection, { rowSelectionModel: includeRowSelection([0]) }));
            expect(getSelectedRowIds()).to.deep.equal([0]);
            setProps({ rowSelectionModel: includeRowSelection([1]) });
            expect(getSelectedRowIds()).to.deep.equal([1]);
        });
        ...
        it('should not update the selection model when the rowSelectionModel prop is set', async () => {
            const rowSelectionModel = includeRowSelection([1]);
            const { user } = render(_jsx(TestDataGridSelection, { rowSelectionModel: rowSelectionModel }));
            expect(getSelectedRowIds()).to.deep.equal([1]);
            await user.click(getCell(0, 0));
            expect(getSelectedRowIds()).to.deep.equal([1]);
        });
        it('should update the selection when the model is not set, but the onChange is set', async () => {
            ...
            const { user } = render(_jsx(TestDataGridSelection, { onRowSelectionModelChange: onModelChange }));
            await user.click(getCell(0, 0));
            expect(getSelectedRowIds()).to.deep.equal([0]);
        ...
        it('should control selection state when the model and the onChange are set', async () => {
            function ControlCase() {
                const [rowSelectionModel, setRowSelectionModel] = React.useState(includeRowSelection([]));
                ...
                return (_jsx(TestDataGridSelection, { rowSelectionModel: rowSelectionModel, onRowSelectionModelChange: handleSelectionChange, checkboxSelection: true }));
            }
            const { user } = render(_jsx(ControlCase, {}));
            expect(getSelectedRowIds()).to.deep.equal([]);
            await user.click(getCell(1, 1));
            expect(getSelectedRowIds()).to.deep.equal([1, 2]);
        });
        it('should throw if rowSelectionModel contains more than 1 row', () => {
            ...
            function ControlCase() {
                ...
                return _jsx(TestDataGridSelection, { apiRef: apiRef });
            }
            render(_jsx(ControlCase, {}));
            expect(() => apiRef.current?.setRowSelectionModel(includeRowSelection([0, 1]))).to.throw(/`rowSelectionModel` can only contain 1 item in DataGrid/);
        });
        it('should not throw if rowSelectionModel contains more than 1 item with checkbox selection', () => {
            let apiRef;
            function ControlCase() {
                apiRef = useGridApiRef();
                return _jsx(TestDataGridSelection, { apiRef: apiRef, checkboxSelection: true });
            }
            render(_jsx(ControlCase, {}));
            expect(() => act(() => {
    ...
    describe('prop: rowSelection = false', () => {
        it('should not select rows when clicking the checkbox', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { rowSelection: false, checkboxSelection: true }));
            expect(getSelectedRowIds()).to.deep.equal([]);
            expect(getRow(0).querySelector('input')).to.have.property('checked', false);
            await user.click(getCell(0, 1));
            expect(getSelectedRowIds()).to.deep.equal([]);
        ...
        it('should not select rows with Shift + Space', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { rowSelection: false, disableRowSelectionOnClick: true }));
            const cell0 = getCell(0, 0);
            await user.click(cell0);
            await user.keyboard('{Shift>}[Space]{/Shift}');
            expect(getSelectedRowIds()).to.deep.equal([]);
        ...
        it('should not select rows passed in the rowSelectionModel prop', () => {
            render(_jsx(TestDataGridSelection, { rowSelection: false, rowSelectionModel: includeRowSelection([0]) }));
            expect(getSelectedRowIds()).to.deep.equal([]);
        });
    });
    describe('accessibility', () => {
        ...
        it('should not add aria-selected attributes if the row selection is disabled', async () => {
            const { user } = render(_jsx(TestDataGridSelection, { rowSelection: false }));
    ...
    describe('prop: disableRowSelectionExcludeModel', () => {
        it('should use include model when disableRowSelectionExcludeModel is true', async () => {
            const onRowSelectionModelChange = spy();
            const { user } = render(_jsx(TestDataGridSelection, { checkboxSelection: true, disableRowSelectionExcludeModel: true, onRowSelectionModelChange: onRowSelectionModelChange }));
            // Click "Select all" checkbox
            const selectAllCheckbox = screen.getByRole('checkbox', { name: 'Select all rows' });
            await user.click(selectAllCheckbox);
            expect(onRowSelectionModelChange.callCount).to.equal(1);
        ...
        it('should not rerender unrelated nodes', async () => {
            ...
            const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { columns: [
                        { field: 'id', headerName: 'id', type: 'number' },
                        {
                            ...
                            renderCell: (params) => _jsx(CustomCell, { ...params }),
                        },
                    ], rows: [
                        { id: 0, currencyPair: 'USDGBP' },
                        { id: 1, currencyPair: 'USDEUR' },
                    ], autoHeight: isJSDOM, checkboxSelection: true }) }));
            expect(getSelectedRowIds()).to.deep.equal([]);
            expect(getRow(0).querySelector('input')).to.have.property('checked', false);
            commits = [];
            await user.click(getCell(0, 1));
