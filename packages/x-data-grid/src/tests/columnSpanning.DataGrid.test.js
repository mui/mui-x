...
describe('<DataGrid /> - Column spanning', () => {
    ...
    it('should support `colSpan` number signature', () => {
        render(_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: [
    ...
    describe('key navigation', () => {
        ...
        it('should move to the cell right when pressing "ArrowRight"', async () => {
            const { user } = render(_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: columns }) }));
        ...
        it('should move to the cell below when pressing "ArrowDown"', async () => {
            const { user } = render(_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: columns, disableVirtualization: isJSDOM }) }));
        ...
        it('should move down by the amount of rows visible on screen when pressing "PageDown"', async () => {
            const { user } = render(_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: columns, autoHeight: isJSDOM, disableVirtualization: isJSDOM }) }));
        ...
        it('should move up by the amount of rows visible on screen when pressing "PageUp"', async () => {
            const { user } = render(_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: columns, autoHeight: isJSDOM }) }));
        ...
        it('should move to the cell below when pressing "Enter" after editing', async () => {
            ...
            const { user } = render(_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: editableColumns, autoHeight: isJSDOM, disableVirtualization: isJSDOM }) }));
            await user.click(getCell(1, 3));
            expect(getActiveCell()).to.equal('1-3');
        ...
        it('should move to the cell on the right when pressing "Tab" after editing', async () => {
            const editableColumns = columns.map((column) => ({ ...column, editable: true }));
            const { user } = render(_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: editableColumns, disableVirtualization: isJSDOM }) }));
            await user.click(getCell(1, 1));
            expect(getActiveCell()).to.equal('1-1');
            // start editing
        ...
        it.skipIf(isJSDOM)('should work with row virtualization', async () => {
            ...
            const rowHeight = 52;
            const { user } = render(_jsx("div", { style: { width: 500, height: (rows.length + 1) * rowHeight }, children: _jsx(DataGrid, { columns: [
        ...
        it.skipIf(isJSDOM)('should work with column virtualization', async () => {
            const { user } = render(_jsx("div", { style: { width: 200, height: 200 }, children: _jsx(DataGrid, { columns: [
        ...
        it('should work with filtering', async () => {
            const { user } = render(_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, columns: [
                        { field: 'brand', colSpan: (value, row) => (row.brand === 'Nike' ? 2 : 1) },
                        { field: 'category' },
                        { field: 'price' },
                        { field: 'rating' },
    ...
    it('should work with pagination', async () => {
        ...
        function TestCase() {
            return (_jsx("div", { style: { width: 500, height: 300 }, children: _jsx(DataGrid, { columns: columns, rows: rows, initialState: { pagination: { paginationModel: { pageSize } } }, pageSizeOptions: [pageSize], disableVirtualization: isJSDOM }) }));
        }
    ...
    it.skipIf(isJSDOM)('should work with column virtualization', async () => {
        render(_jsx("div", { style: { width: 390, height: 300 }, children: _jsx(DataGrid, { columns: [
        ...
        expect(getCell(0, 4).offsetLeft).to.equal(getCell(1, 4).offsetLeft, 'last cells in both rows should be aligned');
        expect(getColumnHeaderCell(4).offsetLeft).to.equal(getCell(1, 4).offsetLeft, 'last cell and column header cell should be aligned');
        const virtualScroller = document.querySelector(`.${gridClasses.virtualScroller}`);
        // scroll to the very end
        await act(async () => virtualScroller.scrollTo({ left: 1000, behavior: 'instant' }));
        expect(getCell(0, 5).offsetLeft).to.equal(getCell(1, 5).offsetLeft, 'last cells in both rows should be aligned after scroll');
        expect(getColumnHeaderCell(5).offsetLeft).to.equal(getCell(1, 5).offsetLeft, 'last cell and column header cell should be aligned after scroll');
    });
    // Need layout for column virtualization
    it.skipIf(isJSDOM)('should work with both column and row virtualization', async () => {
        ...
        expect(getCell(2, 4).offsetLeft).to.equal(getCell(1, 4).offsetLeft, 'last cells in both rows should be aligned');
        expect(getColumnHeaderCell(4).offsetLeft).to.equal(getCell(1, 4).offsetLeft, 'last cell and column header cell should be aligned');
        const virtualScroller = document.querySelector(`.${gridClasses.virtualScroller}`);
        await act(async () => virtualScroller.scrollTo({
        ...
        await waitFor(() => {
            expect(getCell(2, 5).offsetLeft).to.equal(getCell(1, 5).offsetLeft, 'last cells in both rows should be aligned after scroll');
        });
        expect(getColumnHeaderCell(5).offsetLeft).to.equal(getCell(1, 5).offsetLeft, 'last cell and column header cell should be aligned after scroll');
    });
    // Need layout for column virtualization
    it.skipIf(isJSDOM)('should work with pagination and column virtualization', async () => {
        const rowHeight = 50;
        function TestCase() {
            return (_jsx("div", { style: { width: 390, height: 300 }, children: _jsx(DataGrid, { pageSizeOptions: [3], initialState: { pagination: { paginationModel: { pageSize: 3 } } }, columns: [
        ...
        const { user } = render(_jsx(TestCase, {}));
        await user.click(screen.getByRole('button', { name: /next page/i }));
        expect(getCell(5, 4).offsetLeft).to.equal(getCell(4, 4).offsetLeft, 'last cells in both rows should be aligned');
        expect(getColumnHeaderCell(4).offsetLeft).to.equal(getCell(4, 4).offsetLeft, 'last cell and column header cell should be aligned');
        const virtualScroller = document.querySelector(`.${gridClasses.virtualScroller}`);
        await act(async () => virtualScroller.scrollTo({
            // hide first row to trigger row virtualization
            top: rowHeight + 10,
        ...
        await waitFor(() => {
            expect(getCell(5, 5).offsetLeft).to.equal(getCell(4, 5).offsetLeft, 'last cells in both rows should be aligned after scroll');
        });
        expect(getColumnHeaderCell(5).offsetLeft).to.equal(getCell(4, 5).offsetLeft, 'last cell and column header cell should be aligned after scroll');
    });
});
