...
describe('<DataGrid /> - Columns', () => {
    ...
    describe('prop: initialState.columns.orderedFields / initialState.columns.dimensions', () => {
        ...
        it('should not add a column when present in the initial state but not in the props', () => {
            render(_jsx(TestDataGrid, { initialState: { columns: { orderedFields: ['idTres'] } } }));
        ...
        it('should move the columns not present in the initial state after the one present in it', () => {
            render(_jsx(TestDataGrid, { initialState: { columns: { orderedFields: ['idBis'] } } }));
            expect(getColumnHeadersTextContent()).to.deep.equal(['idBis', 'id']);
        });
        it('should allow to remove the sizing properties by setting them to `undefined`', () => {
            render(_jsx(TestDataGrid, { columns: [{ field: 'id', flex: 1 }], initialState: { columns: { dimensions: { id: { flex: undefined } } } } }));
            expect(getColumnHeaderCell(0)).toHaveInlineStyle({ width: '100px' });
        });
    });
    it('should allow to change the column type', () => {
        const { setProps } = render(_jsx(TestDataGrid, { columns: [{ field: 'id', type: 'string' }, { field: 'idBis' }] }));
    ...
    it('should not persist valueFormatter on column type change', () => {
        const { setProps } = render(_jsx(TestDataGrid, { columns: [{ field: 'price', type: 'number', valueFormatter: (value) => `$${value}` }], rows: [{ id: 0, price: 1 }] }));
    ...
    it('should not override column properties when changing column type', () => {
        const { setProps } = render(_jsx(TestDataGrid, { columns: [
                {
                    field: 'id',
                    type: 'string',
                    width: 200,
    ...
    // Needs layout
    it.skipIf(isJSDOM)('should not crash when updating columns immediately after scrolling', async () => {
        const data = [
            { id: 1, value: 'A' },
            { id: 2, value: 'B' },
            { id: 3, value: 'C' },
        ...
        function DynamicVirtualizationRange() {
            ...
            return (_jsxs("div", { style: { width: '100%' }, children: [_jsx("button", { onClick: () => setCols([{ field: 'id' }]), children: "Update columns" }), _jsx("div", { style: { height: 400 }, children: _jsx(DataGrid, { rows: data, columns: cols }) })] }));
        }
        const { user } = render(_jsx(DynamicVirtualizationRange, {}));
    ...
    it('should revert to the default column properties if not specified otherwise', async () => {
        const columns1 = [{ field: 'status', type: 'string' }];
        const columns2 = [
            { field: 'status', type: 'string', sortable: false, filterable: false },
        ];
        const apiRef = { current: null };
        function Component(props) {
            return (_jsx("div", { style: { height: 560, width: '100%' }, children: _jsx(DataGrid, { apiRef: apiRef, rows: [{ id: 1, status: 'pending' }], ...props }) }));
        }
        const { setProps } = render(_jsx(Component, { columns: columns1 }));
