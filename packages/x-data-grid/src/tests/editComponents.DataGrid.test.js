...
describe('<DataGrid /> - Edit components', () => {
    ...
    function TestCase(props) {
        ...
        return (_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { apiRef: apiRef, ...defaultData, ...props }) }));
    }
    describe('column type: longText', () => {
        ...
        it('should pass the value prop to the textarea', async () => {
            defaultData.columns[0].valueParser = (value) => value.toUpperCase();
            const { user } = render(_jsx(TestCase, {}));
            const cell = getCell(0, 0);
            await openLongTextEditPopup(cell, user);
            const textarea = screen.getByRole('textbox');
        ...
        it('should call onValueChange if defined', async () => {
            ...
            defaultData.columns[0].renderEditCell = (params) => renderEditLongTextCell({ ...params, onValueChange });
            const { user } = render(_jsx(TestCase, {}));
            const cell = getCell(0, 0);
            await openLongTextEditPopup(cell, user);
            const textarea = screen.getByRole('textbox');
