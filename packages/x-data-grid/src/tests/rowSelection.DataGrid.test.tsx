...
describe('<DataGrid /> - Row selection', () => {
  ...
  describe('prop: checkboxSelection = false (single selection)', () => {
    ...
    it(`should not deselect the selected row on click WITHOUT ctrl or meta pressed`, async () => {
      const { user } = render(<TestDataGridSelection />);
      await user.click(getCell(0, 0));
      expect(getSelectedRowIds()).to.deep.equal([0]);
      await user.click(getCell(0, 0));
  ...
  describe('prop: checkboxSelection = true (multi selection)', () => {
    ...
    describe('prop: indeterminateCheckboxAction = "select"', () => {
      it('should select all the rows when clicking on "Select All" checkbox in indeterminate state', async () => {
        const { user } = render(
          <TestDataGridSelection checkboxSelection indeterminateCheckboxAction="select" />,
        );
        const selectAllCheckbox = screen.getByRole('checkbox', { name: 'Select all rows' });
    ...
    describe('prop: indeterminateCheckboxAction = "deselect"', () => {
      it('should deselect all the rows when clicking on "Select All" checkbox in indeterminate state', async () => {
        const { user } = render(
          <TestDataGridSelection checkboxSelection indeterminateCheckboxAction="deselect" />,
        );
        const selectAllCheckbox = screen.getByRole('checkbox', { name: 'Select all rows' });
        await user.click(screen.getAllByRole('checkbox', { name: /select row/i })[0]);
        await user.click(selectAllCheckbox);
