...
describe('<DataGrid /> - Quick filter', () => {
    ...
    function TestCase(props) {
        return (_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...baselineProps, showToolbar: true, disableColumnSelector: true, disableDensitySelector: true, disableColumnFilter: true, ...props, slotProps: {
                    ...props?.slotProps,
                    toolbar: {
    ...
    describe('component', () => {
        ...
        it('should allow to customize input splitting', async () => {
            ...
            const { user } = render(_jsx(TestCase, { onFilterModelChange: onFilterModelChange, slotProps: {
                    toolbar: {
                        quickFilterProps: {
        ...
        it('should allow to customize input formatting', () => {
            const { setProps } = render(_jsx(TestCase, { slotProps: {
                    toolbar: {
                        quickFilterProps: {
        ...
        it('should be collapsed by default if there is no value', () => {
            ...
            expect(screen.getByRole('button', { name: 'Search' }).getAttribute('aria-expanded')).to.equal('false');
        });
        it('should be expanded by default if there is a value', () => {
            render(_jsx(TestCase, { filterModel: { items: [], quickFilterValues: ['adidas'] } }));
            expect(screen.getByRole('searchbox').value).to.equal('adidas');
            expect(screen.getByRole('searchbox').tabIndex).to.equal(0);
            expect(screen.getByRole('button', { name: 'Search' }).getAttribute('aria-expanded')).to.equal('true');
        });
        ...
        it('should clear the value when the clear button is clicked and focus to `the input', async () => {
            const { user } = render(_jsx(TestCase, { filterModel: { items: [], quickFilterValues: ['adidas'] } }));
    ...
    describe('quick filter logic', () => {
        ...
        it('should return rows that match some values if quickFilterLogicOperator="or"', async () => {
            const { user } = render(_jsx(TestCase, { initialState: {
        ...
        it('should ignore hidden columns by default', async () => {
            const { user } = render(_jsx(TestCase, { columns: [{ field: 'id' }, { field: 'brand' }], initialState: {
        ...
        it('should apply filters on quickFilterExcludeHiddenColumns value change', () => {
            const { setProps } = render(_jsx(TestCase, { columns: [{ field: 'id' }, { field: 'brand' }], columnVisibilityModel: { brand: false }, filterModel: {
                    items: [],
        ...
        it('should apply filters on column visibility change when quickFilterExcludeHiddenColumns=true', () => {
            ...
            const { setProps } = render(_jsx(TestCase, { columns: [
                    {
                        field: 'id',
    ...
    describe('column type: string', () => {
        const getRows = ({ quickFilterValues }) => {
            const { unmount } = render(_jsx(TestCase, { filterModel: {
                    items: [],
                    quickFilterValues,
                }, rows: [
        ...
        describe('ignoreDiacritics', () => {
            function DiacriticsTestCase({ quickFilterValues, ...props }) {
                return (_jsx(TestCase, { ...props, filterModel: {
                        items: [],
                        quickFilterValues,
                    }, rows: [{ id: 0, label: 'Apă' }], columns: [{ field: 'label', type: 'string' }] }));
            }
            it('should not ignore diacritics by default', () => {
                const { unmount } = render(_jsx(DiacriticsTestCase, { quickFilterValues: ['apa'] }));
                expect(getColumnValues(0)).to.deep.equal([]);
                unmount();
                const { unmount: unmount2 } = render(_jsx(DiacriticsTestCase, { quickFilterValues: ['apă'] }));
                expect(getColumnValues(0)).to.deep.equal(['Apă']);
                unmount2();
            });
            it('should ignore diacritics when `ignoreDiacritics` is enabled', () => {
                const { unmount } = render(_jsx(DiacriticsTestCase, { quickFilterValues: ['apa'], ignoreDiacritics: true }));
                expect(getColumnValues(0)).to.deep.equal(['Apă']);
                unmount();
                const { unmount: unmount2 } = render(_jsx(DiacriticsTestCase, { quickFilterValues: ['apă'], ignoreDiacritics: true }));
                expect(getColumnValues(0)).to.deep.equal(['Apă']);
                unmount2();
            });
        });
    ...
    it('should not override user input when typing', async () => {
        ...
        const debounceMs = 50;
        const { user } = render(_jsx(TestCase, { slotProps: {
                toolbar: {
