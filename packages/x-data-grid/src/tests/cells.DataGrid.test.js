...
describe('<DataGrid /> - Cells', () => {
    ...
    describe('cellClassName', () => {
        it('should append the CSS class defined in cellClassName', () => {
            render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, columns: [{ field: 'brand', cellClassName: 'foobar' }] }) }));
    ...
    describe('prop: showCellVerticalBorder', () => {
        ...
        it('should add right border to cells', () => {
            render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, columns: [{ field: 'id' }, { field: 'brand' }], showCellVerticalBorder: true }) }));
        ...
        it('should add right border to cells in the last row', () => {
            render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, autoHeight: true, columns: [{ field: 'id' }, { field: 'brand' }], showCellVerticalBorder: true }) }));
            expectRightBorder(getCell(2, 0));
        });
    });
    it('should append the CSS class returned by cellClassName', () => {
        render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, columns: [{ field: 'brand' }], getCellClassName: () => 'foobar' }) }));
        expect(getCell(0, 0)).to.have.class('foobar');
    });
    it('should allow renderCell to return a false-ish value', () => {
        render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { autoHeight: isJSDOM, columns: [{ field: 'brand', renderCell: () => 0 }], rows: [{ id: 1, brand: 'Nike' }] }) }));
        expect(getCell(0, 0)).to.have.text('0');
    });
    it('should render nothing in cell when renderCell returns a `null` value', () => {
        render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { autoHeight: isJSDOM, columns: [{ field: 'brand', renderCell: () => null }], rows: [{ id: 1, brand: 'Nike' }] }) }));
    ...
    it('should call the valueFormatter with the correct params', () => {
        ...
        render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { columns: [
                    {
                        field: 'isActive',
                        valueFormatter,
                        width: 200,
    ...
    it('should throw when focusing cell without updating the state', async () => {
        render(_jsx("div", { style: { width: 300, height: 500 }, children: _jsx(DataGrid, { ...baselineProps, columns: [{ field: 'brand', cellClassName: 'foobar' }], experimentalFeatures: { warnIfFocusStateIsNotSynced: true } }) }));
        expect(() => {
            getCell(1, 0).focus();
    ...
    it('should keep the focused cell/row rendered in the DOM if it scrolls outside the viewport', async () => {
        ...
        const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { columns: defaultData.columns, rows: defaultData.rows, rowHeight: rowHeight }) }));
    ...
    it('should not cause scroll jump when focused cell mounts in the render zone', async () => {
        ...
        const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { columns: columns, rows: rows, rowHeight: rowHeight }) }));
        const virtualScroller = document.querySelector('.MuiDataGrid-virtualScroller');
    ...
    describe('column type: longText', () => {
        ...
        it('should render expand button on hover/focus', async () => {
            const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { ...longTextBaselineProps }) }));
        ...
        describe('popup', () => {
            it('should display full text in edit popup', async () => {
                const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { autoHeight: isJSDOM, rows: [{ id: 0, bio: 'This is a long text with multiple lines' }], columns: [{ field: 'bio', type: 'longText', editable: true }] }) }));
                const cell = getCell(0, 0);
            ...
            describe('view popup', () => {
                ...
                it('should render custom content via renderContent prop', async () => {
                    const customRenderContent = spy((value) => (_jsxs("span", { "data-testid": "custom-content", children: ["Custom: ", value] })));
                    const { user } = render(_jsx("div", { style: { width: 300, height: 300 }, children: _jsx(DataGrid, { rows: [{ id: 0, bio: 'Test content' }], columns: [
                                {
                                    field: 'bio',
                                    type: 'longText',
                                    renderCell: (params) => renderLongTextCell({ ...params, renderContent: customRenderContent }),
                                },
                            ] }) }));
                    const cell = getCell(0, 0);
                    await openLongTextViewPopup(cell, user, 'spacebar');
