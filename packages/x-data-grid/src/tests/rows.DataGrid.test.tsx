...
describe('<DataGrid /> - Rows', () => {
  ...
  describe('apiRef: scrollToIndexes', () => {
    it('should accept a row index in the current page', () => {
      const localApiRef = React.createRef<GridApiCommunity>();
      ...
      expect(() => {
        localApiRef.current?.scrollToIndexes({ rowIndex: 3 });
      }).not.toWarnDev();
    });
    ...
    it('should warn when the row index is outside the current page', () => {
      ...
      expect(() => {
        result = localApiRef.current?.scrollToIndexes({ rowIndex: 0 });
      }).toWarnDev(
        [
    ...
    it('should warn when the column index is out of bounds', () => {
      ...
      render(
        <div style={{ width: 300, height: 300 }}>
          <DataGrid apiRef={localApiRef} columns={[{ field: 'id' }]} rows={[{ id: 1 }]} />
        </div>,
      );

      let result: boolean | undefined;
      expect(() => {
        result = localApiRef.current?.scrollToIndexes({ colIndex: 1 });
      }).toWarnDev(
        [
