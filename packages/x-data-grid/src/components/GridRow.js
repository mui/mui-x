...
import { gridEditRowsStateSelector, gridRowIsEditingSelector, } from '../hooks/features/editing/gridEditingSelectors';
...
const isRowReorderingEnabledSelector = createSelector(gridEditRowsStateSelector, (editRows, { rowReordering, treeData }) => {
...
const GridRow = forwardRef(function GridRow(props, refProp) {
    const { selected, rowId, row, index, style: styleProp, rowHeight, className, visibleColumns, pinnedColumns, offsetLeft, columnsTotalWidth, firstColumnIndex, lastColumnIndex, focusedColumnIndex, isFirstVisible, isLastVisible, isNotVisible, showBottomBorder, scrollbarWidth, gridHasFiller, onClick, onDoubleClick, onMouseEnter, onMouseLeave, onMouseOut, onMouseOver, ...other } = props;
    ...
    const hasVirtualFocusCellLeft = hasFocusCell &&
        focusedColumnIndex >= pinnedColumns.left.length &&
        focusedColumnIndex < firstColumnIndex;
    const hasVirtualFocusCellRight = hasFocusCell &&
    ...
    const heightEntry = useGridSelector(apiRef, () => ({ ...apiRef.current.getRowHeightEntry(rowId) }), undefined, objectShallowCompare);
    ...
    const getCell = (column, indexInSection, indexRelativeToAllColumns, sectionLength, pinnedPosition = PinnedColumnPosition.NONE) => {
        ...
        if (rowNode.type === 'skeletonRow') {
            return (_jsx(slots.skeletonCell, { type: column.type, width: width, height: rowHeight, field: column.field, align: column.align }, column.field));
        }
        ...
        const disableDragEvents = !(canReorderColumn ||
        ...
        return (_jsx(slots.cell, { column: column, width: width, rowId: rowId, align: column.align || 'left', colIndex: indexRelativeToAllColumns, colSpan: colSpan, disableDragEvents: disableDragEvents, isNotVisible: cellIsNotVisible, pinnedOffset: pinnedOffset, pinnedPosition: pinnedPosition, showLeftBorder: showLeftBorder, showRightBorder: showRightBorder, row: row, rowNode: rowNode, ...slotProps?.cell }, column.field));
    };
    ...
    const eventHandlers = row
        ...
        : null;
    return (_jsxs("div", { "data-id": rowId, "data-rowindex": index, role: "row", className: clsx(...rowClassNames, classes.root, className), style: style, ...ariaAttributes, ...eventHandlers, ...other, ref: handleRef, children: [leftCells, _jsx("div", { role: "presentation", className: gridClasses.cellOffsetLeft, style: { width: offsetLeft } }), cells, _jsx("div", { role: "presentation", className: clsx(gridClasses.cell, gridClasses.cellEmpty) }), rightCells, _jsx(GridRowDragAndDropOverlay, { rowId: rowId })] }));
});
GridRow.propTypes = {
    // ----------------------------- Warning --------------------------------
