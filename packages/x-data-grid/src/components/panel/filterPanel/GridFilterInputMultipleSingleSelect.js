...
function GridFilterInputMultipleSingleSelect(props) {
    ...
    var resolvedColumn = apiRef.current.getColumn(item.field);
    var getOptionValue = resolvedColumn.getOptionValue;
    var getOptionLabel = resolvedColumn.getOptionLabel;
    ...
    if (!resolvedColumn || !(0, filterPanelUtils_1.isSingleSelectColDef)(resolvedColumn)) {
        return null;
    }
    var BaseAutocomplete = rootProps.slots.baseAutocomplete;
    return ((0, jsx_runtime_1.jsx)(BaseAutocomplete, __assign({ multiple: true, options: resolvedValueOptions, isOptionEqualToValue: isOptionEqualToValue, id: id, value: filteredValues, onChange: handleChange, getOptionLabel: getOptionLabel, label: apiRef.current.getLocaleText('filterPanelInputLabel'), placeholder: apiRef.current.getLocaleText('filterPanelInputPlaceholder'), slotProps: {
            textField: {
                type: type || 'text',
