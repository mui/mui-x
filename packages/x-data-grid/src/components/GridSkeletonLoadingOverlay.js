...
import { gridColumnPositionsSelector, gridDimensionsSelector, gridVisibleColumnDefinitionsSelector, gridVisiblePinnedColumnDefinitionsSelector, useGridEvent, useGridSelector, } from '../hooks';
...
export const GridSkeletonLoadingOverlayInner = forwardRef(function GridSkeletonLoadingOverlayInner(props, forwardedRef) {
    ...
    const children = React.useMemo(() => {
        ...
        for (let i = 0; i < skeletonRowsCount; i += 1) {
            ...
            for (let colIndex = 0; colIndex < columns.length; colIndex += 1) {
                ...
                const emptyCell = (_jsx(slots.skeletonCell, { width: emptyCellWidth, empty: true }, `skeleton-filler-column-${i}`));
                if (hasFillerBefore) {
                    rowCells.push(emptyCell);
                }
                rowCells.push(_jsx(slots.skeletonCell, { field: column.field, type: column.type, align: column.align, width: "var(--width)", height: dimensions.rowHeight, "data-colindex": colIndex, empty: visibleColumns && !visibleColumns.has(column.field), className: clsx(isPinnedLeft && gridClasses['cell--pinnedLeft'], isPinnedRight && gridClasses['cell--pinnedRight'], showRightBorder && gridClasses['cell--withRightBorder'], showLeftBorder && gridClasses['cell--withLeftBorder']), style: { '--width': `${column.computedWidth}px`, ...pinnedStyle } }, `skeleton-column-${i}-${column.field}`));
                if (hasFillerAfter) {
                    rowCells.push(emptyCell);
                }
            }
            array.push(_jsx("div", { className: clsx(gridClasses.row, gridClasses.rowSkeleton, i === 0 && !showFirstRowBorder && gridClasses['row--firstVisible']), children: rowCells }, `skeleton-row-${i}`));
        }
        return array;
    }, [
        skeletonRowsCount,
    ...
    return (_jsx(SkeletonOverlay, { className: classes.root, ...other, ref: handleRef, children: children }));
});
export const GridSkeletonLoadingOverlay = forwardRef(function GridSkeletonLoadingOverlay(props, forwardedRef) {
    ...
    return (_jsx(GridSkeletonLoadingOverlayInner, { ...props, skeletonRowsCount: skeletonRowsCount, ref: forwardedRef }));
});
