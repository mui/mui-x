...
import { gridDimensionsSelector, gridPinnedColumnsSelector, useGridEvent, useGridSelector, } from '../hooks';
import { gridPinnedRowsSelector } from '../hooks/features/rows/gridRowsSelector';
import { useGridRootProps } from '../hooks/utils/useGridRootProps';
import { vars } from '../constants/cssVariables';
import { useGridPrivateApiContext } from '../hooks/utils/useGridPrivateApiContext';
import { gridHasScrollXSelector, gridHasScrollYSelector, } from '../hooks/features/dimensions/gridDimensionsSelectors';
...
function GridScrollShadows(props) {
    ...
    const initialScrollable = position === 'vertical'
    ...
    const updateScrollShadowVisibility = React.useCallback((scrollPosition) => {
        ...
        const maxScroll = Math.round(dimensions.contentSize[position === 'vertical' ? 'height' : 'width'] -
            dimensions.viewportInnerSize[position === 'vertical' ? 'height' : 'width']);
        const hasPinnedStart = position === 'vertical'
            ? pinnedRows?.top?.length > 0
            : pinnedColumns?.left?.length !== undefined && pinnedColumns?.left?.length > 0;
        const hasPinnedEnd = position === 'vertical'
    ...
    React.useEffect(() => {
        updateScrollShadowVisibility((position === 'horizontal'
    ...
    return (_jsx(ScrollShadow, { className: classes.root, ownerState: ownerState, ref: ref, style: {
            '--hasScrollStart': 0,
