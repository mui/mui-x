...
const GridHeaderCheckbox = forwardRef(function GridHeaderCheckbox(props, ref) {
    ...
    const filteredSelection = React.useMemo(() => {
        ...
        const selectionModel = { type: 'include', ids: new Set() };
    ...
    const selectionCandidates = React.useMemo(() => {
        const rowIds = !rootProps.pagination ||
    ...
    const tabIndex = tabIndexState !== null && tabIndexState.field === props.field && multipleSelectionEnabled
        ? 0
        : -1;
    useEnhancedEffect(() => {
    ...
    const label = apiRef.current.getLocaleText(isChecked && !isIndeterminate
        ? 'checkboxSelectionUnselectAllRows'
        : 'checkboxSelectionSelectAllRows');
    return (_jsx(rootProps.slots.baseCheckbox, { indeterminate: isIndeterminate, checked: isChecked && !isIndeterminate, onChange: handleChange, className: classes.root, slotProps: {
            htmlInput: { 'aria-label': label, name: 'select_all_rows' },
        }, tabIndex: tabIndex, onKeyDown: handleKeyDown, disabled: !multipleSelectionEnabled, ...rootProps.slotProps?.baseCheckbox, ...other, ref: ref }));
});
