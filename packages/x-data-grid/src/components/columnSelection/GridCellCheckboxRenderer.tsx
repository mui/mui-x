...
import { gridFilteredRowsLookupSelector } from '../../hooks/features/filter/gridFilterSelector';
...
const GridCellCheckboxForwardRef = React.forwardRef<HTMLInputElement, GridRenderCellParams>(
  function GridCellCheckboxRenderer(props, ref) {
    ...
    const filteredRowsLookup = useGridSelector(apiRef, gridFilteredRowsLookupSelector);
    const children: GridRowId[] = React.useMemo(() => {
      if (rowNode.type === 'group') {
        // @ts-expect-error Access `GridProApi` method
        return apiRef.current.getRowGroupChildren({
          groupId: id,
    ...
    const someChildrenSelected = React.useMemo(() => {
      if (rowNode.type === 'group') {
        return children.some(
          (child) => filteredRowsLookup[child] && rowSelectionLookup[child] !== undefined,
        );
      }
      return false;
    }, [children, rowNode.type, rowSelectionLookup, filteredRowsLookup]);
