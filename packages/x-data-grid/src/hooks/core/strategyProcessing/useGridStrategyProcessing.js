...
export const useGridStrategyProcessing = (apiRef) => {
    ...
    const registerStrategyProcessor = React.useCallback((strategyName, processorName, processor) => {
        const cleanup = () => {
            const { [strategyName]: removedPreProcessor, ...otherProcessors } = strategiesCache.current[processorName];
    ...
    const applyStrategyProcessor = React.useCallback((processorName, params) => {
        ...
        if (activeStrategy == null) {
            throw new Error("MUI X: Can't apply a strategy processor before defining an active strategy");
        }
        const groupCache = strategiesCache.current[processorName];
        if (!groupCache || !groupCache[activeStrategy]) {
            throw new Error(`MUI X: No processor found for processor "${processorName}" on strategy "${activeStrategy}"`);
        }
