...
import { gridColumnFieldsSelector, gridColumnLookupSelector, gridColumnVisibilityModelSelector, } from '../columns';
let hasEval;
function getHasEval() {
...
export const mergeStateWithFilterModel = (filterModel, disableMultipleColumnsFiltering, apiRef) => (filteringState) => ({
    ...filteringState,
...
const getFilterCallbackFromItem = (filterItem, filterValueGetter, apiRef) => {
    ...
    if (!filterOperators?.length) {
        throw new Error(`MUI X Data Grid: No filter operators found for column "${column.field}". ` +
    ...
    if (!filterOperator) {
        throw new Error(`MUI X Data Grid: No filter operator "${newFilterItem.operator}" found for column "${column.field}". ` +
...
${appliers
        .map((applier, i) => `const shouldApply${i} = !shouldApplyFilter || shouldApplyFilter(${JSON.stringify(applier.item.field)});`)
        .join('\n')}

const result$$ = {
${appliers
        .map((applier, i) => `  ${JSON.stringify(String(applier.item.id))}: !shouldApply${i} ? false : appliers[${i}].fn(row),`)
        .join('\n')}
};
