...
var getFilterCallbackFromItem = function (filterItem, filterValueGetter, apiRef) {
    var _a;
    ...
    return {
        item: newFilterItem,
        fn: function (row) {
            var value = filterValueGetter(row, column);
            if (ignoreDiacritics) {
 ...
 */
var buildAggregatedFilterItemsApplier = function (filterModel, filterValueGetter, apiRef, disableEval) {
    var items = filterModel.items;
    var appliers = items
        .map(function (item) { return getFilterCallbackFromItem(item, filterValueGetter, apiRef); })
 ...
 * @returns {GridAggregatedFilterItemApplier | null} A method that checks if a row is matching the current filter model. If `null`, we consider that all the rows are matching the filters.
 */
var buildAggregatedQuickFilterApplier = function (filterModel, filterValueGetter, apiRef) {
    var _a, _b;
...
var buildAggregatedFilterApplier = function (filterModel, filterValueGetter, apiRef, disableEval) {
    var isRowMatchingFilterItems = buildAggregatedFilterItemsApplier(filterModel, filterValueGetter, apiRef, disableEval);
    var isRowMatchingQuickFilter = buildAggregatedQuickFilterApplier(filterModel, filterValueGetter, apiRef);
...
var filterModelItems = function (cache, filterValueGetter, apiRef, items) {
    if (!cache.cleanedFilterItems) {
        cache.cleanedFilterItems = items.filter(function (item) { return getFilterCallbackFromItem(item, filterValueGetter, apiRef) !== null; });
    }
    return cache.cleanedFilterItems;
};
var passFilterLogic = function (allFilterItemResults, allQuickFilterResults, filterModel, filterValueGetter, apiRef, cache) {
    var _a, _b;
    var cleanedFilterItems = filterModelItems(cache, filterValueGetter, apiRef, filterModel.items);
