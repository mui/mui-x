...
export const useGridFilter = (apiRef, props, configuration) => {
    ...
    const showFilterPanel = React.useCallback((targetColumnField, panelId, labelId) => {
        ...
        if (targetColumnField) {
            ...
            const filterItemsWithValue = filterModel.items.filter((item) => {
                ...
                const requiresFilterValue = typeof filterOperator?.requiresFilterValue === 'undefined'
                    ? true
            ...
            if (filterItemOnTarget) {
                ...
                newFilterItems = [
                    cleanFilterItem({ field: targetColumnField, operator: targetColumn.filterOperators[0].value }, apiRef),
                ];
            }
            else {
                newFilterItems = [
    ...
    const getFilterState = React.useCallback((inputFilterModel) => {
        ...
        const isRowMatchingFilters = props.filterMode === 'client'
    ...
    const preferencePanelPreProcessing = React.useCallback((initialValue, value) => {
        if (value === GridPreferencePanelsValue.filters) {
            ...
            return _jsx(FilterPanel, { ...props.slotProps?.filterPanel });
        }
        return initialValue;
