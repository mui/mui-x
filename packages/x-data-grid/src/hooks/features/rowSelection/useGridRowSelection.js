...
import { gridRowsLookupSelector, gridRowMaximumTreeDepthSelector, gridRowNodeSelector, gridRowTreeSelector, } from '../rows/gridRowsSelector';
import { gridRowSelectionManagerSelector, gridRowSelectionStateSelector, gridRowSelectionCountSelector, gridRowSelectionIdsSelector, } from './gridRowSelectionSelector';
import { gridFocusCellSelector } from '../focus/gridFocusStateSelector';
import { gridExpandedSortedRowIdsSelector, gridFilteredRowsLookupSelector, gridFilterModelSelector, gridQuickFilterValuesSelector, } from '../filter/gridFilterSelector';
...
const emptyModel = { type: 'include', ids: new Set() };
...
export const useGridRowSelection = (apiRef, props) => {
    ...
    const applyAutoSelection = props.signature !== GridSignature.DataGrid &&
    ...
    const getRowsToBeSelected = useEventCallback(() => {
        const rowsToBeSelected = props.pagination && props.checkboxSelectionVisibleOnly && props.paginationMode === 'client'
    ...
    const setRowSelectionModel = React.useCallback((model, reason) => {
        if (props.signature === GridSignature.DataGrid &&
            ...
            throw new Error(`MUI X: \`rowSelectionModel\` can only contain 1 item in DataGrid.
You need to upgrade to DataGridPro or DataGridPremium component to unlock multiple selection.`);
    ...
    const selectRow = React.useCallback((id, isSelected = true, resetSelection = false) => {
        ...
        if (resetSelection) {
            ...
            const isSelectionValid = (newSelectionModel.type === 'include' && newSelectionModel.ids.size < 2) ||
                canHaveMultipleSelection;
            if (isSelectionValid) {
                apiRef.current.setRowSelectionModel(newSelectionModel, 'singleRowSelection');
            }
    ...
    const selectRowRange = React.useCallback(({ startId, endId, }, isSelected = true, resetSelection = false) => {
    ...
    const removeOutdatedSelection = React.useCallback(() => {
        ...
        const shouldReapplyPropagation = isNestedData &&
            props.rowSelectionPropagation?.parents &&
