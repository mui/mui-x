...
export const useGridRowSelection = (
  ...
  const handleFilteredRowsSet = React.useCallback(() => {
    if (!props.propagateRowSelection) {
      return;
    }
    const filteredRows = gridExpandedSortedRowIdsSelector(apiRef);
    const selectedRows = selectedGridRowsSelector(apiRef);
    const newSelectedRows: GridRowId[] = [];
    selectedRows.forEach((row) => {
      if (filteredRows.includes(row?.id) && !(tree[row.id] as GridGroupNode)?.isAutoGenerated) {
  ...
  useGridApiEventHandler(
    apiRef,
    'filteredRowsSet',
    runIfRowSelectionIsEnabled(handleFilteredRowsSet),
  );
