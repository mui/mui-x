...
function useGridColumns(apiRef, props) {
    ...
    var updateColumns = React.useCallback(function (columns) {
        ...
        if ((0, gridPivotingSelectors_1.gridPivotActiveSelector)(apiRef)) {
            var nonPivotColumns_1 = [];
            var pivotColumns_1 = [];
            var pivotInitialColumns_1 = (0, gridPivotingSelectors_1.gridPivotInitialColumnsSelector)(apiRef);
            columns.forEach(function (column) {
                var isNonPivotColumn = pivotInitialColumns_1.has(column.field);
                if (isNonPivotColumn) {
                    nonPivotColumns_1.push(column);
                }
                else {
                    pivotColumns_1.push(column);
                }
            });
            if (nonPivotColumns_1.length > 0) {
                apiRef.current.updateNonPivotColumns(nonPivotColumns_1);
            }
            if (pivotColumns_1.length === 0) {
                return;
            }
            columnsToUpdate = pivotColumns_1;
        }
        var columnsState = (0, gridColumnsUtils_1.createColumnsState)({
            apiRef: apiRef,
    ...
    var preferencePanelPreProcessing = React.useCallback(function (initialValue, value) {
        ...
        if (value === preferencesPanel_1.GridPreferencePanelsValue.columns) {
            ...
            return (0, jsx_runtime_1.jsx)(ColumnsPanel, __assign({}, (_a = props.slotProps) === null || _a === void 0 ? void 0 : _a.columnsPanel));
        }
        return initialValue;
