 ...
 */
export function computeFlexColumnsWidth({ initialFreeSpace, totalFlexUnits, flexColumns, }) {
    ...
    function loopOverFlexItems() {
        ...
        const violationsLookup = { min: {}, max: {} };
...
export const hydrateColumnsWidth = (rawState, dimensions) => {
    ...
    const availableWidth = dimensions === undefined
        ? 0
...
export const applyInitialState = (columnsState, initialState) => {
    ...
    const newOrderedFields = cleanOrderedFields.length === 0
        ? columnsState.orderedFields
        : [
            ...cleanOrderedFields,
            ...columnsState.orderedFields.filter((field) => !orderedFieldsLookup[field]),
        ];
    const newColumnLookup = { ...columnsState.lookup };
...
export const createColumnsState = ({ apiRef, columnsToUpsert, initialState, columnVisibilityModel = gridColumnVisibilityModelSelector(apiRef), keepOnlyColumnsToUpsert = false, updateInitialVisibilityModel = false, }) => {
...
export function getFirstNonSpannedColumnToRender({ firstColumnToRender, apiRef, firstRowToRender, lastRowToRender, visibleRows, }) {
