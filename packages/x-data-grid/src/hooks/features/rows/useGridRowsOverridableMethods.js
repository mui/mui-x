...
export const useGridRowsOverridableMethods = (apiRef) => {
    ...
    const setRowIndex = React.useCallback((rowId, targetIndex) => {
        ...
        if (!node) {
            throw new Error(`MUI X Data Grid: No row with id "${rowId}" found. ` +
                'The row to reorder does not exist in the grid. ' +
                'Verify the row id is correct and the row exists.');
        }
        if (node.type !== 'leaf') {
            throw new Error('MUI X Data Grid: Row reordering does not support reordering of footer or grouping rows. ' +
