...
var useGridRows = function (apiRef, props, configuration) {
    ...
    var setRowIndex = configuration.hooks.useGridRowsOverridableMethods(apiRef, props).setRowIndex;
    ...
    var setRows = React.useCallback(function (rows) {
        ...
        if (!props.dataSource && (0, pivoting_1.gridPivotActiveSelector)(apiRef)) {
    ...
    var expandAllRows = React.useCallback(function () {
        ...
        var traverse = function (nodeId) {
            ...
            if ((node === null || node === void 0 ? void 0 : node.type) === 'group') {
                tree[nodeId] = __assign(__assign({}, node), { childrenExpanded: true });
                node.children.forEach(traverse);
            }
        };
        traverse(gridRowsUtils_1.GRID_ROOT_GROUP_ID);
        apiRef.current.setState(function (state) { return (__assign(__assign({}, state), { rows: __assign(__assign({}, state.rows), { tree: tree }) })); });
        apiRef.current.publishEvent('rowExpansionChange', tree[gridRowsUtils_1.GRID_ROOT_GROUP_ID]);
    }, [apiRef]);
    var collapseAllRows = React.useCallback(function () {
        var tree = __assign({}, (0, gridRowsSelector_1.gridRowTreeSelector)(apiRef));
        var traverse = function (nodeId) {
            var node = tree[nodeId];
            if ((node === null || node === void 0 ? void 0 : node.type) === 'group') {
                tree[nodeId] = __assign(__assign({}, node), { childrenExpanded: false });
                node.children.forEach(traverse);
            }
        };
        traverse(gridRowsUtils_1.GRID_ROOT_GROUP_ID);
