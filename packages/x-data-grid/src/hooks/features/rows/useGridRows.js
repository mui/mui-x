...
import { gridRowCountSelector, gridRowsLookupSelector, gridRowTreeSelector, gridRowGroupingNameSelector, gridRowTreeDepthsSelector, gridDataRowIdsSelector, gridRowMaximumTreeDepthSelector, gridRowGroupsToFetchSelector, gridRowNodeSelector, gridDataRowsSelector, } from './gridRowsSelector';
...
export const useGridRows = (apiRef, props, configuration) => {
    ...
    const throttledRowsChange = React.useCallback(({ cache, throttle }) => {
        const run = () => {
    ...
    const updateRows = React.useCallback((updates) => {
        if (props.signature === GridSignature.DataGrid && updates.length > 1) {
            throw new Error(`MUI X: You cannot update several rows at once in \`apiRef.current.updateRows\` on the DataGrid.
You need to upgrade to DataGridPro or DataGridPremium component to unlock this feature.`);
    ...
    const setRowChildrenExpansion = React.useCallback((id, isExpanded) => {
        ...
        const newNode = { ...currentNode, childrenExpanded: isExpanded };
        apiRef.current.setState((state) => {
            return {
                ...state,
                rows: {
    ...
    const getRowGroupChildren = React.useCallback(({ skipAutoGeneratedRows = true, groupId, applySorting, applyFiltering, directChildrenOnly = false, }) => {
        const tree = gridRowTreeSelector(apiRef);
        let children;
        if (applySorting) {
            ...
            for (let index = startIndex; index < sortedRowIds.length &&
                (directChildrenOnly
    ...
    const replaceRows = React.useCallback((firstRowToRender, newRows) => {
        if (props.signature === GridSignature.DataGrid && newRows.length > 1) {
            throw new Error(`MUI X: You cannot replace rows using \`apiRef.current.unstable_replaceRows\` on the DataGrid.
You need to upgrade to DataGridPro or DataGridPremium component to unlock this feature.`);
        }
        if (newRows.length === 0) {
            return;
        ...
        if (treeDepth > 1) {
            throw new Error('MUI X: `apiRef.current.unstable_replaceRows` is not compatible with tree data and row grouping');
        }
        const tree = { ...gridRowTreeSelector(apiRef) };
    ...
    const handleStrategyActivityChange = React.useCallback(() => {
        ...
        if (apiRef.current.getActiveStrategy(GridStrategyGroup.RowTree) !==
