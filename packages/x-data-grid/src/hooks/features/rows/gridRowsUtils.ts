...
/**
 * Get the row id from the row model.
 * If the row model has a `GRID_ID_AUTOGENERATED` property, it is returned.
 * Otherwise, the `getRowId` prop is used to get the row id.
 * If the `getRowId` prop is not provided, the `row.id` is returned.
 */
export const getRowId = (row: GridRowModel, getRowIdProp?: DataGridProcessedProps['getRowId']) => {
  if (GRID_ID_AUTOGENERATED in row) {
    return row[GRID_ID_AUTOGENERATED];
  }
  return getRowIdProp ? getRowIdProp(row) : row.id;
};
