...
export const getReplaceRow = (update: GridRowModelReplace): GridRowModel => {
  ...
  if (row == null) {
    throw new Error(
      `MUI X Data Grid: A row update with \`_action: 'replace'\` was provided without a \`row\` property.
 ...
 */
export const mergeRowUpdate = (
  oldRow: GridRowModel,
  partialRow: GridRowModelUpdate,
): GridRowModel => {
...
export const warnIfReplaceLosesPrototype = (
  ...
  // The message must stay free of row-specific details: `warn()` caches by message, so
...
export const updateCacheWithNewRows = ({
  ...
  const replaceIds = new Set<GridRowId>();
