...
export function throwMissingRowError(id: GridRowId): never {
  // Construct via `new Error(...)` so the babel error-minifier replaces the
  // message in production builds, then re-tag the instance so callers can
  // still distinguish this case via `error.name` or `instanceof`.
  const error = new Error(`MUI X: No row with id #${id} found`);
  error.name = 'MissingRowIdError';
  Object.setPrototypeOf(error, MissingRowIdError.prototype);
  throw error;
}
