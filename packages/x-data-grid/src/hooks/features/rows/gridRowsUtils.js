...
export function checkGridRowIdIsValid(id, row, detailErrorMessage = 'A row was provided without id in the rows prop:') {
    if (id == null) {
        throw new Error(`MUI X: The Data Grid component requires all rows to have a unique \`id\` property.
...
export const createRowsInternalCache = ({ rows, getRowId, loading, rowCount, }) => {
    const updates = {
        type: 'full',
        rows: [],
    };
...
export const getTopLevelRowCount = ({ tree, rowCountProp = 0, }) => {
    const rootGroupNode = tree[GRID_ROOT_GROUP_ID];
    return Math.max(rowCountProp, rootGroupNode.children.length + (rootGroupNode.footerId == null ? 0 : 1));
};
export const getRowsStateFromCache = ({ apiRef, rowCountProp = 0, loadingProp, previousTree, previousTreeDepths, previousGroupsToFetch, }) => {
...
export const isAutogeneratedRowNode = (rowNode) => rowNode.type === 'skeletonRow' ||
    rowNode.type === 'footer' ||
...
export const updateCacheWithNewRows = ({ previousCache, getRowId, updates, groupKeys, }) => {
    ...
    const alreadyAppliedActionsToRemove = { insert: {}, modify: {}, remove: {} };
