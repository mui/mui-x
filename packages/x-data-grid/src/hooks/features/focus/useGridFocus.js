...
export const focusStateInitializer = (state) => ({
    ...state,
...
export const useGridFocus = (apiRef, props) => {
    ...
    const setCellFocus = React.useCallback((id, field) => {
        ...
        if (focusedCell?.id === id && focusedCell?.field === field) {
            ...
            const allowTakingFocus = !activeElement ||
    ...
    const handleCellDoubleClick = React.useCallback(({ id, field }) => {
    ...
    const handleColumnHeaderFocus = React.useCallback(({ field }, event) => {
        if (event.target !== event.currentTarget) {
            return;
        }
        apiRef.current.setColumnHeaderFocus(field, event);
    }, [apiRef]);
    const handleColumnGroupHeaderFocus = React.useCallback(({ fields, depth }, event) => {
        if (event.target !== event.currentTarget) {
            return;
        }
