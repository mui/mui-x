...
export type GridValueGetter<
  ...
  F = V,
  TValue = never,
> = (value: TValue, row: R, column: GridColDef<R, V, F>, apiRef: ApiRef<GridApiCommunity>) => V;
...
export type GridValueFormatter<
  ...
  F = V,
  TValue = never,
> = (value: TValue, row: R, column: GridColDef<R, V, F>, apiRef: ApiRef<GridApiCommunity>) => F;
...
export interface GridBaseColDef<R extends GridValidRowModel = GridValidRowModel, V = any, F = V> {
   ...
   * @param {ApiRef<GridApiCommunity>} apiRef Deprecated: The API of the grid.
