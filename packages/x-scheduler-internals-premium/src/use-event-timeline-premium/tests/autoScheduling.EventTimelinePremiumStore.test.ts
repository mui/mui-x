...
describe('Auto-scheduling - EventTimelinePremiumStore', () => {
  ...
  describe('batches with a deletion', () => {
  ...
  describe('prop: dataSource', () => {
    it('should include the cascaded updates in the same persistEvents batch', async () => {
      ...
      try {
        ...
        expect(timestampOf(persistedA.start)).to.equal(
          adapter.getTime(date('2025-07-03T11:00:00Z')),
        );
        ...
        expect(timestampOf(persistedB.start)).to.equal(
          adapter.getTime(date('2025-07-03T12:00:00Z')),
        );
      } finally {
        vi.useRealTimers();
      }
