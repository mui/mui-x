...
/**
 * Minimal, spec-compatible `DisposableStack` fallback. Implements the subset of
 * the explicit resource management proposal MUI X relies on (`use`, `adopt`,
 * `defer`, `move`, `dispose`, `disposed`) without pulling in the multi-kilobyte
 * `core-js` ponyfill. Native `DisposableStack` is preferred when present.
 */
...
/**
 * Minimal, spec-compatible `AsyncDisposableStack` fallback. See
 * {@link FallbackDisposableStack}; disposers may return promises and are
 * awaited sequentially in LIFO order.
 */
