...
export function showExpiredAnnualLicenseKeyError({ plan }: { plan: string }) {
  showError([
    ...
    '',
    `Your annual license key to use MUI X ${plan}'s on non-production environments is expired. If you are seeing this development console message, it's likely that you are about to breach the license terms by making direct or indirect changes to the frontend of an app that render a MUI X ${plan} component (more details: https://mui.com/r/x-license-annual).`,
    '',
    'To solve the issue you can either:',
    '',
    '- Renew your annual license https://mui.com/r/x-get-license and use the new key',
