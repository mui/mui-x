...
import type { SeriesId, CommonHighlightScope } from '@mui/x-charts/internals';
...
export interface TreemapTilingOptions {
  ...
  paddingInner?: number;
  /**
   * The inset between a group tile's edge and its children, in pixels.
   * @default 2
   */
  paddingOuter?: number;
  /**
   * The inset at the top of a group tile, reserving a header band for its label, in pixels.
   */
  paddingTop?: number;
}
