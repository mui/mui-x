...
export default function getItemAtPosition(
  ...
  // In 'leaf' mode only leaf tiles are rendered, so gaps over a group hit nothing.
  const renderMode = series.series[seriesId].nodeOptions?.renderMode ?? 'all';
  if (renderMode === 'leaf' && node.height !== 0) {
    return undefined;
  }
