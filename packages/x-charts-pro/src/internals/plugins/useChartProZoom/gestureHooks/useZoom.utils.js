 ...
 */
function getHorizontalCenterRatio(point, area, reverse) {
    var left = area.left, width = area.width;
    var ratio = (point.x - left) / width;
    return reverse ? 1 - ratio : ratio;
}
/**
 * Get the ratio of the point in the vertical center of the area.
 */
function getVerticalCenterRatio(point, area, reverse) {
    var top = area.top, height = area.height;
    var ratio = (top - point.y) / height + 1;
    return reverse ? 1 - ratio : ratio;
}
/**
 * Translate the zoom data by a given movement.
 */
function translateZoom(initialZoomData, movement, drawingArea, optionsLookup, filterMode) {
    if (filterMode === void 0) { filterMode = 'xy'; }
    return initialZoomData.map(function (zoom) {
        ...
        var rawDisplacement = options.axisDirection === 'x' ? movement.x : movement.y;
        var displacement = options.reverse ? -rawDisplacement : rawDisplacement;
