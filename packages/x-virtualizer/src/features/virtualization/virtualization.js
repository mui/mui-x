...
import { ScrollPosition, ScrollDirection, } from '../../models';
...
function useVirtualization(store, params, api) {
    ...
    const updateRenderContext = React.useCallback((nextRenderContext) => {
        ...
        const didRowsIntervalChange = nextRenderContext.firstRowIndex !== previousRowContext.current.firstRowIndex ||
    ...
    const forceUpdateRenderContext = () => {
        ...
        if (!Dimensions.selectors.dimensions(store.state).isReady &&
    ...
    const getRows = (rowParams = {}, unstable_rowTree) => {
        ...
        const isLastSection = (!hasBottomPinnedRows && rowParams.position === undefined) ||
...
export function computeOffsetLeft(columnPositions, renderContext, pinnedLeftLength) {
    const left = (columnPositions[renderContext.firstColumnIndex] ?? 0) -
