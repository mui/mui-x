...
export default function transformer(file: JsCodeShiftFileInfo, api: JsCodeShiftAPI, options: any) {
  ...
  Object.entries(removedTypeLocalNames).forEach(([localName, originalName]) => {
    ...
    root.find(j.TSTypeReference).forEach((path) => {
      if (path.node.typeName.type === 'Identifier' && path.node.typeName.name === localName) {
        ...
        j(path).replaceWith(newTypeReference);
      }
    });
  });
  ...
  if (genericTypesToAdd.length > 0) {
    ...
    if (internalsImport.size() > 0) {
      ...
      const chartImports = root.find(j.ImportDeclaration).filter((path) => {
        const source = path.node.source.value?.toString() ?? '';
