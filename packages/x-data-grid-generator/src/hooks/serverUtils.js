...
const getAvailableAggregationFunctions = (columnType) => {
    ...
    Object.keys(GRID_AGGREGATION_FUNCTIONS).forEach((functionName) => {
        const columnTypes = GRID_AGGREGATION_FUNCTIONS[functionName]
            .columnTypes;
...
export const disableDelay = typeof __DISABLE_CHANCE_RANDOM__ !== 'undefined' && __DISABLE_CHANCE_RANDOM__;
...
const getRowComparator = (sortModel, aggregationModel, columnsWithDefaultColDef) => {
    ...
    const sortOperators = sortModel.map((sortItem) => {
        ...
        const colDef = columnsWithDefaultColDef.find(({ field }) => field === sortItem.field);
        return {
            ...sortItem,
    ...
    const comparator = (row1, row2) => sortOperators.reduce((acc, { valueGetter, sort, sortComparator }) => {
        if (acc !== 0) {
            return acc;
        }
...
const getFilteredRows = (rows, filterModel, columnsWithDefaultColDef) => {
    ...
    const valueGetters = filterModel.items.map(({ field }) => simplifiedValueGetter(field, columnsWithDefaultColDef.find((column) => column.field === field)));
    const filterFunctions = filterModel.items.map((filterItem) => {
        ...
        const filterOperator = colDef.filterOperators.find(({ value }) => operator === value);
...
export const processRowGroupingRows = (rows, queryOptions, serverOptions, columnsWithDefaultColDef) => {
    ...
    if (queryOptions.groupFields == null) {
        throw new Error('MUI X: serverOptions.groupFields must be defined to compute row grouping data');
    }
    const delay = randomInt(minDelay, maxDelay);
    ...
    let childRowsWithDescendantCounts = childRows.map((row) => {
        const descendants = findTreeDataRowChildren(filteredRows, row[pathKey], pathKey, -1, ({ id }) => typeof id !== 'string' || !id.startsWith('auto-generated-parent-'));
        const descendantCount = descendants.length;
        if (descendantCount > 0 && queryOptions.aggregationModel) {
            // Parent row, compute aggregation
            return {
...
export const processPivotingRows = (rows, queryOptions, serverOptions, columnsWithDefaultColDef) => {
    ...
    let childRowsWithDescendantCounts = childRows.map((row) => {
        const descendants = findTreeDataRowChildren(filteredRowsWithGroups, row.path, 'path', -1, ({ id }) => typeof id !== 'string' || !id.startsWith('auto-generated-parent-'));
        const descendantCount = descendants.length;
        if (descendantCount > 0) {
            // Parent row, compute aggregation for both regular aggregation model and pivot values
            const regularAggregation = applyAggregation(queryOptions.pivotModel.values.map((value) => ({ [value.field]: value.aggFunc })), columnsWithDefaultColDef, descendants, row.id);
    ...
    if (visibleValues.length > 0) {
        const regularAggregation = applyAggregation(visibleValues.map((value) => ({ [value.field]: value.aggFunc })), columnsWithDefaultColDef, filteredRowsWithGroups);
