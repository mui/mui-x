...
var GStyled = (0, styles_1.styled)('g')(function (_a) {
    var theme = _a.theme;
    return ({
        '& text': {
...
var GaugeContainer = React.forwardRef(function GaugeContainer(props, ref) {
    ...
    return (<ChartProvider_1.ChartProvider pluginParams={{
            width: inWidth,
            height: inHeight,
            margin: (0, defaultizeMargin_1.defaultizeMargin)(margin, { left: 10, right: 10, top: 10, bottom: 10 }),
        }} plugins={[]}>
      <GaugeProvider_1.GaugeProvider value={value} valueMin={valueMin} valueMax={valueMax} startAngle={startAngle} endAngle={endAngle} outerRadius={outerRadius} innerRadius={innerRadius} cornerRadius={cornerRadius} cx={cx} cy={cy}>
        <ChartsSurface_1.ChartsSurface title={title} desc={desc} role="meter" aria-valuenow={value === null ? undefined : value} aria-valuemin={valueMin} aria-valuemax={valueMax} {...other} ref={ref}>
          <GStyled aria-hidden="true">{children}</GStyled>
        </ChartsSurface_1.ChartsSurface>
      </GaugeProvider_1.GaugeProvider>
