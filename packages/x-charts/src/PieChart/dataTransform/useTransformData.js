...
function useTransformData(series) {
    ...
    var dataWithHighlight = React.useMemo(function () {
        return data.map(function (item, itemIndex) {
            ...
            var isFocused = isItemFocused({ seriesType: 'pie', seriesId: seriesId, dataIndex: itemIndex });
            ...
            return __assign(__assign(__assign({}, item), attributesOverride), { dataIndex: itemIndex, isFaded: isFaded, isHighlighted: isHighlighted, isFocused: isFocused, paddingAngle: paddingAngle, innerRadius: innerRadius, outerRadius: outerRadius, cornerRadius: cornerRadius, arcLabelRadius: arcLabelRadius });
        });
    }, [
        baseCornerRadius,
        baseInnerRadius,
