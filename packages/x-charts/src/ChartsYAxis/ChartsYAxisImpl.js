...
function ChartsYAxisImpl(_a) {
    ...
    var yScale = axis.scale, tickNumber = axis.tickNumber, reverse = axis.reverse, settings = __rest(axis, ["scale", "tickNumber", "reverse"]);
    ...
    var axisLabelHeight = label == null ? 0 : (0, domUtils_1.measureText)(label, axisLabelProps.style).height;
    ...
    if (!skipTickRendering) {
        children =
            'groups' in axis && Array.isArray(axis.groups) ? (<ChartsGroupedYAxisTicks_1.ChartsGroupedYAxisTicks {...inProps}/>) : (<ChartsSingleYAxisTicks_1.ChartsSingleYAxisTicks {...inProps} axisLabelHeight={axisLabelHeight}/>);
    }
    return (<YAxisRoot transform={"translate(".concat(position === 'right' ? left + width + offset : left - offset, ", 0)")} className={classes.root} sx={sx}>
