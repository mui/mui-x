  ...
  return Object.keys(series)
    ...
    .reduce(
      (acc: ExtremumGetterResult, seriesId) => {
        ...
        const getValues: GetValuesTypes = (d: [number, number]) => {
          if (isArea && axis.scaleType !== 'log' && series[seriesId].baseline === undefined) {
            return d;
          }

          if (isArea && typeof series[seriesId].baseline === 'number') {
            return [series[seriesId].baseline, d[1]];
          }
