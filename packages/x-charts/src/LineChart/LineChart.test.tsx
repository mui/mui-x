...
describe('<LineChart />', () => {
  ...
  describe('experimentalFeatures.enablePositionBasedPointerInteraction', () => {
    // These tests verify the opt-in pointer-based interaction detection for line/area series.
    ...
    const areaConfig = {
      ...
      skipAnimation: true,
      hideLegend: true,
      experimentalFeatures: { enablePositionBasedPointerInteraction: true } as const,
    };
    ...
    it.skipIf(isJSDOM)(
      'should not activate pointer interaction when experimentalFeatures is not set',
      async () => {
        const onHighlightChange = vi.fn();
        const { container, user } = render(
          <div style={{ width: 400, height: 400 }}>
        ...
        // Move pointer to a position between lines but not over any SVG element.
        // Without experimental feature, the global pointer detection is inactive,
        // so no highlight should fire from pointer position alone.
        await user.pointer({
          target: layerContainer,
