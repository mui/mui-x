...
const getNextFocusedItem: GetNextFocusedItem<'line'> = (currentItem, event, state) => {
  switch (event.key) {
    case 'ArrowRight':
      return getNextIndexFocusedItem<'line', 'bar' | 'line' | 'scatter'>(
        currentItem,
        outSeriesTypes,
        state,
      );
    case 'ArrowLeft':
      return getPreviousIndexFocusedItem<'line', 'bar' | 'line' | 'scatter'>(
        currentItem,
        outSeriesTypes,
        state,
      );
    case 'ArrowDown':
      return getPreviousSeriesFocusedItem<'line', 'bar' | 'line' | 'scatter'>(
        currentItem,
        outSeriesTypes,
        state,
      );
    case 'ArrowUp':
      return getNextSeriesFocusedItem<'line', 'bar' | 'line' | 'scatter'>(
        currentItem,
        outSeriesTypes,
        state,
      );
