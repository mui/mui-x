...
function offsetDiverging(series, order) {
    ...
    var numericOrder = order;
    var pointCount = series[numericOrder[0]].length;
    for (var pointIndex = 0; pointIndex < pointCount; pointIndex += 1) {
        var positiveSum = 0;
        var negativeSum = 0;
        for (var seriesIndex = 0; seriesIndex < seriesCount; seriesIndex += 1) {
            var currentSeries = series[numericOrder[seriesIndex]];
            var dataPoint = currentSeries[pointIndex];
            var difference = dataPoint[1] - dataPoint[0];
            if (difference > 0) {
