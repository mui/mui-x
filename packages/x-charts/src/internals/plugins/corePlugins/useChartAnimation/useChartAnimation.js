...
var useChartAnimation = function (_a) {
    ...
    React.useEffect(function () {
        store.update(function (prevState) {
            return __assign(__assign({}, prevState), { animation: __assign(__assign({}, prevState.animation), { skip: params.skipAnimation }) });
        });
    }, [store, params.skipAnimation]);
    var disableAnimation = React.useCallback(function () {
        var disableCalled = false;
        store.update(function (prevState) { return (__assign(__assign({}, prevState), { animation: __assign(__assign({}, prevState.animation), { skipAnimationRequests: prevState.animation.skipAnimationRequests + 1 }) })); });
        return function () {
            if (disableCalled) {
                return;
            }
