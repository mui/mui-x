...
function computeAxisValue(_a) {
    ...
    allAxis.forEach(function (eachAxis, axisIndex) {
        ...
        if ((0, axis_1.isBandScaleConfig)(axis)) {
            ...
            completeAxis[axis.id] = __assign(__assign({ offset: 0, categoryGapRatio: categoryGapRatio, barGapRatio: barGapRatio, triggerTooltip: triggerTooltip }, axis), { data: data, scale: (0, d3_scale_1.scaleBand)(axis.data, range)
            ...
            if ((0, dateHelpers_1.isDateData)(axis.data)) {
                var dateFormatter = (0, dateHelpers_1.createDateFormatter)(axis, range);
        ...
        if ((0, axis_1.isPointScaleConfig)(axis)) {
            completeAxis[axis.id] = __assign(__assign({ offset: 0, triggerTooltip: triggerTooltip }, axis), { data: data, scale: (0, d3_scale_1.scalePoint)(axis.data, range), tickNumber: axis.data.length, colorScale: axis.colorMap &&
                    (axis.colorMap.type === 'ordinal'
                        ? (0, colorScale_1.getOrdinalColorScale)(__assign({ values: axis.data }, axis.colorMap))
                        : (0, colorScale_1.getColorScale)(axis.colorMap)) });
            if ((0, dateHelpers_1.isDateData)(axis.data)) {
