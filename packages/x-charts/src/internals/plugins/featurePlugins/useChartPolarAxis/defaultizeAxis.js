...
function defaultizeAxis(inAxis, dataset, axisName) {
    ...
    return inputAxes.map(function (axisConfig, index) {
        ...
        if (dataset === undefined) {
            throw new Error("MUI X Charts: The ".concat(axisName, "-axis uses `dataKey` but no `dataset` is provided. ") +
                'When using dataKey, a dataset must be provided to retrieve the axis data. ' +
                "Either provide a dataset prop or use the data property directly on the ".concat(axisName, "-axis."));
        }
