...
export const useChartCartesianAxis: ChartPlugin<UseChartCartesianAxisSignature<any>> = ({
  ...
  const { xAxis, yAxis, dataset, onXAxisInteraction, onYAxisInteraction } = params;
  ...
  useAssertModelConsistency({
    ...
    componentName: 'Chart',
    propName: 'xAxisHighlight',
    controlled: params.xAxisHighlight,
    defaultValue: undefined,
  });

  useEnhancedEffect(() => {
    if (params.xAxisHighlight !== undefined) {
      store.update((prevState) => {
        if (prevState.controlledCartesianAxisHighlight?.x === params.xAxisHighlight) {
          return prevState;
        }
        ...
        return {
          ...
          controlledCartesianAxisHighlight: {
            ...prevState.controlledCartesianAxisHighlight,
            x: params.xAxisHighlight,
          },
        };
      });
    }
  }, [store, params.xAxisHighlight]);

  useAssertModelConsistency({
    warningPrefix: 'MUI X Charts',
    componentName: 'Chart',
    propName: 'yAxisHighlight',
    controlled: params.yAxisHighlight,
    defaultValue: undefined,
  });

  useEnhancedEffect(() => {
    if (params.yAxisHighlight !== undefined) {
      store.update((prevState) => {
        if (prevState.controlledCartesianAxisHighlight?.y === params.yAxisHighlight) {
          return prevState;
        }
  ...
  React.useEffect(() => {
    ...
    const handleMove = (event: MouseEvent | TouchEvent) => {
      ...
      if (!instance.isPointInside(svgPoint, { targetElement: event.target as SVGElement })) {
        ...
        onYAxisInteraction?.(null);
        return;
      }

      if (onXAxisInteraction) {
        ...
        for (const axisId of xAxes.axisIds) {
          const prevXAxisIndex = selectorChartsInteractionXAxisIndex(store.value, axisId);
          const prevXAxisValue = selectorChartsInteractionXAxisValue(store.value, axisId);
          const nextXAxisIndex = selectorChartsInteractionXAxisIndex(newStore, axisId);
          const nextXAxisValue = selectorChartsInteractionXAxisValue(newStore, axisId);

          if (prevXAxisIndex !== nextXAxisIndex || prevXAxisValue !== nextXAxisValue) {
            isSame = false;
          }

          newXAxisHighlight.push({ axisId, dataIndex: nextXAxisIndex, value: nextXAxisValue });
        }

        if (!isSame) {
          onXAxisInteraction(newXAxisHighlight);
        }
      }

      if (onYAxisInteraction) {
        ...
        for (const axisId of yAxes.axisIds) {
          const prevYAxisIndex = selectorChartsInteractionYAxisIndex(store.value, axisId);
          const prevYAxisValue = selectorChartsInteractionYAxisValue(store.value, axisId);
          const nextYAxisIndex = selectorChartsInteractionYAxisIndex(newStore, axisId);
          const nextYAxisValue = selectorChartsInteractionYAxisValue(newStore, axisId);

          if (prevYAxisIndex !== nextYAxisIndex || prevYAxisValue !== nextYAxisValue) {
            isSame = false;
          }

          newYAxisHighlight.push({ axisId, dataIndex: nextYAxisIndex, value: nextYAxisValue });
        }

        if (!isSame) {
          onYAxisInteraction(newYAxisHighlight);
        }
      }
...
useChartCartesianAxis.params = {
  ...
  onXAxisInteraction: true,
  onYAxisInteraction: true,
  xAxisHighlight: true,
  yAxisHighlight: true,
};
