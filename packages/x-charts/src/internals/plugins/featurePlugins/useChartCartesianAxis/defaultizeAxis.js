...
function defaultizeXAxis(inAxes, dataset, axesGap) {
    ...
    var parsedAxes = inputAxes.map(function (axisConfig, index) {
        ...
        if (dataset === undefined) {
            throw new Error('MUI X Charts: The x-axis uses `dataKey` but no `dataset` is provided. ' +
...
function defaultizeYAxis(inAxes, dataset, axesGap) {
    ...
    var parsedAxes = inputAxes.map(function (axisConfig, index) {
        ...
        if (dataset === undefined) {
            throw new Error('MUI X Charts: The y-axis uses `dataKey` but no `dataset` is provided. ' +
                'When using dataKey, a dataset must be provided to retrieve the axis data. ' +
