...
describe('createContinuousScaleGetAxisFilter', function () {
    describe('linear scale', function () {
        it('should filter values within zoom range', function () {
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('linear', [0, 100], 20, 80, 'x', undefined);
        ...
        it('should handle edge values at zoom boundaries', function () {
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('linear', [0, 100], 25, 75, 'x', undefined);
        ...
        it('should work with y direction', function () {
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('linear', [0, 100], 30, 70, 'y', undefined);
    ...
    describe('time scale', function () {
        it('should filter date values correctly', function () {
            ...
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('time', [startDate, endDate], 25, 75, 'x', undefined);
    ...
    describe('fallback to axis data', function () {
        it('should use axis data when value direction is missing', function () {
            ...
            var extrema = [10, 50];
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('linear', extrema, 40, 80, 'x', axisData);
        ...
        it('should handle out of range index gracefully', function () {
            ...
            var extrema = [10, 30];
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('linear', extrema, 20, 80, 'x', axisData);
    ...
    describe('edge cases', function () {
        it('should handle full zoom range (0-100)', function () {
            var extrema = [-50, 50];
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('linear', extrema, 0, 100, 'x', undefined);
        ...
        it('should handle very narrow zoom range', function () {
            var extrema = [0, 100];
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('linear', extrema, 49, 51, 'x', undefined);
            expect(filter({ x: 48, y: null }, 0)).toBe(false);
            expect(filter({ x: 50, y: null }, 0)).toBe(true);
            expect(filter({ x: 52, y: null }, 0)).toBe(false);
        });
        it('should handle undefined scale type (defaults to linear)', function () {
            var extrema = [0, 100];
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)(undefined, extrema, 25, 75, 'x', undefined);
        ...
        it('should handle negative extrema values', function () {
            var extrema = [-50, 50];
            var filter = (0, createAxisFilterMapper_1.createContinuousScaleGetAxisFilter)('linear', extrema, 25, 75, 'x', undefined);
            expect(filter({ x: 0, y: null }, 0)).toBe(true);
