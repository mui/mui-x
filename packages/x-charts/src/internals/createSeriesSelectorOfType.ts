...
import { createChartSelector } from './plugins/utils/selectors';
...
export function createSeriesSelectorsOfType<T extends keyof ChartsSeriesConfig>(seriesType: T) {
  const selectorSeriesWithIds = createChartSelector(
  ...
  return (ids?: SeriesId | SeriesId[]) => {
    ...
    return useStore(store, selectorSeriesWithIds, [ids], fastArrayCompare);
  };
}

export function createAllSeriesSelectorOfType<T extends keyof ChartsSeriesConfig>(seriesType: T) {
  const selectorSeries = createChartSelector(
  ...
  return () => {
    ...
    return useStore(store, selectorSeries);
  };
}
