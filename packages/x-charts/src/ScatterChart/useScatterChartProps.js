...
var useScatterChartProps = function (props) {
    var _a, _b, _c, _d, _e;
    var xAxis = props.xAxis, yAxis = props.yAxis, zAxis = props.zAxis, series = props.series, axisHighlight = props.axisHighlight, hitAreaRadius = props.hitAreaRadius, disableHitArea = props.disableHitArea, hideLegend = props.hideLegend, width = props.width, height = props.height, margin = props.margin, colors = props.colors, sx = props.sx, grid = props.grid, onItemClick = props.onItemClick, children = props.children, slots = props.slots, slotProps = props.slotProps, loading = props.loading, highlightedItem = props.highlightedItem, onHighlightChange = props.onHighlightChange, className = props.className, showToolbar = props.showToolbar, renderer = props.renderer, brushConfig = props.brushConfig, other = __rest(props, ["xAxis", "yAxis", "zAxis", "series", "axisHighlight", "hitAreaRadius", "disableHitArea", "hideLegend", "width", "height", "margin", "colors", "sx", "grid", "onItemClick", "children", "slots", "slotProps", "loading", "highlightedItem", "onHighlightChange", "className", "showToolbar", "renderer", "brushConfig"]);
    var seriesWithDefault = React.useMemo(function () { return series.map(function (s) { return (__assign({ type: 'scatter' }, s)); }); }, [series]);
    var resolvedDisableHitArea = disableHitArea;
    var useVoronoiOnItemClick = resolvedDisableHitArea !== true || renderer === 'svg-batch';
    var chartsContainerProps = __assign(__assign({}, other), { series: seriesWithDefault, width: width, height: height, margin: margin, colors: colors, xAxis: xAxis, yAxis: yAxis, zAxis: zAxis, highlightedItem: highlightedItem, onHighlightChange: onHighlightChange, disableHitArea: disableHitArea, hitAreaRadius: hitAreaRadius, onItemClick: useVoronoiOnItemClick
            ? onItemClick
            : undefined, plugins: ScatterChart_plugins_1.SCATTER_CHART_PLUGINS, slots: slots, slotProps: slotProps, brushConfig: brushConfig });
    var chartsAxisProps = {
        slots: slots,
        slotProps: slotProps,
    };
