...
function RadarAxis(props) {
    ...
    return (<g className={classes.root}>
      <path d={"M ".concat(center.x, " ").concat(center.y, " L ").concat(labels[labels.length - 1].x, " ").concat(labels[labels.length - 1].y)} stroke={((_a = theme.vars) !== null && _a !== void 0 ? _a : theme).palette.text.primary} strokeOpacity={0.3} className={classes.line}/>
      {labels.map(function (_a) {
            var _b;
            var x = _a.x, y = _a.y, formattedValue = _a.formattedValue;
            return (<text key={formattedValue} fontSize={12} fill={((_b = theme.vars) !== null && _b !== void 0 ? _b : theme).palette.text.primary} stroke="none" className={classes.label} {...(0, RadarAxis_utils_1.getLabelAttributes)({ labelOrientation: labelOrientation, x: x, y: y, angle: angle, textAnchor: textAnchor, dominantBaseline: dominantBaseline })}>
          {formattedValue}
        </text>);
        })}
    </g>);
...
RadarAxis.propTypes = {
     ...
     */
    dominantBaseline: prop_types_1.default.oneOfType([prop_types_1.default.func, prop_types_1.default.string]),
    /**
     ...
     */
    textAnchor: prop_types_1.default.oneOfType([prop_types_1.default.func, prop_types_1.default.string]),
};
