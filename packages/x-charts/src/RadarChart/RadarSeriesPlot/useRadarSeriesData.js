...
function useRadarSeriesData(querySeriesId) {
    ...
    return radarSeries.map(function (series) {
        ...
        var isSeriesHighlighted = isItemHighlighted({ type: 'radar', seriesId: seriesId });
        var isSeriesFaded = !isSeriesHighlighted && isItemFaded({ type: 'radar', seriesId: seriesId });
        ...
        return __assign(__assign({}, series), { seriesId: series.id, isSeriesHighlighted: isSeriesHighlighted, isSeriesFaded: isSeriesFaded, points: series.data.map(function (value, dataIndex) {
                var highlighted = isItemHighlighted({ type: 'radar', seriesId: seriesId, dataIndex: dataIndex });
                var faded = !highlighted && isItemFaded({ type: 'radar', seriesId: seriesId, dataIndex: dataIndex });
