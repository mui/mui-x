...
function RadarSeriesMarks(props) {
    ...
    return (<React.Fragment>
      {seriesCoordinates === null || seriesCoordinates === void 0 ? void 0 : seriesCoordinates.map(function (_a) {
            var id = _a.seriesId, points = _a.points, color = _a.color, hideMark = _a.hideMark, fillArea = _a.fillArea;
            if (hideMark) {
                return null;
            }
            return (<g key={id}>
            {points.map(function (point, index) { return (<circle key={index} {...getCircleProps({
                    seriesId: id,
                    point: point,
                    color: color,
                    fillArea: fillArea,
                    ...
                    }} cursor={onItemClick ? 'pointer' : 'unset'} {...other}/>); })}
          </g>);
        })}
    </React.Fragment>);
}
