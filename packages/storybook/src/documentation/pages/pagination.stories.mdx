...
Please find below an example of a grid with pagination using the options properties described above.

<SimplePaginationDemo />

## Auto Pagination

To complement the pagination feature, we added the property `paginationAutoPageSize` in options, that auto-scale the `pageSize` with the container size to the max number of rows to avoid rendering a vertical scroll bar.
...
Our default pagination component is our Material-UI `TablePagination` available [here](https://material-ui.com/api/table-pagination/#tablepagination-api).
