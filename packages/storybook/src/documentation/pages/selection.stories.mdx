...
By default, the grid allows multiple selection. However, this setting can be turned off using the property `disableMultipleSelection` of the `options`React prop.
...
- `onSelectionChange` - Event emitted when one or multiple rows had a selection state change.
