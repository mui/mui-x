...
import {ColumnTypesDemo, HideColumnDemo, ValueGetterDemo, FormattingDemo, StyleHeaderDemo, StyleCellDemo} from './demos/columns.demo'
...
### Header
`ColDef` has an attribute called `headerClass` that let you inject some css classes in your column header.
You can also specify where to align the header text using `headerAlign`, default value is `left`
...
### Cell
`ColDef` let you style cells with different attributes.
- `align` to align cell content, `'left' | 'right' | 'center'`
- `cellClass` of type `string | string[]`, allows to apply css classes on every cells. It can also be of type `CellClassFn` which is a function that let you apply css classes dynamically.
...
## Options
We added more options in `ColDef` which you can find the full definition in the Api page (TODO add link here)

   - `disableClickEventBubbling`: allows to disable the default click behaviour of the grid. If you render a clickable component in a column such as a link or a button, you can use this options to disable the row selection on click.
