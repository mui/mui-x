...
export const testTextFieldValidation: DescribeValidationTestSuite = (ElementToTest, getOptions) => {
  ...
  describe.skipIf(!['picker', 'field'].includes(componentFamily))('text field:', () => {
    it.skipIf(['picker', 'field'].includes(componentFamily) && !withDate)(
      ...
      () => {
        ...
        if (withDate) {
          ...
          expect(onErrorMock.mock.lastCall?.[0]).to.equal('shouldDisableDate');

          setProps({ value: adapterToUse.date('2018-03-09') });

          expect(onErrorMock.mock.calls.length).to.equal(2);
          expect(onErrorMock.mock.lastCall?.[0]).to.equal(null);
    ...
    it.skipIf(!withDate)('should apply shouldDisableYear', () => {
      ...
      expect(onErrorMock.mock.calls.length).to.equal(1);
      expect(onErrorMock.mock.lastCall?.[0]).to.equal('shouldDisableYear');
      expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
    ...
    it.skipIf(!withDate)('should apply shouldDisableMonth', () => {
      ...
      expect(onErrorMock.mock.calls.length).to.equal(1);
      expect(onErrorMock.mock.lastCall?.[0]).to.equal('shouldDisableMonth');
      expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
    ...
    it.skipIf(!withTime)('should apply shouldDisableTime', () => {
      ...
      expect(onErrorMock.mock.calls.length).to.equal(1);
      expect(onErrorMock.mock.lastCall?.[0]).to.equal('shouldDisableTime-hours');
      expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
      ...
      expect(onErrorMock.mock.lastCall?.[0]).to.equal('shouldDisableTime-minutes');
      expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
      ...
      expect(onErrorMock.mock.calls.length).to.equal(5);
      expect(onErrorMock.mock.lastCall?.[0]).to.equal('shouldDisableTime-seconds');
      expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
    });
    ...
    describe.skipIf(!withDate)('with fake timers', () => {
      ...
      it('should apply disablePast', () => {
        ...
        expect(onErrorMock.mock.calls.length).to.equal(1);
        expect(onErrorMock.mock.lastCall?.[0]).to.equal('disablePast');
        expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
    ...
    it.skipIf(!withDate)('should apply disableFuture', () => {
      ...
      setProps({ value: tomorrow });
      expect(onErrorMock.mock.calls.length).to.equal(1);
      expect(onErrorMock.mock.lastCall?.[0]).to.equal('disableFuture');
      expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
    ...
    it.skipIf(['picker', 'field'].includes(componentFamily) && !withDate)(
      ...
      () => {
        ...
        if (withDate) {
          expect(onErrorMock.mock.calls.length).to.equal(1);
          expect(onErrorMock.mock.lastCall?.[0]).to.equal('minDate');
          expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
    ...
    it.skipIf(['picker', 'field'].includes(componentFamily) && !withDate)(
      ...
      () => {
        ...
        if (withDate) {
          expect(onErrorMock.mock.calls.length).to.equal(1);
          expect(onErrorMock.mock.lastCall?.[0]).to.equal('maxDate');
          expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
    ...
    it.skipIf(['picker', 'field'].includes(componentFamily) && !withTime)(
      ...
      () => {
        ...
        if (withTime) {
          expect(onErrorMock.mock.calls.length).to.equal(1);
          expect(onErrorMock.mock.lastCall?.[0]).to.equal('minTime');
          expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
    ...
    it.skipIf(['picker', 'field'].includes(componentFamily) && !withTime)(
      ...
      () => {
        ...
        if (withTime) {
          ...
          expect(onErrorMock.mock.calls.length).to.equal(1);
          expect(onErrorMock.mock.lastCall?.[0]).to.equal('maxTime');
          expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
        } else {
          expect(onErrorMock.mock.calls.length).to.equal(0);
          expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'false');
    ...
    it.skipIf(['picker', 'field'].includes(componentFamily) && !withTime)(
      ...
      () => {
        ...
        if (withTime) {
          expect(onErrorMock.mock.calls.length).to.equal(1);
          expect(onErrorMock.mock.lastCall?.[0]).to.equal('minutesStep');
          expect(getFieldInputRoot()).to.have.attribute('aria-invalid', 'true');
