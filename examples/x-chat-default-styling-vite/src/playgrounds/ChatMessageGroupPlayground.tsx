...
export function ChatMessageGroupPlayground() {
  ...
  const codeExample = `import { ChatMessageGroup, ChatMessage } from '@mui/x-chat';

// ChatMessageGroup clusters consecutive messages
{messages.map((msg) => (
  <ChatMessageGroup key={msg.id} messageId={msg.id}>
  ...
  return (
    <PlaygroundCard
      ...
      controls={
        <React.Fragment>
          <NumberControl label="message count" value={count} min={1} max={5} onChange={setCount} />
