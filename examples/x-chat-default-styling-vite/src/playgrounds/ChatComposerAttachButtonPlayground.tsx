...
import { ChatComposer, ChatComposerAttachButton, ChatComposerTextArea } from '@mui/x-chat';
...
export function ChatComposerAttachButtonPlayground() {
      ...
      preview={
        <ScopedChat conversations={[emptyConversation]} activeConversationId={emptyConversation.id}>
          <Box sx={{ width: '100%' }}>
            <ChatComposer
              features={{
                attachments: { acceptedMimeTypes: accept.split(',').map((s) => s.trim()) },
              }}
            >
